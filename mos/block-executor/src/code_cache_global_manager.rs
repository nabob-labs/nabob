...
macro_rules! alert_or_println {
    ($($arg:tt)*) => {
        if cfg!(any(test, feature = "testing")) {
            ...
            use nabob_vm_logging::{alert, prelude::CRITICAL_ERRORS};
            use nabob_logger::error;
            alert!($($arg)*);
        }
    };
}
...
pub struct ModuleCacheManager<K, D, V, E> {
    ...
    environment: Option<NabobEnvironment>,
    ...
    E: WithSize,
{
    ...
    fn check_ready(
        &mut self,
        storage_environment: NabobEnvironment,
        config: &BlockExecutorModuleCacheLocalConfig,
        transaction_slice_metadata: TransactionSliceMetadata,
    ) -> Result<(), VMStatus> {
...
/// Module cache manager used by Nabob block executor. Ensures that only one thread has exclusive
/// access to it at a time.
pub struct NabobModuleCacheManager {
    inner: Mutex<ModuleCacheManager<ModuleId, CompiledModule, Module, NabobModuleExtension>>,
}

impl NabobModuleCacheManager {
    ...
    /// is ready for execution and updates states. If fails, [NabobModuleCacheManagerGuard::None]
    ...
    fn try_lock_inner(
        ...
        config: &BlockExecutorModuleCacheLocalConfig,
        transaction_slice_metadata: TransactionSliceMetadata,
    ) -> Result<NabobModuleCacheManagerGuard<'_>, VMStatus> {
        ...
        Ok(match self.inner.try_lock() {
            Some(mut guard) => {
                ...
                NabobModuleCacheManagerGuard::Guard { guard }
            },
            None => {
    ...
    /// Tries to lock the manager using [NabobModuleCacheManager::try_lock_inner]. Additionally, if
    /// the module cache is empty, can prefetch Nabob framework into it.
    pub fn try_lock(
        &self,
        state_view: &impl StateView,
        config: &BlockExecutorModuleCacheLocalConfig,
...
/// A guard that can be acquired from [NabobModuleCacheManager]. Variants represent successful and
/// no-successful lock acquisition.
pub enum NabobModuleCacheManagerGuard<'a> {
    /// Holds the guard to the [NabobModuleCacheManager], and has exclusive access to it.
    Guard {
        guard: MutexGuard<
            'a,
            ModuleCacheManager<ModuleId, CompiledModule, Module, NabobModuleExtension>,
        >,
    },
    /// Either there is no [NabobModuleCacheManager], or acquiring the lock for it failed.
    None {
        environment: NabobEnvironment,
        module_cache: GlobalModuleCache<ModuleId, CompiledModule, Module, NabobModuleExtension>,
    },
}

impl<'a> NabobModuleCacheManagerGuard<'a> {
    /// Returns the references to the environment. If environment is not set, panics.
    pub fn environment(&self) -> &NabobEnvironment {
        use NabobModuleCacheManagerGuard::*;
        match self {
            Guard { guard } => guard
                .environment
                .as_ref()
    ...
    pub fn module_cache(
        &self,
    ) -> &GlobalModuleCache<ModuleId, CompiledModule, Module, NabobModuleExtension> {
        use NabobModuleCacheManagerGuard::*;
        match self {
    ...
    pub fn module_cache_mut(
        &mut self,
    ) -> &mut GlobalModuleCache<ModuleId, CompiledModule, Module, NabobModuleExtension> {
        use NabobModuleCacheManagerGuard::*;
        match self {
    ...
    /// A guard in [NabobModuleCacheManagerGuard::None] state with empty module cache and default
...
fn prefetch_nabob_framework<S: StateView>(
    ...
    let maybe_loaded = code_storage
        ...
        .map_err(|err| {
            ...
            PanicError::CodeInvariantError(format!("Unable to fetch Nabob framework: {:?}", err))
        })?;
...
mod test {
    ...
    #[test]
    fn test_prefetch_existing_nabob_framework() {
    ...
    fn test_check_ready() {
        ...
        assert_ok!(manager.check_ready(NabobEnvironment::new(&state_view), &config, metadata_1));
        ...
        assert_ok!(manager.check_ready(NabobEnvironment::new(&state_view), &config, metadata_2));
        ...
        assert_ok!(manager.check_ready(NabobEnvironment::new(&state_view), &config, metadata_3));
        ...
        assert_ok!(manager.check_ready(NabobEnvironment::new(&state_view), &config, metadata_4));
        ...
        assert_ok!(manager.check_ready(NabobEnvironment::new(&state_view), &config, metadata_5));
        ...
        assert_ok!(manager.check_ready(NabobEnvironment::new(&state_view), &config, metadata_6));
    ...
    fn test_try_lock_inner_single_thread() {
        let manager = NabobModuleCacheManager::new();
        ...
        assert!(matches!(guard, NabobModuleCacheManagerGuard::Guard { .. }));
    }

    #[test]
    fn test_try_lock_inner_multiple_threads() {
        let manager = Arc::new(NabobModuleCacheManager::new());
        ...
        for _ in 0..num_threads {
            let handle = std::thread::spawn({
                ...
                move || {
                    ...
                    if matches!(guard, NabobModuleCacheManagerGuard::Guard { .. }) {
                        1
                    } else {
                        0
                    }
