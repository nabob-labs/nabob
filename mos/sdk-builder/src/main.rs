...
//!
//! '''bash
//! cargo run -p nabob-sdk-builder -- --help
//! '''
...
#[derive(Debug, Parser)]
#[clap(name = "Nabob SDK Builder", about = "Generate boilerplate Nabob SDKs")]
struct Options {
    ...
    /// Also install the nabob types described by the given YAML file, along with the BCS runtime.
    #[clap(long)]
    with_nabob_types: Option<PathBuf>,
    ...
    /// and `nabob_types` is assumed to be in "go_module/path/nabob_types".
    #[clap(long)]
    ...
    /// Optional version number for the `nabob_types` module (useful in Rust).
    /// If `--with-nabob-types` is passed, this will be the version of the generated `nabob_types` module.
    #[clap(long, default_value = "0.1.0")]
    nabob_version_number: String,

    /// Optional package name (Python) or module path (Go) of the `nabob_types` dependency.
    #[clap(long)]
...
fn main() {
    ...
    let abis = nabob_sdk_builder::read_abis(&options.abi_directories)
    ...
    let install_dir = match options.target_source_dir {
        None => {
            ...
            match options.language {
                Language::Rust => {
                    nabob_sdk_builder::rust::output(&mut out, &abis, /* local types */ true)
                        .unwrap()
                },
                Language::Go => {
                    nabob_sdk_builder::golang::output(
                        &mut out,
    ...
    // Nabob types
    if let Some(registry_file) = options.with_nabob_types {
        ...
        if let Language::Rust = options.language {
            nabob_sdk_builder::rust::replace_keywords(&mut registry)
        }

        let (package_name, _package_path) = match options.language {
            Language::Rust => (
                if options.nabob_version_number == "0.1.0" {
                    "nabob-types".to_string()
                } else {
                    format!("nabob-types:{}", options.nabob_version_number)
                },
                vec!["nabob-types"],
            ),
            Language::Go => ("nabobtypes".to_string(), vec!["nabobtypes"]),
        };
    ...
    let installer: Box<dyn nabob_sdk_builder::SourceInstaller<Error = Box<dyn std::error::Error>>> =
        match options.language {
            Language::Rust => Box::new(nabob_sdk_builder::rust::Installer::new(
                install_dir,
                options.nabob_version_number,
            )),
            Language::Go => Box::new(nabob_sdk_builder::golang::Installer::new(
                install_dir,
