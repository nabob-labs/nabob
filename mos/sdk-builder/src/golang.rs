...
pub fn output(
    ...
    nabob_module_path: Option<String>,
    package_name: String,
    abis: &[EntryABI],
) -> Result<()> {
        ...
        .filter(|abi| {
            if let EntryABI::EntryFunction(sf) = abi {
                sf.module_name().name().as_str() != "code"
                    ...
                    && sf.module_name().name().as_str() != "nabob_account"
...
struct GoEmitter<T> {
    ...
    serde_module_path: Option<String>,
    /// Go module path for Nabob types.
    ...
    T: Write,
{
    fn output_script_call_enum_with_imports(&mut self, abis: &[EntryABI]) -> Result<()> {
        let nabob_types_package = match &self.nabob_module_path {
            Some(path) => format!("{}/nabobtypes", path),
            None => "nabobtypes".into(),
        };
        let mut external_definitions =
            crate::common::get_external_definitions(&nabob_types_package);
        // We need BCS for argument encoding and decoding
        external_definitions.insert(
            "github.com/nabob-labs/serde-reflection/serde-generate/runtime/golang/bcs".to_string(),
            Vec::new(),
        );
    ...
    fn output_encode_method(&mut self, abis: &[EntryABI]) -> Result<()> {
        ...
        if !transaction_script_abis.is_empty() {
            writeln!(
                self.out,
                r#"
// Build an Nabob `Script` from a structured object `ScriptCall`.
func EncodeScript(call ScriptCall) nabobtypes.Script {{"#
            )?;
            self.out.indent();
        ...
        if !entry_fun_abis.is_empty() {
            writeln!(
                self.out,
                r#"
// Build an Nabob `TransactionPayload` from a structured object `EntryFunctionCall`.
func EncodeEntryFunction(call EntryFunctionCall) nabobtypes.TransactionPayload {{"#
            )?;
            self.out.indent();
            writeln!(self.out, "switch call := call.(type) {{")?;
    ...
    fn output_transaction_script_decode_method(&mut self) -> Result<()> {
        writeln!(
            self.out,
            r#"
// Try to recognize an Nabob `Script` and convert it into a structured object `ScriptCall`.
func DecodeScript(script *nabobtypes.Script) (ScriptCall, error) {{
    ...
    fn output_entry_function_decode_method(&mut self) -> Result<()> {
        writeln!(
            self.out,
            r#"
// Try to recognize an Nabob `TransactionPayload` and convert it into a structured object `EntryFunctionCall`.
func DecodeEntryFunctionPayload(script nabobtypes.TransactionPayload) (EntryFunctionCall, error) {{
    switch script := script.(type) {{
        case *nabobtypes.TransactionPayload__EntryFunction:
    ...
    fn output_transaction_script_encoder_function(
        ...
        writeln!(
            self.out,
            r#"return nabobtypes.Script {{
	Code: append([]byte(nil), {}_code...),
	TyArgs: []nabobtypes.TypeTag{{{}}},
	Args: []nabobtypes.TransactionArgument{{{}}},
}}"#,
            abi.name(),
    ...
    fn output_entry_function_encoder_function(&mut self, abi: &EntryFunctionABI) -> Result<()> {
        writeln!(
            self.out,
            "\n{}\nfunc Encode{}{}({}) nabobtypes.TransactionPayload {{",
            Self::quote_doc(abi.doc()),
            abi.module_name().name().to_string().to_upper_camel_case(),
            abi.name().to_upper_camel_case(),
            [
        ...
        writeln!(
            self.out,
            r#"return &nabobtypes.TransactionPayload__EntryFunction {{
            nabobtypes.EntryFunction {{
                Module: {},
                Function: {},
                TyArgs: []nabobtypes.TypeTag{{{}}},
                Args: [][]byte{{{}}},
    ...
    fn output_transaction_script_decoder_function(
        ...
        writeln!(
            self.out,
            "\nfunc decode_{}(script *nabobtypes.Script) (ScriptCall, error) {{",
            abi.name(),
        )?;
        self.out.indent();
        writeln!(
...
"#,
    ...
    fn output_entry_function_decoder_function(&mut self, abi: &EntryFunctionABI) -> Result<()> {
        writeln!(
            self.out,
            "\nfunc decode_{}_{}(script nabobtypes.TransactionPayload) (EntryFunctionCall, error) {{",
...
"#,
    ...
    fn output_transaction_script_decoder_map(
        ...
        writeln!(
            self.out,
            r#"
var script_decoder_map = map[string]func(*nabobtypes.Script) (ScriptCall, error) {{"#
        )?;
        self.out.indent();
        for abi in abis {
    ...
    fn output_entry_function_decoder_map(&mut self, abis: &[EntryFunctionABI]) -> Result<()> {
        writeln!(
            self.out,
            r#"
var entry_function_decoder_map = map[string]func(nabobtypes.TransactionPayload) (EntryFunctionCall, error) {{"#
        )?;
        self.out.indent();
        for abi in abis {
            writeln!(
...
"#,
    ...
    fn output_decoding_helper(&mut self, type_tag: &TypeTag) -> Result<()> {
        ...
        writeln!(
            self.out,
            r#"
func decode_{0}_argument(arg nabobtypes.TransactionArgument) (value {1}, err error) {{
	if arg, ok := arg.(*nabobtypes.TransactionArgument__{2}); ok {{
		{3}
	}} else {{
...
"#,
    ...
    fn quote_module_id(module_id: &ModuleId) -> String {
        format!(
            "nabobtypes.ModuleId {{ Address: {}, Name: {} }}",
    ...
    fn quote_type_parameters(ty_args: &[TypeArgumentABI]) -> Vec<String> {
        ty_args
            .iter()
            .map(|ty_arg| format!("{} nabobtypes.TypeTag", ty_arg.name()))
            .collect()
    }
    ...
    fn quote_type(type_tag: &TypeTag) -> String {
        ...
        match type_tag {
            ...
            U256 => unimplemented!(),
            Address => "nabobtypes.AccountAddress".into(),
            Vector(type_tag) => {
    ...
    fn quote_transaction_argument_for_script(type_tag: &TypeTag, name: &str) -> String {
        use TypeTag::*;
        match type_tag {
            Bool => format!("(*nabobtypes.TransactionArgument__Bool)(&{})", name),
            U8 => format!("(*nabobtypes.TransactionArgument__U8)(&{})", name),
            U16 => format!("(*nabobtypes.TransactionArgument__U16)(&{})", name),
            U32 => format!("(*nabobtypes.TransactionArgument__U32)(&{})", name),
            U64 => format!("(*nabobtypes.TransactionArgument__U64)(&{})", name),
            U128 => format!("(*nabobtypes.TransactionArgument__U128)(&{})", name),
            U256 => format!("(*nabobtypes.TransactionArgument__U256)(&{})", name),
            Address => format!("&nabobtypes.TransactionArgument__Address{{{}}}", name),
            Vector(type_tag) => match type_tag.as_ref() {
                U8 => format!("(*nabobtypes.TransactionArgument__U8Vector)(&{})", name),
                _ => common::type_not_allowed(type_tag),
            },
            Struct(_) | Signer | Function(..) => common::type_not_allowed(type_tag),
        }
