...
/// If `local_types` is true, we generate a file suitable for the Nabob codebase itself
...
struct RustEmitter<T> {
    ...
    /// Whether we are targeting the Nabob repository itself (as opposed to generated Nabob types).
    local_types: bool,
}
    ...
    fn get_external_definitions(local_types: bool) -> serde_generate::ExternalDefinitions {
        let definitions = if local_types {
            vec![
                ...
                ("nabob_types::transaction", vec![
                    "TransactionPayload",
                    "EntryFunction",
                ]),
                ("nabob_types::account_address", vec!["AccountAddress"]),
            ]
        } else {
            vec![("nabob_types", vec![
                "AccountAddress",
...
pub fn decode(payload: &TransactionPayload) -> Option<EntryFunctionCall> {{
    ...
    fn output_transaction_script_name_method(
        ...
        writeln!(
            self.out,
            r#"
/// Return the name of an Nabob `Script` from a structured object `ScriptCall`.
...
pub struct Installer {
    install_dir: PathBuf,
    nabob_types_version: String,
}

impl Installer {
    pub fn new(install_dir: PathBuf, nabob_types_version: String) -> Self {
        Installer {
            install_dir,
            nabob_types_version,
        }
...
serde_bytes = "0.11.6"
nabob-types = {{ path = "../nabob-types", version = "{}" }}
"#,
