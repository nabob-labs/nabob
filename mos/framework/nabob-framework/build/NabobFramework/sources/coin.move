...
module nabob_framework::coin {
    ...
    use nabob_framework::guid;
    use nabob_framework::optional_aggregator::{Self, OptionalAggregator};
    use nabob_framework::permissioned_signer;
    use nabob_framework::system_addresses;

    use nabob_framework::fungible_asset::{Self, FungibleAsset, Metadata, MintRef, TransferRef, BurnRef};
    use nabob_framework::object::{Self, Object, object_address};
    use nabob_framework::primary_fungible_store;
    use nabob_std::type_info::{Self, TypeInfo};
    use nabob_framework::create_signer;

    friend nabob_framework::nabob_coin;
    ...
    #[resource_group_member(group = nabob_framework::object::ObjectGroup)]
    ...
    public fun paired_metadata<CoinType>(): Option<Object<Metadata>> acquires CoinConversionMap {
        if (exists<CoinConversionMap>(@nabob_framework) && features::coin_to_fungible_asset_migration_feature_enabled(
        )) {
            let map = &borrow_global<CoinConversionMap>(@nabob_framework).coin_to_fungible_asset_map;
    ...
    public entry fun create_coin_conversion_map(nabob_framework: &signer) {
        system_addresses::assert_nabob_framework(nabob_framework);
        if (!exists<CoinConversionMap>(@nabob_framework)) {
            move_to(nabob_framework, CoinConversionMap {
    ...
    inline fun create_and_return_paired_metadata_if_not_exist<CoinType>(allow_bob_creation: bool): Object<Metadata> {
        ...
        assert!(exists<CoinConversionMap>(@nabob_framework), error::not_found(ECOIN_CONVERSION_MAP_NOT_FOUND));
            ...
            let metadata_object_cref =
                if (is_bob) {
                    ...
                    object::create_named_object(
                        &create_signer::create_signer(@nabob_fungible_asset),
    ...
    public fun allow_supply_upgrades(_nabob_framework: &signer, _allowed: bool) {
    ...
    fun mint_internal<CoinType>(amount: u64): Coin<CoinType> acquires CoinInfo {
        ...
        if (option::is_some(maybe_supply)) {
            ...
            spec {
                ...
                use nabob_framework::aggregator;
    ...
    #[test(source = @0x2, framework = @nabob_framework)]
    ...
    #[test(account = @0x1)]
    #[expected_failure(abort_code = 0x50003, location = nabob_framework::fungible_asset)]
    ...
    #[test(framework = @nabob_framework, other = @0x123)]
    ...
    #[test(framework = @nabob_framework)]
    ...
    #[test(framework = @nabob_framework)]
    #[expected_failure(abort_code = 0x20001, location = nabob_framework::aggregator)]
    ...
    #[test(account = @nabob_framework, aaron = @0xcafe)]
    ...
    #[test(account = @nabob_framework, aaron = @0xaa10, bob = @0xb0b)]
    ...
    #[test(account = @nabob_framework)]
    #[expected_failure(abort_code = 0x50024, location = nabob_framework::fungible_asset)]
