module nabob_framework::account_abstraction {
    use std::bcs;
    use std::hash;
    ...
    use nabob_std::ordered_map::{Self, OrderedMap};
    use nabob_std::big_ordered_map::{Self, BigOrderedMap};
    use nabob_framework::create_signer;
    use nabob_framework::event;
    use nabob_framework::function_info::{Self, FunctionInfo};
    use nabob_framework::object;
    use nabob_framework::auth_data::AbstractionAuthData;
    use nabob_framework::system_addresses;
    use nabob_framework::permissioned_signer::{Self, is_permissioned_signer};
    #[test_only]
    use nabob_framework::account::create_account_for_test;
    #[test_only]
    use nabob_framework::auth_data;

    friend nabob_framework::transaction_validation;
    #[test_only]
    friend nabob_framework::account_abstraction_tests;
    ...
    public entry fun register_domain_with_authentication_function(
        ...
        borrow_global_mut<DomainDispatchableAuthenticator>(@nabob_framework).auth_functions.add(
            function_info::new_function_info_from_address(module_address, module_name, function_name),
            DomainRegisterValue::Empty,
        );
    }

    entry fun initialize(nabob_framework: &signer) {
        system_addresses::assert_nabob_framework(nabob_framework);
        move_to(
            nabob_framework,
    ...
    inline fun resource_addr(source: address): address {
        object::create_user_derived_object_address(source, @nabob_fungible_asset)
    }
    ...
    inline fun dispatchable_domain_authenticator_internal(): &BigOrderedMap<FunctionInfo, DomainRegisterValue> {
        assert!(exists<DomainDispatchableAuthenticator>(@nabob_framework), error::not_found(EDOMAIN_AA_NOT_INITIALIZED));
        &borrow_global<DomainDispatchableAuthenticator>(@nabob_framework).auth_functions
    }
    ...
    fun authenticate(
        ...
        // Returned signer MUST represent the same account address. Otherwise, it may break the invariant of Nabob blockchain!
        assert!(
