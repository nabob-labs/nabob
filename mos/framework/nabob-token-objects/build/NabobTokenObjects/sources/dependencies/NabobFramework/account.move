module nabob_framework::account {
    use std::bcs;
    use std::error;
    use std::hash;
    ...
    use std::signer;
    use std::vector;
    use nabob_framework::chain_id;
    use nabob_framework::create_signer::create_signer;
    ...
    use nabob_std::ed25519;
    use nabob_std::from_bcs;
    use nabob_std::multi_ed25519;
    ...
    friend nabob_framework::resource_account;
    ...
    #[test_only]
    /// Create signer for testing, independently of an Nabob-style `Account`.
    ...
    public(friend) fun initialize(nabob_framework: &signer) {
        ...
        move_to(nabob_framework, OriginatingAddress {
    ...
    public fun originating_address(auth_key: address): Option<address> acquires OriginatingAddress {
        let address_map_ref = &borrow_global<OriginatingAddress>(@nabob_framework).address_map;
    ...
    entry fun set_originating_address(account: &signer) acquires Account, OriginatingAddress {
        ...
        let address_map_ref_mut =
            &mut borrow_global_mut<OriginatingAddress>(@nabob_framework).address_map;
    ...
    fun update_auth_key_and_originating_address_table(
        ...
        let address_map = &mut borrow_global_mut<OriginatingAddress>(@nabob_framework).address_map;
    ...
    /// `create_account` to attempt to claim an account ahead of the creation of a resource account, if found Nabob will
    ...
    fun test_create_signer_for_test() {
        assert!(signer::address_of(&create_signer_for_test(@nabob_framework)) == @0x1, 0);
    ...
    #[test(alice = @0xa11ce)]
    #[expected_failure(abort_code = 65537, location = nabob_framework::ed25519)]
    ...
    //
    #[test(bob = @0x345, framework = @nabob_framework)]
    ...
    #[test(bob = @0x345, charlie = @0x567, framework = @nabob_framework)]
