...
impl ReleasePackage {
    ...
    fn generate_script_proposal_impl(
        ...
        emitln!(writer, "use nabob_framework::nabob_governance;");
        emitln!(writer, "use nabob_framework::code;\n");
        ...
        if is_testnet && !is_multi_step {
            ...
            emitln!(
                writer,
                "let framework_signer = nabob_governance::get_signer_testnet_only(core_resources, @{});",
                for_address
            );
            ...
            emitln!(
                writer,
                "let framework_signer = nabob_governance::resolve(proposal_id, @{});",
                for_address
            );
        } else {
            emitln!(writer, "fun main(proposal_id: u64){");
...
pub fn generate_next_execution_hash_blob(
    ...
    match next_execution_hash {
        None => {
            emitln!(
            writer,
            "let framework_signer = nabob_governance::resolve_multi_step_proposal(proposal_id, @{}, {});\n",
            for_address,
            "x\"\"",
        );
        },
        Some(next_execution_hash) => {
            emitln!(
                writer,
                "let framework_signer = nabob_governance::resolve_multi_step_proposal("
            );
            writer.indent();
