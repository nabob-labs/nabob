...
static NAMED_ADDRESSES: Lazy<BTreeMap<String, NumericalAddress>> = Lazy::new(|| {
    ...
    result.insert("nabob_std".to_owned(), one);
    result.insert("nabob_framework".to_owned(), one);
    result.insert("nabob_token".to_owned(), three);
    result.insert("nabob_token_objects".to_owned(), four);
    result.insert("nabob_fungible_asset".to_owned(), ten);
