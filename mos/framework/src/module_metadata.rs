...
use nabob_vm_types::module_and_script_storage::module_storage::NabobModuleStorage;
use lru::LruCache;
...
/// Nabob specific metadata (`nabob::` here).
pub static NABOB_METADATA_KEY: &[u8] = "nabob::metadata_v0".as_bytes();
pub static NABOB_METADATA_KEY_V1: &[u8] = "nabob::metadata_v1".as_bytes();

/// Nabob specific metadata attached to the metadata section of file_format.
...
/// V1 of Nabob specific metadata attached to the metadata section of file_format.
...
pub fn get_metadata(md: &[Metadata]) -> Option<Arc<RuntimeModuleMetadataV1>> {
    if let Some(data) = md.iter().find(|md| md.key == NABOB_METADATA_KEY_V1) {
...
pub fn get_metadata_v0(md: &[Metadata]) -> Option<Arc<RuntimeModuleMetadataV1>> {
    if let Some(data) = md.iter().find(|md| md.key == NABOB_METADATA_KEY) {
...
pub fn check_metadata_format(
    ...
    for data in module.metadata.iter() {
        if data.key == *NABOB_METADATA_KEY || data.key == *NABOB_METADATA_KEY_V1 {
            if exist {
                return Err(MalformedError::DuplicateKey);
            }
            exist = true;

            if data.key == *NABOB_METADATA_KEY {
                bcs::from_bytes::<RuntimeModuleMetadata>(&data.value)
                    .map_err(|e| MalformedError::DeserializedError(data.key.clone(), e))?;
            } else if data.key == *NABOB_METADATA_KEY_V1 {
...
pub fn get_metadata_from_compiled_module(
    ...
    if let Some(data) = find_metadata(module, NABOB_METADATA_KEY_V1) {
        ...
        metadata
    } else if let Some(data) = find_metadata(module, NABOB_METADATA_KEY) {
...
pub fn get_metadata_from_compiled_script(
    ...
    if let Some(data) = find_metadata_in_script(script, NABOB_METADATA_KEY_V1) {
        let mut metadata = bcs::from_bytes::<RuntimeModuleMetadataV1>(&data.value).ok();
        // Clear out metadata for v5, since it shouldn't have existed in the first place and isn't
        // being used. Note, this should have been gated in the verify module metadata.
        if script.version == 5 {
        ...
        metadata
    } else if let Some(data) = find_metadata_in_script(script, NABOB_METADATA_KEY) {
        // Old format available, upgrade to new one on the fly
        let data_v0 = bcs::from_bytes::<RuntimeModuleMetadata>(&data.value).ok()?;
        Some(data_v0.upgrade())
    } else {
