...
module nabob_framework::vesting {
    use std::bcs;
    use std::error;
    ...
    use nabob_std::pool_u64::{Self, Pool};
    ...
    use nabob_framework::staking_contract;
    ...
    const VESTING_POOL_SALT: vector<u8> = b"nabob_framework::vesting";
    ...
    public fun create_vesting_contract(
        ...
        buy_ins: SimpleMap<address, Coin<NabobCoin>>,
        vesting_schedule: VestingSchedule,
        withdrawal_address: address,
        operator: address,
        voter: address,
        ...
        let admin_address = permissioned_signer::address_of(admin);
        ...
        let contract_address = permissioned_signer::address_of(&contract_signer);
    ...
    public entry fun distribute(contract_address: address) acquires VestingContract {
        ...
        vector::for_each_ref(shareholders, |shareholder| {
            ...
            nabob_account::deposit_coins(recipient_address, share_of_coins);
        });

        // Send any remaining "dust" (leftover due to rounding error) to the withdrawal address.
        if (coin::value(&coins) > 0) {
            nabob_account::deposit_coins(vesting_contract.withdrawal_address, coins);
        } else {
            coin::destroy_zero(coins);
        };
    ...
    fun create_vesting_contract_account(
        ...
        check_vest_permission(admin);
        let admin_store = borrow_global_mut<AdminStore>(permissioned_signer::address_of(admin));
        let seed = bcs::to_bytes(&permissioned_signer::address_of(admin));
        ...
        // Register the vesting contract account to receive BOB as it'll be sent to it when claiming unlocked stake from
    ...
    fun verify_admin(admin: &signer, vesting_contract: &VestingContract) {
        check_vest_permission(admin);
        assert!(permissioned_signer::address_of(admin) == vesting_contract.admin, error::unauthenticated(ENOT_ADMIN));
    }
    ...
    fun withdraw_stake(vesting_contract: &VestingContract, contract_address: address): Coin<NabobCoin> {
        ...
        let withdrawn_coins = coin::balance<NabobCoin>(contract_address);
        let contract_signer = &get_vesting_account_signer_internal(vesting_contract);
        coin::withdraw<NabobCoin>(contract_signer, withdrawn_coins)
    }
    ...
    #[test_only]
    use nabob_framework::account::create_account_for_test;
    use nabob_std::math64::min;

    #[test_only]
    const MIN_STAKE: u64 = 100000000000000; // 1M BOB coins with 8 decimals.

    #[test_only]
    const GRANT_AMOUNT: u64 = 20000000000000000; // 200M BOB coins with 8 decimals.
    ...
    #[test_only]
    public fun setup(nabob_framework: &signer, accounts: &vector<address>) {
        use nabob_framework::nabob_account::create_account;
        ...
        std::features::change_feature_flags_for_testing(nabob_framework, vector[], vector[std::features::get_periodical_reward_rate_decrease_feature()]);
    }
    ...
    public fun setup_vesting_contract_with_schedule(
        ...
        let admin_address = permissioned_signer::address_of(admin);
        let buy_ins = simple_map::create<address, Coin<NabobCoin>>();
    ...
    #[test(nabob_framework = @0x1, admin = @0x123, shareholder_1 = @0x234, shareholder_2 = @0x345, withdrawal = @111)]
    public entry fun test_end_to_end(
        ...
        setup(
            nabob_framework, &vector[admin_address, withdrawal_address, shareholder_1_address, shareholder_2_address]);
        ...
        distribute(contract_address);
        let shareholder_1_bal = coin::balance<NabobCoin>(shareholder_1_address);
        let shareholder_2_bal = coin::balance<NabobCoin>(shareholder_2_address);
        ...
        assert!(coin::balance<NabobCoin>(shareholder_1_address) == shareholder_1_bal + pending_distribution / 4, 0);
        assert!(coin::balance<NabobCoin>(shareholder_2_address) == shareholder_2_bal + pending_distribution * 3 / 4, 1);
        // Withdrawal address receives the left-over dust of 1 coin due to rounding error.
        assert!(coin::balance<NabobCoin>(withdrawal_address) == 1, 0);
        ...
        let previous_bal = coin::balance<NabobCoin>(withdrawal_address);
        admin_withdraw(admin, contract_address);
        assert!(coin::balance<NabobCoin>(withdrawal_address) == previous_bal + withdrawn_amount, 0);
    }

    #[test(nabob_framework = @0x1, admin = @0x123)]
    ...
    public entry fun test_create_vesting_contract_with_zero_grant_should_fail(
        ...
        let admin_address = signer::address_of(admin);
        setup(nabob_framework, &vector[admin_address]);
    ...
    #[test(nabob_framework = @0x1, admin = @0x123)]
    #[expected_failure(abort_code = 0x60001, location = nabob_framework::nabob_account)]
    ...
    #[test(nabob_framework = @0x1, admin = @0x123)]
    #[expected_failure(abort_code = 0x60002, location = nabob_framework::nabob_account)]
    ...
    public entry fun test_create_empty_vesting_schedule_should_fail(nabob_framework: &signer) {
        setup(nabob_framework, &vector[]);
        create_vesting_schedule(vector[], 1, 1);
    }

    #[test(nabob_framework = @0x1)]
    #[expected_failure(abort_code = 0x10003, location = Self)]
    public entry fun test_create_vesting_schedule_with_zero_period_duration_should_fail(nabob_framework: &signer) {
        setup(nabob_framework, &vector[]);
    ...
    public entry fun test_create_vesting_schedule_with_invalid_vesting_start_should_fail(nabob_framework: &signer) {
        setup(nabob_framework, &vector[]);
    ...
    #[test(nabob_framework = @0x1, admin = @0x123, shareholder = @0x234)]
    public entry fun test_vest_twice_should_not_double_count(
        ...
        setup(nabob_framework, &vector[admin_address, shareholder_address]);
    ...
    #[test(nabob_framework = @0x1, admin = @0x123, shareholder = @0x234, operator = @0x345)]
    public entry fun test_unlock_rewards_should_pay_commission_first(
        ...
        let shareholder_address = signer::address_of(shareholder);
        setup(nabob_framework, &vector[admin_address, shareholder_address, operator_address]);
        let contract_address = setup_vesting_contract(
            admin, &vector[shareholder_address], &vector[GRANT_AMOUNT], admin_address, 0);
        ...
        assert!(coin::balance<NabobCoin>(shareholder_address) == staker_rewards + 1, 0);
        assert!(coin::balance<NabobCoin>(operator_address) == commission - 1, 1);
    }
    ...
    #[test(nabob_framework = @0x1, admin = @0x123, operator = @0x345)]
    public entry fun test_update_operator_with_same_commission(
        ...
        let admin_address = signer::address_of(admin);
        let operator_address = signer::address_of(operator);
        setup(nabob_framework, &vector[admin_address, @11, operator_address]);
        let contract_address = setup_vesting_contract(
    ...
    public entry fun test_commission_percentage_change(
        ...
        assert!(coin::balance<NabobCoin>(operator_address) == expected_commission, 1);
    }
    ...
    public entry fun test_set_beneficiary_for_operator(
        ...
        setup(nabob_framework, &vector[admin_address, shareholder_address, operator_address1, beneficiary_address]);
        let contract_address = setup_vesting_contract(
            admin, &vector[shareholder_address], &vector[GRANT_AMOUNT], admin_address, 0);
        assert!(operator_commission_percentage(contract_address) == 0, 0);
        let stake_pool_address = stake_pool_address(contract_address);
        ...
        assert!(coin::balance<NabobCoin>(operator_address1) == 0, 1);
        assert!(coin::balance<NabobCoin>(beneficiary_address) == expected_commission, 1);
        let old_beneficiay_balance = coin::balance<NabobCoin>(beneficiary_address);
        ...
        assert!(coin::balance<NabobCoin>(operator_address2) >= expected_commission, 1);
        assert!(coin::balance<NabobCoin>(beneficiary_address) == old_beneficiay_balance, 1);
    }
    ...
    public entry fun test_set_beneficiary_should_send_distribution(
        ...
        let admin_address = signer::address_of(admin);
        setup(nabob_framework, &vector[admin_address, @11]);
        let contract_address = setup_vesting_contract(
        ...
        let vested_amount = fraction(GRANT_AMOUNT, 3, 48);
        distribute(contract_address);
        let balance = coin::balance<NabobCoin>(@11);
    ...
    public entry fun test_reset_beneficiary(
        ...
        let admin_address = signer::address_of(admin);
        setup(nabob_framework, &vector[admin_address, @11, @12]);
        let contract_address = setup_vesting_contract(
            admin, &vector[@11], &vector[GRANT_AMOUNT], admin_address, 0);
        ...
        let vested_amount = fraction(GRANT_AMOUNT, 3, 48);
        distribute(contract_address);
        assert!(coin::balance<NabobCoin>(@11) == vested_amount, 0);
        assert!(coin::balance<NabobCoin>(@12) == 0, 1);
    }

    #[test(nabob_framework = @0x1, admin = @0x123, resetter = @0x234)]
    ...
    #[test(nabob_framework = @0x1, admin = @0x123, resetter = @0x234, random = @0x345)]
