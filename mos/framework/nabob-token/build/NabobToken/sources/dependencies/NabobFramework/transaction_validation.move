module nabob_framework::transaction_validation {
    use std::bcs;
    use std::error;
    use std::features;
    use std::option;
    ...
    use nabob_framework::account_abstraction;
    ...
    use nabob_framework::transaction_fee;
    ...
    public(friend) fun initialize(
        ...
        move_to(nabob_framework, TransactionValidation {
    ...
    fun prologue_common(
        ...
        if (!features::transaction_simulation_enhancement_enabled() || !skip_gas_payment(
            ...
            if (features::operations_default_to_fa_bob_store_enabled()) {
                ...
                assert!(
                    coin::is_balance_at_least<NabobCoin>(gas_payer_address, max_transaction_fee),
                    error::invalid_argument(PROLOGUE_ECANT_PAY_GAS_DEPOSIT)
                );
            }
        }
    ...
    // Once the transaction_simulation_enhancement feature is enabled, the Nabob VM will invoke this function instead.
    ...
    fun epilogue_gas_payer_extended(
        ...
        if (!features::transaction_simulation_enhancement_enabled() || !skip_gas_payment(is_simulation, gas_payer)) {
            if (features::operations_default_to_fa_bob_store_enabled()) {
                ...
                assert!(
                    coin::is_balance_at_least<NabobCoin>(gas_payer, transaction_fee_amount),
                    error::out_of_range(PROLOGUE_ECANT_PAY_GAS_DEPOSIT),
                );
            };
    ...
    fun unified_epilogue(
        ...
        if (!features::transaction_simulation_enhancement_enabled() || !skip_gas_payment(
            ...
            if (features::operations_default_to_fa_bob_store_enabled()) {
                ...
                assert!(
                    coin::is_balance_at_least<NabobCoin>(gas_payer_address, transaction_fee_amount),
                    error::out_of_range(PROLOGUE_ECANT_PAY_GAS_DEPOSIT),
                );
            };
