module nabob_framework::nabob_account {
    use nabob_framework::account::{Self, new_event_handle};
    ...
    use nabob_framework::event::{EventHandle, emit_event, emit};
    use nabob_framework::fungible_asset::{Self, Metadata, BurnRef, FungibleAsset};
    ...
    use std::signer;
    use std::vector;
    use nabob_framework::object::Object;
    ...
    public entry fun transfer(source: &signer, to: address, amount: u64) {
        ...
        if (features::operations_default_to_fa_bob_store_enabled()) {
            ...
            coin::transfer<NabobCoin>(source, to, amount)
        }
    }
    ...
    public fun deposit_coins<CoinType>(to: address, coins: Coin<CoinType>) acquires DirectTransferConfig {
        if (!account::exists_at(to)) {
            create_account(to);
            spec {
                assert coin::spec_is_account_registered<NabobCoin>(to);
                assume nabob_std::type_info::type_of<CoinType>() == nabob_std::type_info::type_of<NabobCoin>() ==>
    ...
    inline fun ensure_primary_fungible_store_exists(owner: address): address {
        ...
        if (fungible_asset::store_exists(store_addr)) {
            ...
            object::object_address(&primary_fungible_store::create_primary_store(owner, object::address_to_object<Metadata>(@nabob_fungible_asset)))
        }
    }
    ...
    public fun test_transfer(alice: &signer, core: &signer) {
        ...
        let (burn_cap, mint_cap) = nabob_framework::nabob_coin::initialize_for_test(core);
        create_account(signer::address_of(alice));
        coin::deposit(signer::address_of(alice), coin::mint(10000, &mint_cap));
        transfer(alice, bob, 500);
        assert!(coin::balance<NabobCoin>(bob) == 500, 0);
        transfer(alice, carol, 500);
        assert!(coin::balance<NabobCoin>(carol) == 500, 1);
        transfer(alice, carol, 1500);
        assert!(coin::balance<NabobCoin>(carol) == 2000, 2);
    ...
    public fun test_transfer_to_resource_account(alice: &signer, core: &signer) {
        ...
        let (burn_cap, mint_cap) = nabob_framework::nabob_coin::initialize_for_test(core);
        assert!(!coin::is_account_registered<NabobCoin>(resource_acc_addr), 0);

        create_account(signer::address_of(alice));
        coin::deposit(signer::address_of(alice), coin::mint(10000, &mint_cap));
        transfer(alice, resource_acc_addr, 500);
        assert!(coin::balance<NabobCoin>(resource_acc_addr) == 500, 1);
    ...
    public fun test_batch_transfer(from: &signer, core: &signer, recipient_1: &signer, recipient_2: &signer) {
        ...
        assert!(coin::balance<NabobCoin>(recipient_1_addr) == 100, 0);
        assert!(coin::balance<NabobCoin>(recipient_2_addr) == 500, 1);
        coin::destroy_burn_cap(burn_cap);
        coin::destroy_mint_cap(mint_cap);
    }
