...
/// ## Overview
///
/// Nabob `NetworkAddress` is a compact, efficient, self-describing and
...
/// 4. Perform a NabobNet version negotiation handshake (version 1).
///
...
/// ## Transport
///
/// In addition, `NetworkAddress` is integrated with the NabobNet concept of a
...
///
/// use nabob_types::network_address::NetworkAddress;
/// use bcs;
...
impl NetworkAddress {
    ...
    ///
    /// ```rust
    /// use nabob_crypto::{traits::ValidCryptoMaterialStringExt, x25519};
    /// use nabob_types::network_address::NetworkAddress;
    /// use std::str::FromStr;
    ///
    ...
    /// Check that a `NetworkAddress` looks like a typical NabobNet address with
    /// associated protocols.
    ///
    /// "typical" NabobNet addresses begin with a transport protocol:
    ///
    ...
    /// assert!(addr.is_nabobnet_addr());
    /// ```
    pub fn is_nabobnet_addr(&self) -> bool {
        parse_nabobnet_protos(self.as_slice()).is_some()
    }
...
#[cfg(any(test, feature = "fuzzing"))]
pub fn arb_nabobnet_addr() -> impl Strategy<Value = NetworkAddress> {
    ...
    let arb_nabobnet_protos = any::<(x25519::PublicKey, u8)>()
        .prop_map(|(pubkey, hs)| vec![Protocol::NoiseIK(pubkey), Protocol::Handshake(hs)]);

    (arb_transport_protos, arb_nabobnet_protos).prop_map(
        |(mut transport_protos, mut nabobnet_protos)| {
            transport_protos.append(&mut nabobnet_protos);
...
/// parse canonical nabobnet protocols
///
/// See: [`NetworkAddress::is_nabobnet_addr`]
fn parse_nabobnet_protos(protos: &[Protocol]) -> Option<&[Protocol]> {
...
mod test {
    ...
    proptest! {
        #[test]
        fn test_network_address_canonical_serialization(addr in arb_nabobnet_addr()) {
        ...
        #[test]
        fn test_network_address_display_roundtrip(addr in arb_nabobnet_addr()) {
        ...
        #[test]
        fn test_is_nabobnet_addr(addr in arb_nabobnet_addr()) {
            assert!(addr.is_nabobnet_addr(), "addr.is_nabobnet_addr() = false; addr: '{}'", addr);
        }
    }
}
