...
pub fn coin_store_location(address: AccountAddress) -> StorageLocation {
    StorageLocation::Specific(
        StateKey::resource_typed::<CoinStoreResource<NabobCoinType>>(&address).unwrap(),
    )
}
...
pub fn nabob_coin_info_location() -> StorageLocation {
    StorageLocation::Specific(
        StateKey::resource_typed::<CoinInfoResource<NabobCoinType>>(
            &NabobCoinType::coin_info_address(),
        )
        .unwrap(),
    )
}
                ...
                TransactionPayload::EntryFunction(func) => {
                    match (
                        *func.module().address(),
                        ...
                        (AccountAddress::ONE, "nabob_account", "transfer") => {
                            let sender_address = signed_txn.sender();
                            let receiver_address = bcs::from_bytes(&func.args()[0]).unwrap();
                            rw_set_for_coin_transfer(
                                sender_address,
                        ...
                        (AccountAddress::ONE, "nabob_account", "create_account") => {
                            let sender_address = signed_txn.sender();
                            let receiver_address = bcs::from_bytes(&func.args()[0]).unwrap();
