...
pub struct NabobCodeStorageAdapter<'ctx, S, E> {
    storage: UnsyncCodeStorage<UnsyncModuleStorage<'ctx, StateViewAdapter<'ctx, S, E>>>,
}

impl<'ctx, S, E> NabobCodeStorageAdapter<'ctx, S, E>
where
    S: StateView,
    E: WithRuntimeEnvironment,
{
...
pub trait AsNabobCodeStorage<'ctx, S, E> {
    fn as_nabob_code_storage(
        &'ctx self,
        environment: &'ctx E,
    ) -> NabobCodeStorageAdapter<'ctx, S, E>;
}

impl<'ctx, S, E> AsNabobCodeStorage<'ctx, S, E> for S
where
    S: StateView,
    E: WithRuntimeEnvironment,
{
    fn as_nabob_code_storage(&'ctx self, environment: &'ctx E) -> NabobCodeStorageAdapter<S, E> {
        let adapter = StateViewAdapter {
            environment,
            state_view: self,
        };
        let storage = adapter.into_unsync_code_storage();
        NabobCodeStorageAdapter { storage }
    }
}
