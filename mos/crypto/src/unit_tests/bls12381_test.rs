...
use crate::{
    ...
    test_utils::{random_subset, KeyPair, TestNabobCrypto},
...
fn random_message_for_signing(rng: &mut OsRng) -> TestNabobCrypto {
    TestNabobCrypto(
...
fn random_messages_for_signing(rng: &mut OsRng, n: usize) -> Vec<TestNabobCrypto> {
    (0..n)
        .map(|_| random_message_for_signing(rng))
        .collect::<Vec<TestNabobCrypto>>()
}
...
fn bls12381_aggsig_should_verify() {
    ...
    let msgs_refs = messages.iter().collect::<Vec<&TestNabobCrypto>>();
    ...
    let msgs_wrong_refs = messages_wrong.iter().collect::<Vec<&TestNabobCrypto>>();
...
/// mos/framework/move-stdlib/sources/signer.move
...
fn bls12381_sample_signature_verifies() {
    ...
    assert!(sig.verify_arbitrary_msg(b"Hello Nabob!", &pk).is_ok());
}
...
fn bls12381_sample_doc_test_for_normal_sigs() {
    ...
    println!("// signature on TestNabobCrypto(\"test\".to_owned())");
    println!(
...
fn bls12381_sample_aggregate_pk_and_aggsig() {
    ...
    println!(
        "// The signed messages are \"Hello, Nabob <i>!\", where <i> \\in {{1, ..., {}}}",
        num
    );
