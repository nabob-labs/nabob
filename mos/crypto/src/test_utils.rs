...
) -> impl Strategy<Value = (EdwardsPoint, EdwardsPoint, TestNabobCrypto)> {
    (
...
pub struct TestNabobCrypto(pub String);
...
// #[cfg(any(test, feature = "fuzzing"))]
pub struct TestNabobCryptoHasher(crate::hash::DefaultHasher);
// #[cfg(any(test, feature = "fuzzing"))]
impl ::core::clone::Clone for TestNabobCryptoHasher {
    #[inline]
    fn clone(&self) -> TestNabobCryptoHasher {
        match *self {
            TestNabobCryptoHasher(ref __self_0_0) => {
                TestNabobCryptoHasher(::core::clone::Clone::clone(__self_0_0))
            },
        }
    }
}
...
// #[cfg(any(test, feature = "fuzzing"))]
impl TestNabobCryptoHasher {
    fn new() -> Self {
        let name = crate::_serde_name::trace_name::<TestNabobCrypto>()
            .expect("The `CryptoHasher` macro only applies to structs and enums");
        TestNabobCryptoHasher(crate::hash::DefaultHasher::new(name.as_bytes()))
    }
}
// #[cfg(any(test, feature = "fuzzing"))]
static TEST_CRYPTO_HASHER: crate::_once_cell::sync::Lazy<TestNabobCryptoHasher> =
    crate::_once_cell::sync::Lazy::new(TestNabobCryptoHasher::new);
// #[cfg(any(test, feature = "fuzzing"))]
impl std::default::Default for TestNabobCryptoHasher {
    fn default() -> Self {
        TEST_CRYPTO_HASHER.clone()
    }
}
// #[cfg(any(test, feature = "fuzzing"))]
impl crate::hash::CryptoHasher for TestNabobCryptoHasher {
...
// #[cfg(any(test, feature = "fuzzing"))]
impl std::io::Write for TestNabobCryptoHasher {
...
// #[cfg(any(test, feature = "fuzzing"))]
impl crate::hash::CryptoHash for TestNabobCrypto {
    type Hasher = TestNabobCryptoHasher;

    fn hash(&self) -> crate::hash::HashValue {
        use crate::hash::CryptoHasher;
        let mut state = Self::Hasher::default();
        bcs::serialize_into(&mut state, &self)
            .expect("BCS serialization of TestNabobCrypto should not fail");
        state.finish()
    }
}

/// Produces a random TestNabobCrypto signable / verifiable struct.
#[cfg(any(test, feature = "fuzzing"))]
pub fn random_serializable_struct() -> impl Strategy<Value = TestNabobCrypto> {
    (String::arbitrary()).prop_map(TestNabobCrypto).no_shrink()
}
