...
/// ### Account
///
/// - An **account** in the Nabob blockchain is a container for an arbitrary number of [Move modules](#move-module) and [Move resources](#move-resources). This essentially means that the state of each [account](../concepts/accounts.md) is comprised of both code and data.
...
/// ### Account Address
///
/// - An **account address** is the address of an Nabob account.
/// - Account address refers to a specific destination on the Nabob network. The address dictates the destination and source of a specific amount of assets exchanged by two parties on the blockchain.
/// - An Nabob address is a 64-character hex string, and sometimes it can be shortened by stripping leading 0s and prefixing `0x`. This makes a hex-encoded 32 byte Nabob account address.
///
/// See [Accounts](../concepts/accounts.md) for more information.
///
/// ### API
///
/// - An **Application Programming Interface (API)(** is a set of protocols and tools that allow users to interact with Nabob blockchain nodes and client networks via external applications. Nabob offers a REST API for this purpose. See the [Nabob API reference](https://nabob.dev/nodes/nabob-api-spec#/) documentation and [Use the Nabob API](../integration/fullnode-rest-api.md) for more details.
///
/// ### BOB
///
/// **Nabob token (BOB)** is the Nabob blockchain native token used for paying network and transaction fees.
///
/// ### Nabob
///
/// **Nabob** is a Layer 1 blockchain for everyone. It uses the Move programming language and launched its mainnet on 2022-10-17 to redefine the web3 user experience. The Nabob blockchain is dedicated to creating better user experiences through increased speed, security, scalability, reliability and usability with low transaction costs.  The word "Nabob" means "The People" in the Ohlone language. See the [Nabob White Paper](../nabob-white-paper/index.md) for more details.
///
/// ### NabobBFT
///
/// - **NabobBFT** is the Nabob protocol's BFT consensus algorithm.
/// - NabobBFT is based on Jolteon.
///
/// ### Nabob Blockchain
///
/// - The **Nabob blockchain** is a ledger of immutable transactions agreed upon by the validators on the Nabob network (the network of validators).
///
/// ### Nabob Name Service (ANS)
///
/// - The **Nabob Name Service (ANS)** is a decentralized naming address service for the Nabob blockchain. An Nabob name is a human-readable *.bob* domain name that is used in place of a public key, for example *love.bob*.
/// - This service also allows users to register subdomain names in addition to the registered domain. Find out more at: [Nabobnames.com](https://www.nabobnames.com/)
///
/// ### Nabob-core
///
/// **Nabob-core** is the open source technology on which the Nabob Payment Network runs. Nabob-core contains software for
///
/// * the Nabob blockchain itself, which generates and stores the immutable ledger of confirmed transactions and
/// * the validation process, which implements the consensus algorithm to validate transactions and add them to the Nabob blockchain immutable ledger.
///
/// ### Nabob Ecosystem
///
/// - **Nabob ecosystem** refers to various components of the Nabob blockchain network and their interactions.  The Nabob ecosystem includes the community, community-driven projects, and events. See [Contribute to the Nabob Ecosystem](../community/index.md) for all possible ways to join Nabob.
///
/// ### Nabob Explorer
///
/// - The **[Nabob Explorer](https://explorer.naboblabs.com/)** is an interface that helps users examine details of the Nabob blockchain, including account information, validators, and transactions.
/// - The Nabob Explorer help users validate their work in Nabob wallets and other tools in the blockchain. Find more details at [Use the Nabob Explorer](../guides/explore-nabob.md).
///
/// ### Nabob Framework
/// The **Nabob Framework** defines the public API for blockchain updates and the structure of on-chain data. It defines the business logic and access control for the three key pillars of Nabob functionality: payments, treasury, and on-chain governance. It is implemented as a set of modules written in the Move programming language and stored on-chain as Move bytecode.
///
/// ### Nabob Node
/// An **Nabob node** is a peer entity of the Nabob network that tracks the state of the Nabob blockchain. There are two types of Nabob nodes, [validators](#validator) and [fullnodes](#fullnode)).
///
/// ### Nabob Protocol
///
/// - **Nabob protocol** is the specification of how transactions are submitted, ordered, executed, and recorded within the Nabob network.
///
/// ### NabobAccount
///
/// - A **`NabobAccount`** is a Move resource that holds all the administrative data associated with an account, such as sequence number, balance, and authentication key.
/// - A **`NabobAccount`** is the only resource that every account is guaranteed to contain.
///
/// ### NabobAccount module
///
/// - **The NabobAccount module** is a Move module that contains the code for manipulating the administrative data held in a particular `NabobAccount.T` resource.
/// - Code for checking or incrementing sequence numbers, withdrawing or depositing currency, and extracting gas deposits is included in the NabobAccount module.
///
/// ### Nabob devnet
///
/// - See [devnet](#devnet).
...
/// - The Nabob blockchain uses NabobBFT, a consensus protocol based on [Jolteon](#Jolteon).
/// - BFT algorithms typically operate with a number of entities, collectively holding N votes (which are called "validators" in the Nabob network's application of the system).
...
/// ### CLI
///
/// - **Command line interface** refers to the Nabob CLI used for developing on the Nabob blockchain, operating nodes, and debugging issues. Find out more at [Use the Nabob CLI](../tools/nabob-cli-tool/use-nabob-cli.md).
///
/// ### Client
///
/// - **Client** is software that receives information from the blockchain and manages transactions. Clients interact with the blockchain through the Nabob nodes.
///
/// ### Code labs
///
/// - **Code labs and tutorials** depict various workflows - such as the use of the Nabob CLI in minting non-fungible tokens (NFTs) - in order for users to understand how the process works and employ related functions in their code. If users have the necessary funds in their accounts, they can follow the same code lab and tutorial steps used in devnet, testnet and mainnet networks.
///
/// ### Consensus
///
...
/// - The Nabob blockchain is formed with these agreed-upon transactions and their corresponding execution results.
/// - The consensus component is accountable for achieving security, trust, and agreement among all validators on the Nabob blockchain.
///
/// ### Consensus Protocol
///
...
/// ### dApps
///
/// - **Decentralized applications (dApps)** are programs or digital applications that run on the Nabob blockchain autonomously. Smart contracts are commonly used to achieve this function.
///
/// ### devnet
///
/// - The **Nabob devnet** is a publicly deployed instance of the Nabob network that runs using a set of validator test nodes.
/// - The devnet is a demonstration of the Nabob network that is built for experimenting with new ideas
...
/// - More specifically, the Nabob network uses the PureEdDSA scheme over the Ed25519 curve, as defined in RFC 8032.
///
/// ### Event
///
/// - An **event** is the user-facing representation of the effects of executing a transaction.
/// - A transaction may be designed to emit any number of events as a list. For example, a `Coin<NabobCoin>` transfer emits a `WithdrawEvent` for the sender account and a `DepositEvent` for the recipient account.
/// - In the Nabob protocol, events provide evidence that the successful execution of a transaction resulted in a specific effect. The `DepositEvent` (in the above example) allows the recipient to confirm that a payment was received into their account.
/// - Events are persisted on the blockchain and are used to answer queries by [clients](#client).
///
/// ### Execution
///
/// - **Execution** in the Nabob blockchain is an Nabob node component that manages the block of transactions. The execution component stores successful transactions.
///
/// ### Expiration Time
///
...
/// ### Faucet
///
/// - **Faucet** is a service that mints BOB on devnet and testnet. BOB on these networks has no real world value, it is only for development purposes.
/// - You can use the faucet in a few different ways:
///   - With the [Nabob CLI](../tools/nabob-cli-tool/use-nabob-cli.md#fund-an-account-with-the-faucet).
///   - Through a wallet, such as Petra, Martian, or Pontem. You can find a full list [here](https://github.com/nabob-labs/ecosystem-projects#wallets).
...
/// ### Fungible Asset
///
/// - A **fungible asset** is an asset, such as a currency, share, in-game resource, etc., that is interchangeable with another identical asset without any loss in its value. For example, BOB is a fungible asset because you can exchange one BOB for another.
/// - Follow the [Digital Asset Standards](../standards/index.md#digital-asset-standards) to create fungible assets on the Nabob blockchain.
/// - Next generation of the Coin standard that addresses shortcomings of `nabob_framework::coin` such as lack of guaranteed enforcement of freeze and burn and advanced functionalities such as programmable transfers, e.g., approve in ERC-20.
///
/// ### Fungible Token
///
/// - For TokenV1 (nabob_token::token), a **fungible token** is a token that is interchangeable with other identical tokens (i.e., tokens that share the same `TokenId`). This means the tokens have the same `creator address`, `collection name`, `token name`, and `property version`.
/// - For TokenV2 (nabob_token_objects::token), a **fungible token** is a fungible asset with metadata object includes a TokenV2 resource.
///
/// ### Fungible Unit
///
/// - A **fungible unit** is an individual unit of a fungible asset. These units are identical and interchangeable without any loss in value. For example, each Octa (the smallest unit of BOB) is a fungible unit.
///
/// ## G
///
/// ### Gas
///
/// - **Gas** is a way to pay for computation and storage on a blockchain network. All transactions on the Nabob network cost a certain amount of gas.
...
/// - NabobBFT, the Nabob network's consensus algorithm, is based on Jolteon.
...
/// ### Indexer
///
/// - **[Indexer](../integration/indexing.md)** is the component of Nabob that retrieves, processes, and efficiently stores raw data in the database to provide speedy access to the Nabob blockchain state.
///
/// ## L
///
/// ### Leader
...
/// - Transactions in the mempool of a validator are added from the JSON-RPC Service of the current node and from the mempool of other Nabob nodes.
...
/// - The Nabob network treats the entire blockchain as a single data structure that records the history of transactions and states over time.
...
/// ### Merkle Accumulator
///
/// - The **[Merkle Accumulator](https://www.usenix.org/legacy/event/sec09/tech/full_papers/crosby.pdf)** is an _append-only_ Merkle tree that the Nabob blockchain uses to store the ledger.
...
/// ### Move
///
/// - **Move** is a new programming language that implements all the transactions on the Nabob blockchain.
/// - It has two different kinds of code &mdash; [transaction scripts](#transaction-script) and [Move modules](#move-module).
/// - Move is a safe and secure programming language for web3 that emphasizes access control and scarcity. It is the programming language used to build the Nabob blockchain. You can read more about it in [Move on Nabob](../move/move-on-nabob.md).
///
/// ### Move Bytecode
///
...
/// ### Move Module
///
/// - A **Move module** defines the rules for updating the global state of the Nabob blockchain.
/// - In the Nabob protocol, a Move module is a **smart contract**.
...
/// ### Node
///
/// - A **node** is a peer entity of the Nabob network that tracks the state of the Nabob blockchain.
/// - An Nabob node consists of logical components. [Mempool](#mempool), [consensus](#consensus), and the [virtual machine](#virtual-machine) are examples of node components.
///
/// ### Nonce
///
...
///
/// - **Open-source community** is a term used for a group of developers who work on open-source software. If you're reading this glossary, then you are part of the Nabob project's developer community.
///
/// ## P
///
/// ### Proof
...
/// - Every operation in the Nabob blockchain can be verified cryptographically that it is indeed correct and that data has not been omitted.
...
/// ### PoS
///
/// **Proof-of-Stake (PoS)** is a security mechanism that serves in confirming the uniqueness and legitimacy of blockchain transactions. The PoS consensus mechanism is leveraged by the Nabob blockchain powered by a network of validators, which in turn update the system and process transactions.
///
/// ## R
///
/// ### Resource Account
...
///
/// See [Resource accounts](../move/move-on-nabob/resource-accounts.md) for instructions on use.
///
/// ### REST Service
///
/// - The **REST Service** component is the external interface of a Nabob node. Any incoming client request, such as submitted transactions or queries, must first go through the REST Service. A client needs to go through the REST Service component to access storage or any other component in the system. This filters requests and protects the system.
...
/// ### SDKs
///
/// - Nabob **software development kits (SDKs)** are sets of tools that enable a developer to quickly create a custom app on the Nabob platform. Find out more at [Use the Nabob SDKs](../sdks/index.md).
///
/// ### Sequence Number
///
...
/// ### State
///
/// - A **state** in the Nabob protocol is a snapshot of the distributed database.
/// - A transaction modifies the database and produces a new and updated state.
///
/// ### State Root Hash
///
/// - **State root hash** is a [Merkle hash](https://en.wikipedia.org/wiki/Merkle_tree) over all keys and values the state of the Nabob blockchain at a given version.
///
/// ## T
///
/// ### Table
///
/// - A [**table**](https://github.com/nabob-labs/nabob/blob/main/mos/framework/nabob-stdlib/doc/table.md) implements the Table type and in Nabob is used to store information as key-value data within an account at large scale.
///
/// See [`table.move`](https://github.com/nabob-labs/nabob/blob/main/mos/framework/nabob-stdlib/sources/table.move) for the associated Nabob source file.
///
/// ### Testnet
///
/// - **Testnet** describes the Nabob network that is not fully functional yet more stable than devnet; it is an alternative network to mainnet to be used for testing.
///
/// ### Tokens
///
...
/// - An executed transaction changes the state of the Nabob blockchain.
///
...
/// - The transaction script is an arbitrary program that interacts with resources published in the global storage of the Nabob blockchain by calling the procedures of a module. It encodes the logic for a transaction.
...
///
/// To see example uses of transaction scripts, follow [Move scripts](../move/move-on-nabob/move-scripts.md) and the [Your First Multisig](../tutorials/first-multisig.md) tutorial.
///
/// ## V
///
/// ### Validator
...
/// - A **validator** is an entity of the Nabob ecosystem that validates on the Nabob blockchain. It receives requests from clients and runs consensus, execution, and storage.
...
/// - Nabob validators are in charge of verifying transactions.
///
/// ### Validator Nodes
///
/// - **Validator nodes** are a unique class of fullnodes that take part in consensus, specifically a Byzantine Fault Tolerance (BFT) consensus protocol in Nabob. Validators agree upon transactions to be added to the Nabob blockchain as well as the order in which they are added.
///
/// ### Version
///
/// - A **version** is also called "height" in blockchain literature.
/// - The Nabob blockchain doesn't have an explicit notion of a block &mdash; it only uses blocks for batching and executing transactions.
...
///
/// An Nabob transaction is **well formed** if each of the following conditions are true for the transaction:
///
