...
use crate::nabob_vm::{serialized_signer, SerializedSigners};
use crate::NabobVM;
#[cfg(any(test, feature = "testing"))]
use crate::{
...
#[cfg(any(test, feature = "testing"))]
use nabob_types::{state_store::StateView, transaction::SignedTransaction};
#[cfg(any(test, feature = "testing"))]
use nabob_vm_logging::log_schema::AdapterLogSchema;
#[cfg(any(test, feature = "testing"))]
use nabob_vm_types::{
    module_and_script_storage::AsNabobCodeStorage, output::VMOutput,
...
impl NabobVM {
    ...
    pub fn test_failed_transaction_cleanup(
        ...
        let module_storage = state_view.as_nabob_code_storage(self.runtime_environment());
