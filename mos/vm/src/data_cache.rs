...
use crate::move_vm_ext::{
    resource_state_key, NabobMoveResolver, AsExecutorView, AsResourceGroupView,
...
use nabob_vm_environment::{
    gas::get_gas_feature_version, prod_configs::nabob_prod_deserializer_config,
};
use nabob_vm_types::{
    resolver::{
...
pub fn get_resource_group_member_from_metadata(
    ...
    let metadata = nabob_framework::get_metadata(metadata)?;
    metadata
        .struct_attributes
...
/// Adapter to convert a `ExecutorView` into a `NabobMoveResolver`.
///
...
impl<'e, E: ExecutorView> NabobMoveResolver for StorageAdapter<'e, E> {}
...
/// Converts `StateView` into `NabobMoveResolver`.
...
pub(crate) mod tests {
    use super::*;
    use nabob_vm_types::resource_group_adapter::GroupSizeKind;
