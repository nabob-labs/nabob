...
use crate::{counters::TIMER, move_vm_ext::NabobMoveResolver};
use nabob_framework::natives::code::PackageRegistry;
use nabob_gas_schedule::NabobGasParameters;
use nabob_infallible::RwLock;
use nabob_metrics_core::TimerHelper;
use nabob_native_interface::SafeNativeBuilder;
use nabob_types::{on_chain_config::OnChainConfig, state_store::state_key::StateKey};
...
impl WarmVmCache {
    pub(crate) fn get_warm_vm(
        env: &NabobEnvironment,
        resolver: &impl NabobMoveResolver,
    ) -> VMResult<MoveVM> {
        WARM_VM_CACHE.get(env, resolver)
    }

    fn get(&self, env: &NabobEnvironment, resolver: &impl NabobMoveResolver) -> VMResult<MoveVM> {
    ...
    fn warm_vm_up(vm: &MoveVM, resolver: &impl NabobMoveResolver) {
...
impl WarmVmId {
    fn new(env: &NabobEnvironment, resolver: &impl NabobMoveResolver) -> VMResult<Self> {
        let natives = {
            ...
            let NabobGasParameters { natives, vm } = env
                .gas_params()
                .as_ref()
                .cloned()
                .unwrap_or(NabobGasParameters::zeros());
    ...
    fn core_packages_id_bytes(resolver: &impl NabobMoveResolver) -> VMResult<Option<Bytes>> {
        let bytes = {
