...
use crate::{
    block_executor::{NabobTransactionOutput, NabobVMBlockExecutorWrapper},
    counters::*,
    ...
    move_vm_ext::{
        ...
        NabobMoveResolver, MoveVmExt, SessionExt, SessionId, UserTransactionContext,
    },
...
use anyhow::anyhow;
use nabob_block_executor::{
    code_cache_global_manager::NabobModuleCacheManager,
...
use nabob_gas_algebra::{Gas, GasQuantity, NumBytes, Octa};
use nabob_gas_meter::{NabobGasMeter, GasAlgebra};
use nabob_gas_schedule::{
    gas_feature_versions::{RELEASE_V1_10, RELEASE_V1_27},
    NabobGasParameters, VMGasParameters,
};
use nabob_logger::{enabled, prelude::*, Level};
use nabob_metrics_core::TimerHelper;
#[cfg(any(test, feature = "testing"))]
use nabob_types::state_store::StateViewId;
use nabob_types::{
...
use nabob_utils::nabob_try;
use nabob_vm_environment::environment::NabobEnvironment;
use nabob_vm_logging::{log_schema::AdapterLogSchema, speculative_error, speculative_log};
use nabob_vm_types::{
    ...
    module_and_script_storage::{
        code_storage::NabobCodeStorage, module_storage::NabobModuleStorage, AsNabobCodeStorage,
    },
...
impl NabobVM {
    ...
    pub fn new(env: NabobEnvironment, state_view: &impl StateView) -> Self {
        let _timer = TIMER.timer_with(&["NabobVM::new"]);
        ...
        let module_storage = state_view.as_nabob_code_storage(env);
    ...
    pub fn new_session<'r, S: NabobMoveResolver>(
        &self,
        resolver: &'r S,
    ...
    pub(crate) fn gas_params(
        &self,
        log_context: &AdapterLogSchema,
    ) -> Result<&NabobGasParameters, VMStatus> {
    ...
    #[inline(always)]
    pub fn environment(&self) -> NabobEnvironment {
    ...
    #[cfg(any(test, feature = "testing"))]
    pub fn gas_params_for_test(&self) -> Result<&NabobGasParameters, VMStatus> {
    ...
    pub(crate) fn failed_transaction_cleanup(
        ...
        gas_meter: &mut impl NabobGasMeter,
        txn_data: &TransactionMetadata,
        ...
        if self.gas_feature_version() >= 12 {
            ...
            if let Err(err) = gas_meter.algebra().check_consistency() {
                println!(
                    "[nabob-vm][gas-meter][failure-epilogue] {}",
                    err.message()
    ...
    fn finish_aborted_transaction(
        ...
        let (previous_session_change_set, fee_statement) =
            if is_account_init_for_sponsored_transaction {
                ...
                let fee_statement =
                    NabobVM::fee_statement_from_gas_meter(txn_data, gas_meter, ZERO_STORAGE_REFUND);
    ...
    fn success_transaction_cleanup(
        ...
        if self.gas_feature_version() >= 12 {
            ...
            if let Err(err) = gas_meter.algebra().check_consistency() {
                println!(
                    "[nabob-vm][gas-meter][success-epilogue] {}",
                    err.message()
                        .unwrap_or("No message found -- this should not happen.")
                );
        ...
        let fee_statement = NabobVM::fee_statement_from_gas_meter(
            txn_data,
            gas_meter,
    ...
    fn validate_and_execute_script<'a>(
        ...
        serialized_signers: &SerializedSigners,
        code_storage: &impl NabobCodeStorage,
        // Note: cannot use NabobGasMeter because it is not implemented for
        //       UnmeteredGasMeter.
    ...
    fn execute_script_or_entry_function<'a, 'r, 'l>(
        ...
        fail_point!("nabob_vm::execute_script_or_entry_function", |_| {
            Err(VMStatus::Error {
    ...
    fn validate_publish_request(
        ...
        for m in modules {
            ...
            nabob_framework::verify_module_metadata(m, self.features(), self.timed_features())
        ...
        if self.chain_id().is_mainnet() {
            for module in modules {
                if let Some(metadata) =
                    nabob_framework::get_compilation_metadata_from_compiled_module(module)
                {
    ...
    pub fn reject_unstable_bytecode_for_script(&self, module: &CompiledScript) -> VMResult<()> {
        if self.chain_id().is_mainnet() {
            if let Some(metadata) =
                nabob_framework::get_compilation_metadata_from_compiled_script(module)
            {
                if metadata.unstable {
                    return Err(PartialVMError::new(StatusCode::UNSTABLE_BYTECODE_REJECTED)
    ...
    fn validate_signed_transaction(
        ...
        let keyless_authenticators = nabob_types::keyless::get_authenticators(transaction)
    ...
    fn execute_user_transaction_impl(
        ...
        let _timer = VM_TIMER.timer_with_label("NabobVM::execute_user_transaction_impl");
        ...
        let account_init_for_sponsored_transaction_timer =
            VM_TIMER.timer_with_label("NabobVM::account_init_for_sponsored_transaction");
        ...
        let payload_timer =
            VM_TIMER.timer_with_label("NabobVM::execute_user_transaction_impl [payload]");
    ...
    pub fn execute_user_transaction_with_custom_gas_meter<'a, C, G, F>(
        &self,
        resolver: &'a impl NabobMoveResolver,
        code_storage: &'a C,
        txn: &SignedTransaction,
        log_context: &AdapterLogSchema,
        make_gas_meter: F,
    ...
    where
        C: NabobCodeStorage + BlockSynchronizationKillSwitch,
        G: NabobGasMeter,
    ...
    pub fn execute_user_transaction_with_modified_gas_meter<'a, G, F>(
        &self,
        resolver: &'a impl NabobMoveResolver,
        code_storage: &'a (impl NabobCodeStorage + BlockSynchronizationKillSwitch),
        txn: &SignedTransaction,
        log_context: &AdapterLogSchema,
        modify_gas_meter: F,
    ) -> Result<(VMStatus, VMOutput, G), VMStatus>
    ...
    pub fn execute_user_transaction(
        &self,
        resolver: &impl NabobMoveResolver,
        code_storage: &(impl NabobCodeStorage + BlockSynchronizationKillSwitch),
        txn: &SignedTransaction,
        log_context: &AdapterLogSchema,
    ) -> (VMStatus, VMOutput) {
        ...
        match write_set_payload {
            WriteSetPayload::Direct(change_set) => {
                ...
                if !Self::should_restart_execution(change_set.events()) {
                    ...
                    error!(
                        "[nabob_vm] direct write set finished without requiring should_restart_execution");
                }
    ...
    fn validate_waypoint_change_set(
        ...
        if has_new_block_event && has_new_epoch_event {
            ...
            error!(
                *log_context,
                "[nabob_vm] waypoint txn needs to emit new epoch and block"
            );
    ...
    fn extract_module_metadata(
        ...
        if self.features().is_enabled(FeatureFlag::VM_BINARY_FORMAT_V6) {
            nabob_framework::get_vm_metadata(&self.move_vm, module_storage, module_id)
        } else {
            nabob_framework::get_vm_metadata_v0(&self.move_vm, module_storage, module_id)
        }
    }
    ...
    pub fn execute_view_function(
        ...
        let env = NabobEnvironment::new(state_view);
        let vm = NabobVM::new(env.clone(), state_view);
    ...
    fn gas_used(max_gas_amount: Gas, gas_meter: &impl NabobGasMeter) -> u64 {
        max_gas_amount
            .checked_sub(gas_meter.balance())
    ...
    pub fn execute_single_transaction(
        ...
        Ok(match txn.expect_valid() {
            Transaction::BlockMetadata(block_metadata) => {
                fail_point!("nabob_vm::execution::block_metadata");
            ...
            Transaction::BlockMetadataExt(block_metadata_ext) => {
                fail_point!("nabob_vm::execution::block_metadata_ext");
            ...
            Transaction::UserTransaction(txn) => {
                fail_point!("nabob_vm::execution::user_transaction");
                    ...
                    match vm_status.status_code() {
                        ...
                        | StatusCode::TYPE_RESOLUTION_FAILURE => {
                            speculative_error!(
                                log_context,
                                format!(
                                    "[nabob_vm] Transaction breaking invariant violation. txn: {:?}, status: {:?}",
                        ...
                        if vm_status.sub_status()
                            == Some(move_core_types::vm_status::sub_status::unknown_invariant_violation::EPARANOID_FAILURE) =>
                            {
                                error!(
                                *log_context,
                                "[nabob_vm] Transaction breaking paranoid mode. txn: {:?}, status: {:?}",
                                bcs::to_bytes::<SignedTransaction>(txn),
                                vm_status,
                            );
                            },
...
pub struct NabobVMBlockExecutor {
    ...
    module_cache_manager: NabobModuleCacheManager,
}

impl NabobVMBlockExecutor {
    ...
    pub fn execute_block_with_config(
        ...
        fail_point!("nabob_vm_block_executor::execute_block_with_config", |_| {
            Err(VMStatus::error(
                StatusCode::UNKNOWN_INVARIANT_VIOLATION_ERROR,
                None,
            ))
        ...
        let result = NabobVMBlockExecutorWrapper::execute_block::<
            _,
            NoOpTransactionCommitHook<NabobTransactionOutput, VMStatus>,
...
impl VMBlockExecutor for NabobVMBlockExecutor {
    fn new() -> Self {
        Self {
            module_cache_manager: NabobModuleCacheManager::new(),
        }
    }
    ...
    fn execute_block(
        ...
        let config = BlockExecutorConfig {
            local: BlockExecutorLocalConfig {
                concurrency_level: NabobVM::get_concurrency_level(),
                allow_fallback: true,
                discard_failed_blocks: NabobVM::get_discard_failed_blocks(),
...
impl VMValidator for NabobVM {
    ...
    fn validate_transaction(
        ...
        {
            if let nabob_types::transaction::authenticator::TransactionAuthenticator::SingleSender{ .. } = transaction.authenticator_ref() {
...
// Ensure encapsulation of NabobVM APIs by using a wrapper.
pub struct NabobSimulationVM(NabobVM);

impl NabobSimulationVM {
    pub fn new(env: NabobEnvironment, state_view: &impl StateView) -> Self {
        let mut vm = NabobVM::new(env, state_view);
    ...
    pub fn create_vm_and_simulate_signed_transaction(
        ...
        let resolver = state_view.as_move_resolver();
        let code_storage = state_view.as_nabob_code_storage(env);
...
#[cfg(test)]
mod tests {
    use crate::{move_vm_ext::MoveVmExt, NabobVM};
    use nabob_types::{
        account_address::AccountAddress,
    ...
    fn should_restart_execution_on_new_epoch() {
        ...
        assert!(NabobVM::should_restart_execution(&[(
            new_epoch_event,
            None
        )]));
        assert!(NabobVM::should_restart_execution(&[(
