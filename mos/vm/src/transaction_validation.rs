...
pub static NABOB_TRANSACTION_VALIDATION: Lazy<TransactionValidation> =
    ...
    if features.is_account_abstraction_enabled() {
        ...
        let (prologue_function_name, serialized_args) =
            if let (Some(_fee_payer), Some(fee_payer_auth_key)) = (
                ...
                (
                    &NABOB_TRANSACTION_VALIDATION.unified_prologue_fee_payer_name,
                    serialized_args,
                )
            } else {
                let serialized_args = vec![
                ...
                (
                    &NABOB_TRANSACTION_VALIDATION.unified_prologue_name,
                    serialized_args,
                )
            };
        session
            .execute_function_bypass_visibility(
                &NABOB_TRANSACTION_VALIDATION.module_id(),
        ...
        let (prologue_function_name, args) = if let (Some(fee_payer), Some(fee_payer_auth_key)) = (
            ...
            if features.is_transaction_simulation_enhancement_enabled() {
                ...
                (
                    &NABOB_TRANSACTION_VALIDATION.fee_payer_prologue_extended_name,
                    args,
                )
            } else {
                let args = vec![
                ...
                (&NABOB_TRANSACTION_VALIDATION.fee_payer_prologue_name, args)
            }
            ...
            if features.is_transaction_simulation_enhancement_enabled() {
                ...
                (
                    &NABOB_TRANSACTION_VALIDATION.multi_agent_prologue_extended_name,
                    args,
                )
            } else {
                let args = vec![
                ...
                (
                    &NABOB_TRANSACTION_VALIDATION.multi_agent_prologue_name,
                    args,
                )
            }
        } else {
            ...
            if features.is_transaction_simulation_enhancement_enabled() {
                ...
                (
                    &NABOB_TRANSACTION_VALIDATION.script_prologue_extended_name,
                    args,
                )
            } else {
                let args = vec![
                ...
                (&NABOB_TRANSACTION_VALIDATION.script_prologue_name, args)
            }
        };
...
fn run_epilogue(
    ...
    if features.is_account_abstraction_enabled() {
        ...
        session.execute_function_bypass_visibility(
            &NABOB_TRANSACTION_VALIDATION.module_id(),
            &NABOB_TRANSACTION_VALIDATION.unified_epilogue_name,
            vec![],
            serialize_args,
            &mut UnmeteredGasMeter,
            traversal_context,
        ...
        if let Some(fee_payer) = txn_data.fee_payer() {
            let (func_name, args) = {
                if features.is_transaction_simulation_enhancement_enabled() {
                    ...
                    (
                        &NABOB_TRANSACTION_VALIDATION.user_epilogue_gas_payer_extended_name,
                        args,
                    )
                } else {
                    let args = vec![
                    ...
                    (
                        &NABOB_TRANSACTION_VALIDATION.user_epilogue_gas_payer_name,
                        args,
                    )
                }
            };
            ...
            let (func_name, args) = {
                if features.is_transaction_simulation_enhancement_enabled() {
                    ...
                    (
                        &NABOB_TRANSACTION_VALIDATION.user_epilogue_extended_name,
                        args,
                    )
                } else {
                    let args = vec![
                    ...
                    (&NABOB_TRANSACTION_VALIDATION.user_epilogue_name, args)
                }
            };
            session.execute_function_bypass_visibility(
                &NABOB_TRANSACTION_VALIDATION.module_id(),
...
pub(crate) fn run_failure_epilogue(
    ...
    .or_else(|e| {
        expect_only_successful_execution(
            e,
            NABOB_TRANSACTION_VALIDATION.user_epilogue_name.as_str(),
            log_context,
        )
    })
}
