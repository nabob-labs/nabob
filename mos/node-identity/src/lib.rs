...
use nabob_types::{chain_id::ChainId, PeerId};
use once_cell::sync::OnceCell;
use std::sync::Arc;

/// The global [NabobNodeIdentity]
static NABOB_NODE_IDENTITY: OnceCell<Arc<NabobNodeIdentity>> = OnceCell::new();

/// Structure that holds information related to a node's identity
pub struct NabobNodeIdentity {
...
/// Initializes the [NabobNodeIdentity] using the provided [PeerId] and
/// sets it globally exactly once.
pub fn init(peer_id: Option<PeerId>) -> Result<()> {
    let identity = NabobNodeIdentity {
    ...
    NABOB_NODE_IDENTITY
        ...
        .map_err(|_| format_err!("NABOB_NODE_IDENTITY was already set"))
}

/// Sets the [ChainId] in the global [NabobNodeIdentity], returning an error
/// if [init] was not called already.
pub fn set_chain_id(chain_id: ChainId) -> Result<()> {
    match NABOB_NODE_IDENTITY.get() {
        Some(identity) => identity
            .chain_id
            .set(chain_id)
            .map_err(|_| format_err!("chain_id was already set.")),
        None => Err(format_err!("NABOB_NODE_IDENTITY has not been set yet")),
    }
}

/// Returns the [PeerId] from the global `NABOB_NODE_IDENTITY`
...
/// Returns the [PeerId] as [str] from the global `NABOB_NODE_IDENTITY`
...
/// Returns the [ChainId] from the global `NABOB_NODE_IDENTITY`
