...
use nabob_types::transaction::{Script, SignedTransaction};
...
";
    let modules = nabob_cached_packages::head_release_bundle().compiled_modules();
...
pub fn create_account_txn(
    ...
    sender
        .transaction()
        .payload(nabob_stdlib::nabob_account_create_account(
            *new_account.address(),
        ))
        .sequence_number(seq_num)
        .sign()
...
pub fn peer_to_peer_txn(
    ...
    sender
        .transaction()
        .payload(nabob_stdlib::nabob_coin_transfer(
            *receiver.address(),
            transfer_amount,
        ))
        .sequence_number(seq_num)
