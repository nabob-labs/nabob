...
//! Test infrastructure for modeling Nabob accounts.

use crate::gas_costs;
use nabob_crypto::ed25519::*;
use nabob_keygen::KeyGen;
use nabob_types::{
    access_path::AccessPath,
    account_address::AccountAddress,
    account_config::{
        self, primary_bob_store, AccountResource, CoinStoreResource,
...
use nabob_vm_genesis::GENESIS_KEYPAIR;
...
/// Details about a Nabob account.
///
/// Tests will typically create a set of `Account` instances to run transactions on. This type
/// encodes the logic to operate on and verify operations on any Nabob account.
///
...
impl Account {
    ...
    /// not automatically get added to the Nabob store. To add an account to the store, use
    ...
    pub fn with_keypair(privkey: Ed25519PrivateKey, pubkey: Ed25519PublicKey) -> Self {
        let addr = nabob_types::account_address::from_public_key(&pubkey);
        Account {
            addr,
            privkey,
    ...
    /// Creates a new account representing the nabob root account in memory.
    ///
    /// The address will be [`nabob_test_root_address`][account_config::nabob_test_root_address], and
    /// the account will use [`GENESIS_KEYPAIR`][static@GENESIS_KEYPAIR] as its keypair.
    pub fn new_nabob_root() -> Self {
        Self::new_genesis_account(account_config::nabob_test_root_address())
    }
    ...
    pub fn make_coin_store_access_path(&self) -> AccessPath {
        AccessPath::resource_access_path(
            self.addr,
            CoinStoreResource::<NabobCoinType>::struct_tag(),
        )
        .expect("access path in  test")
    }
...
impl CoinStore {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        let coin_store = CoinStoreResource::<NabobCoinType>::new(
            self.coin,
            self.frozen,
...
impl FungibleStore {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        let primary_store_object_address = primary_bob_store(self.owner);
...
impl AccountData {
    ...
    pub fn to_writeset(&self) -> WriteSet {
        ...
        if let Some(coin_store) = &self.coin_store {
            write_set.push((
                StateKey::resource_typed::<CoinStoreResource<NabobCoinType>>(self.address())
                    .unwrap(),
        ...
        if let Some(fungible_store) = &self.fungible_store {
            let primary_store_object_address = primary_bob_store(*self.address());
