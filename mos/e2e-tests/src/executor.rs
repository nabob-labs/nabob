...
use nabob_abstract_gas_usage::CalibrationAlgebra;
use nabob_bitvec::BitVec;
use nabob_block_executor::{
    code_cache_global_manager::NabobModuleCacheManager, txn_commit_hook::NoOpTransactionCommitHook,
    txn_provider::default::DefaultTxnProvider,
};
use nabob_crypto::HashValue;
use nabob_framework::ReleaseBundle;
use nabob_gas_algebra::DynamicExpression;
use nabob_gas_meter::{NabobGasMeter, GasAlgebra, StandardGasAlgebra, StandardGasMeter};
use nabob_gas_profiling::{GasProfiler, TransactionGasLog};
...
use nabob_types::{
    ...
    move_utils::MemberId,
    on_chain_config::{NabobVersion, FeatureFlag, Features, OnChainConfig, ValidatorSet},
...
use nabob_vm_genesis::{generate_genesis_change_set_for_testing_with_count, GenesisOptions};
...
///
/// This struct is a mock in-memory implementation of the Nabob executor.
...
impl FakeExecutor {
    ...
    fn set_tracing(&mut self, test_name: &str, file_name: String) {
        ...
        if let Some(env_trace_dir) = env::var_os(ENV_TRACE_DIR) {
            let nabob_version = NabobVersion::fetch_config(&self.data_store.as_move_resolver())
                .map_or(0, |v| v.major);
            ...
            let mut name_file = OpenOptions::new()
                ...
                .unwrap();
            write!(name_file, "{}::{}", test_name, nabob_version).unwrap();
            for sub_dir in &[
                TRACE_DIR_META,
                TRACE_DIR_DATA,
                TRACE_DIR_INPUT,
    ...
    pub fn stdlib_only_genesis() -> Self {
        ...
        for (bytes, module) in
            nabob_cached_packages::head_release_bundle().code_and_compiled_modules()
        {
    ...
    pub fn custom_genesis(framework: &ReleaseBundle, validator_accounts: Option<usize>) -> Self {
        let genesis = nabob_vm_genesis::generate_test_genesis(framework, validator_accounts);
    ...
    pub fn new_account_data_at(&mut self, addr: AccountAddress) -> AccountData {
        ...
        // Mint the account 10M Nabob coins (with 8 decimals).
    ...
    pub fn store_and_fund_account(
        ...
        let use_fa_balance = features.is_enabled(FeatureFlag::NEW_ACCOUNTS_DEFAULT_TO_FA_BOB_STORE);
    ...
    pub fn read_bob_coin_store_resource(
        &self,
        account: &Account,
    ) -> Option<CoinStoreResource<NabobCoinType>> {
        self.read_bob_coin_store_resource_at_address(account.address())
    }
    ...
    pub fn read_bob_coin_info_resource(&self) -> Option<CoinInfoResource<NabobCoinType>> {
        self.read_resource(&NabobCoinType::coin_info_address())
    }

    /// Reads the CoinStore resource value for an account under the given address from this executor's
    /// data store.
    pub fn read_bob_coin_store_resource_at_address(
        &self,
        addr: &AccountAddress,
    ) -> Option<CoinStoreResource<NabobCoinType>> {
        self.read_resource(addr)
    ...
    fn execute_transaction_block_impl_with_state_view(
        ...
        NabobVMBlockExecutorWrapper::execute_block_on_thread_pool::<
            _,
    ...
    pub fn execute_transaction_with_gas_profiler(
        ...
        let env = NabobEnvironment::new(&self.data_store);
        let vm = NabobVM::new(&env, self.get_state_view());

        let resolver = self.data_store.as_move_resolver();
        let code_storage = self.get_state_view().as_nabob_code_storage(&env);
    ...
    pub fn validate_transaction(&self, txn: SignedTransaction) -> VMValidatorResult {
        ...
        vm.validate_transaction(
            txn,
            &self.data_store,
            &self.data_store.as_nabob_code_storage(&env),
        )
    }
    ...
    pub fn exec_func_record_running_time(
        ...
        // cached.
        let module_storage = self.data_store.as_nabob_code_storage(&env);
    ...
    pub fn exec_abstract_usage(
        ...
        let (write_set, _events) = {
            let env = NabobEnvironment::new_with_gas_hook(
                &self.data_store,
...
/// responsibility of the adapter, i.e., [NabobVM]).
