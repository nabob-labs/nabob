...
use nabob_gas_algebra::{FeePerGasUnit, Gas, GasExpression};
use nabob_gas_schedule::{NabobGasParameters, InitialGasSchedule, LATEST_GAS_FEATURE_VERSION};
...
impl AUTransactionGen for InsufficientBalanceGen {
    fn apply(
        ...
        let gas_params = NabobGasParameters::initial();
...
pub struct InvalidAuthkeyGen {
    sender: Index,
    #[proptest(
        strategy = "nabob_crypto::test_utils::uniform_keypair_strategy_with_perturbation(1)"
    )]
