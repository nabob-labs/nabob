...
use nabob_types::transaction::{Module, Script};
...
pub fn compile_module(code: &str) -> (CompiledModule, Module) {
    let framework_modules = nabob_cached_packages::head_release_bundle().compiled_modules();
...
pub fn compile_script(code: &str, mut extra_deps: Vec<CompiledModule>) -> Script {
    let mut framework_modules = nabob_cached_packages::head_release_bundle().compiled_modules();
