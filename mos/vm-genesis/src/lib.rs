...
use nabob_framework::{ReleaseBundle, ReleasePackage};
use nabob_gas_schedule::{
    NabobGasParameters, InitialGasSchedule, ToOnChainGasSchedule, LATEST_GAS_FEATURE_VERSION,
};
use nabob_types::{
    account_config::{self, nabob_test_root_address, events::NewEpochEvent, CORE_CODE_ADDRESS},
    chain_id::ChainId,
...
use nabob_vm_types::{
    ...
    module_and_script_storage::{module_storage::NabobModuleStorage, AsNabobCodeStorage},
...
const GOVERNANCE_MODULE_NAME: &str = "nabob_governance";
...
const NABOB_COINS_BASE_WITH_DECIMALS: u64 = u64::pow(10, 8);
...
pub fn default_gas_schedule() -> GasScheduleV2 {
    GasScheduleV2 {
        ...
        entries: NabobGasParameters::initial().to_on_chain_gas_schedule(LATEST_GAS_FEATURE_VERSION),
    }
}

pub fn encode_nabob_mainnet_genesis_transaction(
    ...
    let module_storage = state_view.as_nabob_code_storage(&genesis_runtime_environment);
    ...
    initialize_nabob_coin(&mut session, &module_storage);
...
pub fn encode_genesis_transaction(
    nabob_root_key: Ed25519PublicKey,
...
pub fn encode_genesis_change_set(
    ...
    if genesis_config.is_test {
        initialize_core_resources_and_nabob_coin(&mut session, &module_storage, core_resources_key);
    } else {
        initialize_nabob_coin(&mut session, &module_storage);
    }
...
fn initialize(
    ...
    exec_function(
        ...
        serialize_values(&vec![
            ...
            MoveValue::U64(NABOB_MAX_KNOWN_VERSION.major),
...
fn initialize_nabob_coin(session: &mut SessionExt, module_storage: &impl NabobModuleStorage) {
    exec_function(
        session,
        module_storage,
        GENESIS_MODULE_NAME,
        "initialize_nabob_coin",
        vec![],
        serialize_values(&vec![MoveValue::Signer(CORE_CODE_ADDRESS)]),
    );
}

fn initialize_config_buffer(session: &mut SessionExt, module_storage: &impl NabobModuleStorage) {
    exec_function(
        session,
        module_storage,
...
fn initialize_dkg(session: &mut SessionExt, module_storage: &impl NabobModuleStorage) {
    exec_function(
        session,
        module_storage,
        DKG_MODULE_NAME,
...
fn initialize_jwks_resources(session: &mut SessionExt, module_storage: &impl NabobModuleStorage) {
    exec_function(
        session,
        module_storage,
        JWKS_MODULE_NAME,
...
fn set_genesis_end(session: &mut SessionExt, module_storage: &impl NabobModuleStorage) {
    exec_function(
        session,
        module_storage,
        GENESIS_MODULE_NAME,
...
fn initialize_core_resources_and_nabob_coin(
    session: &mut SessionExt,
    module_storage: &impl NabobModuleStorage,
    core_resources_key: &Ed25519PublicKey,
) {
    ...
    exec_function(
        ...
        module_storage,
        GENESIS_MODULE_NAME,
        "initialize_core_resources_and_nabob_coin",
        vec![],
        serialize_values(&vec![
            MoveValue::Signer(CORE_CODE_ADDRESS),
...
fn allow_core_resources_to_set_version(
    ...
    exec_function(
        ...
        "initialize_for_test",
        vec![],
        serialize_values(&vec![MoveValue::Signer(nabob_test_root_address())]),
    );
}
...
fn code_to_writes_for_publishing(
    ...
    let module_storage = genesis_state_view.as_nabob_code_storage(genesis_runtime_environment);
...
pub fn generate_genesis_change_set_for_testing_with_count(
    ...
    let framework = match genesis_options {
        GenesisOptions::Head => nabob_cached_packages::head_release_bundle(),
        GenesisOptions::Testnet => nabob_framework::testnet_release_bundle(),
        GenesisOptions::Mainnet => {
            // We don't yet have mainnet, so returning testnet here
            nabob_framework::testnet_release_bundle()
        },
    };
...
pub fn generate_genesis_change_set_for_mainnet(genesis_options: GenesisOptions) -> ChangeSet {
    let framework = match genesis_options {
        ...
        GenesisOptions::Testnet => nabob_framework::testnet_release_bundle(),
        // We don't yet have mainnet, so returning testnet here
        GenesisOptions::Mainnet => nabob_framework::testnet_release_bundle(),
    };
...
pub fn test_genesis_change_set_and_validators(
    ...
    generate_test_genesis(nabob_cached_packages::head_release_bundle(), count)
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Validator {
    /// The Nabob account address of the validator or the admin in the case of a commissioned or
    /// vesting managed validator.
    pub owner_address: AccountAddress,
    /// The Nabob account address of the validator's operator (same as `address` if the validator is
    /// its own operator).
...
pub fn generate_test_genesis(
    ...
    let genesis = encode_genesis_change_set(
        ...
        &GenesisConfiguration {
            ...
            // 1M NABOB coins (with 8 decimals).
...
fn mainnet_genesis_config() -> GenesisConfiguration {
    ...
    GenesisConfiguration {
        ...
        is_test: false,
        min_stake: 1_000_000 * NABOB_COINS_BASE_WITH_DECIMALS, // 1M BOB
        // 400M BOB
        min_voting_threshold: (400_000_000 * NABOB_COINS_BASE_WITH_DECIMALS as u128),
        max_stake: 50_000_000 * NABOB_COINS_BASE_WITH_DECIMALS, // 50M BOB.
        recurring_lockup_duration_secs: 30 * 24 * 3600,         // 1 month
        required_proposer_stake: 1_000_000 * NABOB_COINS_BASE_WITH_DECIMALS, // 1M BOB
        rewards_apy_percentage: 10,
...
pub fn test_genesis_module_publishing() {
    ...
    let (change_set, module_write_set) = publish_framework(
        ...
        &genesis_runtime_environment,
        HashValue::zero(),
        nabob_cached_packages::head_release_bundle(),
    );
...
pub fn test_mainnet_end_to_end() {
    ...
    let balance = 10_000_000 * NABOB_COINS_BASE_WITH_DECIMALS;
    let non_validator_balance = 10 * NABOB_COINS_BASE_WITH_DECIMALS;
    ...
    let transaction = encode_nabob_mainnet_genesis_transaction(
        &accounts,
        &employees,
        &validators,
        nabob_cached_packages::head_release_bundle(),
