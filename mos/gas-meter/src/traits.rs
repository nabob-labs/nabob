...
use nabob_gas_algebra::{Fee, FeePerGasUnit, Gas, GasExpression, GasScalingFactor, Octa};
use nabob_gas_schedule::VMGasParameters;
use nabob_types::{
...
/// Trait representing a gas meter used inside the Nabob VM.
///
/// It extends Move VM's `GasMeter` trait with a few Nabob-specific callbacks, along with
/// some primitives that would allow gas meters to be composable.
pub trait NabobGasMeter: MoveGasMeter {
    ...
    ///
    /// Since the fee is measured in BOB/Octa, it needs to be converted into gas units
