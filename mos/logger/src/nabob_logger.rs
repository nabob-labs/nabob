...
impl LogEntry {
    fn new(event: &Event, thread_name: Option<&str>, enable_backtrace: bool) -> Self {
        ...
        #[cfg(node_identity)]
        {
            peer_id = nabob_node_identity::peer_id_as_str();
            chain_id = nabob_node_identity::chain_id().map(|chain_id| chain_id.id());
        }
...
/// A builder for a `NabobData`, configures what, where, and how to write logs.
pub struct NabobDataBuilder {
    channel_size: usize,
...
impl NabobDataBuilder {
    ...
    fn build_logger(&mut self) -> Arc<NabobData> {
    ...
    pub fn build(&mut self) -> Arc<NabobData> {
...
impl NabobData {
    pub fn builder() -> NabobDataBuilder {
        NabobDataBuilder::new()
    }

    #[allow(clippy::new_ret_no_self)]
    pub fn new() -> NabobDataBuilder {
        Self::builder()
    }

    pub fn init_for_testing() {
        // Create the Nabob Data Builder
...
/// or to a `NabobData` for external logging.
struct LoggerService {
    receiver: sync::mpsc::Receiver<LoggerServiceEvent>,
    printer: Option<Box<dyn Writer>>,
    facade: Arc<NabobData>,
...
pub struct LoggerFilterUpdater {
    logger: Arc<NabobData>,
    logger_builder: NabobDataBuilder,
}

impl LoggerFilterUpdater {
    pub fn new(logger: Arc<NabobData>, logger_builder: NabobDataBuilder) -> Self {
        Self {
            logger,
            logger_builder,
        }
...
#[cfg(test)]
mod tests {
    use super::{text_format, NabobData, LogEntry};
    use crate::{
        nabob_logger::{json_format, TruncatedLogString, RUST_LOG_TELEMETRY},
        debug, error, info,
        logger::Logger,
        telemetry_log_writer::TelemetryLog,
        trace, warn, NabobDataBuilder, Event, Key, KeyValue, Level, LoggerFilterUpdater, Metadata,
    ...
    fn new_async_logger() -> (NabobDataBuilder, Arc<NabobData>) {
        let mut logger_builder = NabobDataBuilder::new();
    ...
    fn new_text_logger() -> (
        ...
        let logger = NabobDataBuilder::new()
            .level(Level::Debug)
