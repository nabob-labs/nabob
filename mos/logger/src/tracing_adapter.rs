...
/// A layer that translates tracing events into nabob-logger events.
pub struct TracingToNabobDataLayer;
...
impl<S> Layer<S> for TracingToNabobDataLayer
where
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    fn on_event(&self, event: &Event, ctx: Context<S>) {
        let metadata = match translate_metadata(event.metadata()) {
            ...
            None => {
                dl::warn!(
                    "[tracing-to-nabob-logger] failed to translate event due to unknown level {:?}",
