...
use crate::{
    ...
    prod_configs::{
        nabob_default_ty_builder, nabob_prod_ty_builder, nabob_prod_vm_config,
        get_timed_feature_override,
    },
};
use nabob_gas_algebra::DynamicExpression;
use nabob_gas_schedule::{NabobGasParameters, MiscGasParameters, NativeGasParameters};
...
use nabob_vm_types::storage::StorageGasParameters;
...
pub struct NabobEnvironment(Arc<Environment>);

impl NabobEnvironment {
    ...
    /// on-chain.
    #[inline]
    pub fn gas_params(&self) -> &Result<NabobGasParameters, String> {
        &self.0.gas_params
    }
...
impl Clone for NabobEnvironment {
    fn clone(&self) -> Self {
        Self(self.0.clone())
    }
}

impl PartialEq for NabobEnvironment {
    fn eq(&self, other: &Self) -> bool {
        self.0.hash == other.0.hash
    }
}

impl Eq for NabobEnvironment {}

impl WithRuntimeEnvironment for NabobEnvironment {
...
struct Environment {
    ...
    /// on-chain.
    gas_params: Result<NabobGasParameters, String>,
...
impl Environment {
    fn new(
        ...
        let (native_gas_params, misc_gas_params, ty_builder) = match &gas_params {
            Ok(gas_params) => {
                let ty_builder = nabob_prod_ty_builder(gas_feature_version, gas_params);
                (
            ...
            Err(_) => {
                let ty_builder = nabob_default_ty_builder();
                (
        ...
        let natives = nabob_natives_with_builder(&mut builder, inject_create_signer_for_gov_sim);
        let vm_config = nabob_prod_vm_config(&features, &timed_features, ty_builder);
...
pub mod tests {
    ...
    fn test_environment_eq() {
        let state_view = MockStateView::empty();
        let environment_1 = NabobEnvironment::new(&state_view);
        let environment_2 = NabobEnvironment::new(&state_view);
    ...
    fn test_environment_ne() {
        ...
        for i in 0..state_views.len() {
            for j in 0..state_views.len() {
                if i != j {
                    let environment_1 = NabobEnvironment::new(&state_views[i]);
                    let environment_2 = NabobEnvironment::new(&state_views[j]);
                    assert!(environment_1 != environment_2);
                }
            }
        }
    ...
    fn test_environment_with_injected_create_signer_for_gov_sim() {
        ...
        let not_injected_envs = [
            NabobEnvironment::new(&state_view),
            NabobEnvironment::new_with_gas_hook(&state_view, Arc::new(|_| {})),
        ...
        // Injected.
        let env = NabobEnvironment::new_with_injected_create_signer_for_gov_sim(&state_view);
        #[allow(deprecated)]
