...
use nabob_gas_schedule::{gas_feature_versions::RELEASE_V1_15, NabobGasParameters};
use nabob_types::{
    on_chain_config::{
...
/// Returns [TypeBuilder] used by the Nabob blockchain in production.
pub fn nabob_prod_ty_builder(
    gas_feature_version: u64,
    gas_params: &NabobGasParameters,
) -> TypeBuilder {
...
pub fn nabob_default_ty_builder() -> TypeBuilder {
    TypeBuilder::with_limits(128, 20)
}

/// Returns [DeserializerConfig] used by the Nabob blockchain in production.
pub fn nabob_prod_deserializer_config(features: &Features) -> DeserializerConfig {
...
/// Returns [VerifierConfig] used by the Nabob blockchain in production.
pub fn nabob_prod_verifier_config(features: &Features) -> VerifierConfig {
...
/// Returns [VMConfig] used by the Nabob blockchain in production, based on the set of feature
/// flags.
pub fn nabob_prod_vm_config(
    features: &Features,
    ...
    let deserializer_config = nabob_prod_deserializer_config(features);
    let verifier_config = nabob_prod_verifier_config(features);
