...
// This is a hack to recognize NABOB StdLib, Framework, and Token libs to avoid warnings on some old errors.
// This will be removed after library attributes are cleaned up.
// (See https://github.com/nabob-labs/nabob/issues/9410)
fn module_is_in_nabob_libs(module_address: Option<Spanned<Address>>) -> bool {
    const NABOB_STDLIB_NAME: &str = "nabob_std";
    static NABOB_STDLIB_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
        Lazy::new(|| NumericalAddress::parse_str("0x1").unwrap());
    const NABOB_FRAMEWORK_NAME: &str = "nabob_framework";
    static NABOB_FRAMEWORK_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
        Lazy::new(|| NumericalAddress::parse_str("0x1").unwrap());
    const NABOB_TOKEN_NAME: &str = "nabob_token";
    static NABOB_TOKEN_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
        Lazy::new(|| NumericalAddress::parse_str("0x3").unwrap());
    const NABOB_TOKEN_OBJECTS_NAME: &str = "nabob_token_objects";
    static NABOB_TOKEN_OBJECTS_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
    ...
    match &module_address {
        Some(spanned_address) => {
            ...
            match address {
                ...
                {
                    Some(spanned_symbol) => {
                        ((&spanned_symbol.value as &str) == NABOB_STDLIB_NAME
                            && (spanned_numerical_address.value == *NABOB_STDLIB_NUMERICAL_ADDRESS))
                            || ((&spanned_symbol.value as &str) == NABOB_FRAMEWORK_NAME
                                && (spanned_numerical_address.value
                                    == *NABOB_FRAMEWORK_NUMERICAL_ADDRESS))
                            || ((&spanned_symbol.value as &str) == NABOB_TOKEN_NAME
                                && (spanned_numerical_address.value
                                    == *NABOB_TOKEN_NUMERICAL_ADDRESS))
                            || ((&spanned_symbol.value as &str) == NABOB_TOKEN_OBJECTS_NAME
                                && (spanned_numerical_address.value
                                    == *NABOB_TOKEN_OBJECTS_NUMERICAL_ADDRESS))
                    },
                    None => false,
                },
...
fn module_(
    ...
    if context.env.flags().warn_of_deprecation_use_in_nabob_libs() {
        context.in_nabob_libs = false;
    } else {
        context.in_nabob_libs = module_is_in_nabob_libs(module_address);
    }
...
fn check_for_deprecated_module_use(context: &mut Context, mident: &ModuleIdent) -> bool {
    ...
    if !warn_deprecation || context.in_deprecated_code || context.in_nabob_libs {
        return false;
    }
