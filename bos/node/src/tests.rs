...
use nabob_config::config::{NodeConfig, WaypointConfig};
use nabob_event_notify::EventSubscriptionService;
use nabob_infallible::RwLock;
use nabob_storage_interface::{DbReader, DbReaderWriter, DbWriter};
use nabob_temppath::TempPath;
use nabob_types::{chain_id::ChainId, waypoint::Waypoint};
...
#[test]
fn test_nabob_vm_does_not_have_test_natives() {
    nabob_vm::natives::assert_no_test_natives(crate::utils::ERROR_MSG_BAD_FEATURE_FLAGS)
}
...
fn test_create_single_node_test_config() {
    ...
    let test_dir = nabob_temppath::TempPath::new().as_ref().to_path_buf();
    fs::DirBuilder::new()
        .recursive(true)
        .create(&test_dir)
