...
use anyhow::anyhow;
use nabob_config::config::{NodeConfig, DEFAULT_EXECUTION_CONCURRENCY_LEVEL};
#[cfg(unix)]
...
use nabob_vm_environment::prod_configs::set_paranoid_type_checks;
use std::cmp::min;

/// Error message to display when non-production features are enabled
pub const ERROR_MSG_BAD_FEATURE_FLAGS: &str = r#"
nabob-node was compiled with feature flags that shouldn't be enabled.
...
PLEASE RECOMPILE NABOB-NODE SEPARATELY using the following command:
    cargo build --package nabob-node
...
pub fn fetch_chain_id(db: &DbReaderWriter) -> anyhow::Result<ChainId> {
    let db_state_view = db
        ...
        .map_err(|err| anyhow!("[nabob-node] failed to create db state view {}", err))?;
    Ok(ChainIdResource::fetch_config(&db_state_view)
        .expect("[nabob-node] missing chain ID resource")
        .chain_id())
}

/// Sets the Nabob VM configuration based on the node configurations
pub fn set_nabob_vm_configurations(node_config: &NodeConfig) {
    ...
    NabobVM::set_concurrency_level_once(effective_concurrency_level as usize);
    NabobVM::set_discard_failed_blocks(node_config.execution.discard_failed_blocks);
    NabobVM::set_num_proof_reading_threads_once(
    ...
    {
        NabobVM::set_processed_transactions_detailed_counters();
    }
}
