...
use nabob_config::{config::NodeConfig, utils::get_genesis_txn};
use nabob_db::{fast_sync_storage_wrapper::FastSyncStorageWrapper, NabobDB};
use nabob_db_indexer::db_indexer::InternalIndexerDB;
use nabob_executor::db_bootstrapper::maybe_bootstrap;
use nabob_indexer_grpc_table_info::internal_indexer_db_service::InternalIndexerDBService;
use nabob_logger::{debug, info};
...
pub(crate) fn maybe_apply_genesis(
    ...
    if let Some(genesis) = get_genesis_txn(node_config) {
        let ledger_info_opt =
            maybe_bootstrap::<NabobVMBlockExecutor>(db_rw, genesis, genesis_waypoint)
...
pub(crate) fn bootstrap_db(
    ...
    let (nabob_db_reader, db_rw, backup_service) = match FastSyncStorageWrapper::initialize_dbs(
        node_config,
...
/// In consensus-only mode, return a in-memory based [FakeNabobDB] and
/// do not run the backup service.
#[cfg(feature = "consensus-only-perf-test")]
pub(crate) fn bootstrap_db(
    node_config: &NodeConfig,
) -> Result<(Arc<dyn DbReader>, DbReaderWriter, Option<Runtime>)> {
    use nabob_db::db::fake_nabobdb::FakeNabobDB;

    let nabob_db = NabobDB::open(
    ...
    let (nabob_db, db_rw) = DbReaderWriter::wrap(FakeNabobDB::new(nabob_db));
    maybe_apply_genesis(&db_rw, node_config)?;
    Ok((nabob_db, db_rw, None))
}
...
fn create_rocksdb_checkpoint_and_change_working_dir(
    ...
    .expect("NabobDB checkpoint creation failed.");

    // Create a consensus db checkpoint
    nabob_consensus::create_checkpoint(&source_dir, &checkpoint_dir)
        .expect("ConsensusDB checkpoint creation failed.");

    // Create a state sync db checkpoint
    let state_sync_db =
        nabob_state_svc::metadata_storage::PersistentMetadataStorage::new(&source_dir);
    state_sync_db
...
pub fn initialize_database_and_checkpoints(
    ...
    let (_nabob_db, db_rw, backup_service, indexer_db_opt, update_receiver) =
