...
use nabob_admin_svc::AdminService;
use nabob_api::bootstrap as bootstrap_api;
use nabob_build_info::build_information;
use nabob_config::config::{merge_node_config, NodeConfig, PersistableConfig};
use nabob_framework::ReleaseBundle;
use nabob_genesis::builder::GenesisConfiguration;
use nabob_logger::{prelude::*, telemetry_log_writer::TelemetryLog, Level, LoggerFilterUpdater};
use nabob_state_svc::driver_factory::StateSyncRuntimes;
use nabob_types::{
...
/// Runs an Nabob validator or fullnode
#[derive(Clone, Debug, Parser)]
#[clap(name = "Nabob Node", author, version)]
pub struct NabobNodeArgs {
    ...
    /// Paths to the Nabob framework release package to be used for genesis.
    #[clap(long, requires("test"))]
...
impl NabobNodeArgs {
    /// Runs an Nabob node based on the given command line arguments and config flags
    pub fn run(self) {
        #[cfg(target_os = "linux")]
...
pub fn start_and_report_ports(
    ...
    // Setup panic handler
    nabob_crash_handler::setup_panic_handler();

    // Create global rayon thread pool
    utils::create_global_rayon_pool(create_global_rayon_pool);

    // Initialize the global nabob-node-identity
    nabob_node_identity::init(config.get_peer_id())?;
...
pub fn start_test_environment_node(
    ...
    let nabob_root_key_path = test_dir.join("mint.key");
    ...
    println!("\tNabob root key path: {:?}", nabob_root_key_path);
    ...
    println!(
        "\tNabobnet fullnode network endpoint: {}",
    ...
    println!("\nNabob is running, press ctrl-c to exit\n");
    ...
    R: rand::RngCore + rand::CryptoRng,
{
    ...
    let test_dir =
        test_dir.unwrap_or_else(|| nabob_temppath::TempPath::new().as_ref().to_path_buf());
    ...
    R: rand::RngCore + rand::CryptoRng,
{
    ...
    let builder = nabob_genesis::builder::Builder::new(test_dir, framework.clone())?
        ...
        .with_init_genesis_config(Some(Arc::new(|genesis_config| {
            ...
            match env::var("ENABLE_KEYLESS_DEFAULT") {
                Ok(val) if val.as_str() == "1" => {
                    let response = ureq::get("https://api.devnet.naboblabs.com/v1/accounts/0x1/resource/0x1::keyless_account::Groth16VerificationKey").call();
    ...
    let mut key_file = fs::File::create(nabob_root_key_path)?;
    ...
    nabob_config::config::sanitize_node_config(validators[0].config.override_config_mut())?;
...
pub fn setup_environment_and_start_node(
    ...
    admin_service.set_nabob_db(db_rw.clone().into());

    // Set the Nabob VM configurations
    utils::set_nabob_vm_configurations(&node_config);

    // Obtain the chain_id from the DB
    let chain_id = utils::fetch_chain_id(&db_rw)?;

    // Set the chain_id in global NabobNodeIdentity
    nabob_node_identity::set_chain_id(chain_id)?;
    ...
    let (nabob_data_client, state_sync_runtimes, mempool_listener, consensus_notifier) =
...
fn verify_tool() {
    ...
    NabobNodeArgs::command().debug_assert()
}
