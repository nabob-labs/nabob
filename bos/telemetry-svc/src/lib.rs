...
#[clap(name = "Nabob Telemetry Service", author, version)]
pub struct NabobTelemetryServiceArgs {
    #[clap(short = 'f', long, value_parser)]
    config_path: PathBuf,
}

impl NabobTelemetryServiceArgs {
    pub async fn run(self) {
        ...
        let validators = Arc::new(nabob_infallible::RwLock::new(HashMap::new()));
        let validator_fullnodes = Arc::new(nabob_infallible::RwLock::new(HashMap::new()));
        let peer_locations = Arc::new(nabob_infallible::RwLock::new(HashMap::new()));
...
fn verify_tool() {
    ...
    NabobTelemetryServiceArgs::command().debug_assert()
}
