...
use nabob_config::config::{Peer, PeerRole, PeerSet};
use nabob_infallible::RwLock;
use nabob_rest_cli::Response;
use nabob_types::{
...
impl PeerSetCacheUpdater {
    ...
    async fn update_for_chain(
        ...
        let client = nabob_rest_cli::Client::new(Url::parse(url).map_err(|e| {
...
mod tests {
    ...
    use nabob_infallible::RwLock;
    use nabob_rest_cli::nabob_api_types::*;
    use nabob_types::{
    ...
    async fn test_validator_cache_updater_with_invalid_address() {
        ...
        let mock = server.mock(|when, then| {
            ...
            then.status(200)
                ...
                .header(X_NABOB_CHAIN_ID, "25")
                .header(X_NABOB_EPOCH, "10")
                .header(X_NABOB_LEDGER_VERSION, "10")
                .header(X_NABOB_LEDGER_OLDEST_VERSION, "2")
                .header(X_NABOB_BLOCK_HEIGHT, "25")
                .header(X_NABOB_OLDEST_BLOCK_HEIGHT, "10")
                .header(X_NABOB_LEDGER_TIMESTAMP, "10");
        });
    ...
    async fn test_validator_cache_updater_with_valid_address() {
        ...
        let validator_info = ValidatorInfo::new(
            ...
            ValidatorConfig::new(
                ...
                bcs::to_bytes(&vec![NetworkAddress::from_str("/dns/fullnode0.testnet.naboblabs.com/tcp/6182/noise-ik/0xea19ab47ed9191865f15d85d751ed0663205c0b2f0f465714b1947c023715973/handshake/0").unwrap()]).unwrap(),
                2,
            ),
        );
