...
use nabob_config::config::{AuthenticationConfig, NodeConfig};
use nabob_consensus::{
...
use nabob_system_utils::utils::reply_with_status;
#[cfg(target_os = "linux")]
use nabob_system_utils::{
...
pub struct Context {
    ...
    nabob_db: RwLock<Option<Arc<DbReaderWriter>>>,
...
impl Context {
    fn set_nabob_db(&self, nabob_db: Arc<DbReaderWriter>) {
        *self.nabob_db.write() = Some(nabob_db);
    }
...
impl AdminService {
    ...
    pub fn new(node_config: &NodeConfig) -> Self {
        ...
        let runtime = nabob_runtimes::spawn_named_runtime("admin".into(), None);
    ...
    pub fn set_nabob_db(&self, nabob_db: Arc<DbReaderWriter>) {
        self.context.set_nabob_db(nabob_db)
    }
