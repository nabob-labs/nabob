...
use nabob_consensus_notify::ConsensusNotificationListener;
use nabob_data_cli::client::NabobDataClient;
use nabob_data_stream_svc::streaming_client::StreamingServiceClient;
use nabob_event_notify::{EventNotificationSender, EventSubscriptionService};
use nabob_executor_types::ChunkExecutorTrait;
...
impl DriverFactory {
    ...
    >(
        ...
        let driver_runtime = if create_runtime {
            let runtime = nabob_runtimes::spawn_named_runtime("sync-driver".into(), None);
            Some(runtime)
        } else {
            None
        };
