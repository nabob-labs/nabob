...
use nabob_config::config::{ConsensusObserverConfig, RoleType, StateSyncDriverConfig};
use nabob_consensus_notify::{
    ConsensusCommitNotification, ConsensusNotification, ConsensusSyncDurationNotification,
    ConsensusSyncTargetNotification,
};
use nabob_data_cli::interface::NabobDataClientInterface;
use nabob_data_stream_svc::streaming_client::{
...
use nabob_storage_notify::StorageServiceNotificationSender;
use nabob_time_svc::{TimeService, TimeServiceTrait};
use nabob_types::{contract_event::ContractEvent, waypoint::Waypoint};
...
impl<
        DataClient: NabobDataClientInterface + Send + Clone + 'static,
    ...
    >
{
    ...
    async fn drive_progress(&mut self) {
        ...
        let global_data_summary = self.nabob_data_client.get_global_data_summary();
