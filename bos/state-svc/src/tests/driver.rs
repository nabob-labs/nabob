...
use nabob_config::config::{NodeConfig, RoleType, StateSyncDriverConfig};
use nabob_consensus_notify::{ConsensusNotificationSender, ConsensusNotifier};
...
async fn create_driver_for_tests(
    ...
    // Create test nabob database
    let db_path = nabob_temppath::TempPath::new();
    db_path.create_as_dir().unwrap();
    let (_, db_rw) = DbReaderWriter::wrap(NabobDB::new_for_test(db_path.path()));

    // Bootstrap the genesis transaction
    let (genesis, _) = nabob_vm_genesis::test_genesis_change_set_and_validators(Some(1));
    let genesis_txn = Transaction::GenesisTransaction(WriteSetPayload::Direct(genesis));
    bootstrap_genesis::<NabobVMBlockExecutor>(&db_rw, &genesis_txn).unwrap();
    ...
    let (consensus_notifier, consensus_listener) =
        nabob_consensus_notify::new_consensus_notifier_listener_pair(5000);
