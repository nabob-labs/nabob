...
use nabob_config::config::StateSyncDriverConfig;
use nabob_data_stream_svc::data_notification::NotificationId;
use nabob_event_notify::EventSubscriptionService;
use nabob_executor_types::ChunkCommitNotification;
use nabob_infallible::{Mutex, RwLock};
use nabob_mempool_notify::MempoolNotificationListener;
use nabob_storage_interface::{NabobDbError, DbReaderWriter};
...
async fn test_save_states_invalid_chunk() {
    ...
    snapshot_receiver
        .expect_add_chunk()
        .with(always(), always())
        .returning(|_, _| Err(NabobDbError::Other("Invalid chunk!".to_string())));
