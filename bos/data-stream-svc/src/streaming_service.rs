...
use nabob_data_cli::{
    ...
    interface::NabobDataClientInterface,
};
...
pub struct DataStreamingService<T> {
    ...
    stream_update_listener: nabob_channel::Receiver<(), StreamUpdateNotification>,
...
impl<T: NabobDataClientInterface + Send + Clone + 'static> DataStreamingService<T> {
    pub fn new(
        data_client_config: NabobDataClientConfig,
        streaming_service_config: DataStreamingServiceConfig,
        nabob_data_cli: T,
        ...
        let (stream_update_notifier, stream_update_listener) =
            nabob_channel::new(QueueStyle::LIFO, STREAM_PROGRESS_UPDATE_CHANNEL_SIZE, None);
...
fn verify_optimal_chunk_sizes(optimal_chunk_sizes: &OptimalChunkSizes) -> Result<(), Error> {
    ...
    {
        Err(Error::NabobDataClientResponseIsInvalid(format!(
...
mod streaming_service_tests {
    ...
    use nabob_channels::{nabob_channel, message_queues::QueueStyle};
    use nabob_config::config::DataStreamingServiceConfig;
    use futures::{
