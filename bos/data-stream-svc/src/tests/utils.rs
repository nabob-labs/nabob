...
use nabob_config::config::NabobDataClientConfig;
use nabob_crypto::{ed25519::Ed25519PrivateKey, HashValue, PrivateKey, SigningKey, Uniform};
use nabob_data_cli::{
...
impl MockNabobDataClient {
    ...
    async fn emulate_network_request_timeout(&self) -> nabob_data_cli::error::Error {
        // Sleep for a while
        tokio::time::sleep(Duration::from_secs(MAX_NOTIFICATION_TIMEOUT_SECS)).await;

        // Return a timeout error
        nabob_data_cli::error::Error::TimeoutWaitingForResponse("RPC timed out!".into())
    }
...
#[async_trait]
impl NabobDataClientInterface for MockNabobDataClient {
    ...
    async fn get_state_values_with_proof(
        ...
        end_index: u64,
        request_timeout_ms: u64,
    ) -> Result<Response<StateValueChunkWithProof>, nabob_data_cli::error::Error> {
    ...
    async fn get_epoch_ending_ledger_infos(
        ...
        end_epoch: Epoch,
        request_timeout_ms: u64,
    ) -> Result<Response<Vec<LedgerInfoWithSignatures>>, nabob_data_cli::error::Error> {
        // Verify the request timeout
    ...
    async fn get_number_of_states(
        ...
        version: Version,
        request_timeout_ms: u64,
    ) -> Result<Response<u64>, nabob_data_cli::error::Error> {
        // Verify the request timeout
    ...
    async fn get_transaction_outputs_with_proof(
        ...
        end_version: Version,
        request_timeout_ms: u64,
    ) -> Result<Response<TransactionOutputListWithProof>, nabob_data_cli::error::Error> {
        // Verify the request timeout
        let data_request =
    ...
    async fn get_transactions_with_proof(
        ...
        include_events: bool,
        request_timeout_ms: u64,
    ) -> Result<Response<TransactionListWithProof>, nabob_data_cli::error::Error> {
        // Verify the request timeout
    ...
    async fn get_transactions_or_outputs_with_proof(
        ...
        include_events: bool,
        request_timeout_ms: u64,
    ) -> nabob_data_cli::error::Result<Response<TransactionOrOutputListWithProof>> {
        // Verify the request timeout
        let data_request =
...
/// Initializes the Nabob logger for tests
