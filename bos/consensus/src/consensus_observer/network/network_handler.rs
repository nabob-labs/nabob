...
use nabob_channels::{
    nabob_channel,
    nabob_channel::{Receiver, Sender},
    message_queues::QueueStyle,
};
use nabob_config::{config::ConsensusObserverConfig, network_id::PeerNetworkId};
use nabob_logger::{error, info, warn};
...
impl ConsensusObserverNetworkHandler {
    pub fn new(
        ...
        let (observer_message_sender, observer_message_receiver) = nabob_channel::new(
            QueueStyle::FIFO,
        ...
        let (publisher_message_sender, publisher_message_receiver) = nabob_channel::new(
            QueueStyle::FIFO,
            consensus_observer_config.max_network_channel_size as usize,
            None,
        );
...
mod test {
    ...
    use nabob_channels::{nabob_channel, nabob_channel::Receiver, message_queues::QueueStyle};
    use nabob_config::{
    ...
    /// Creates and returns a single Nabob channel
    fn create_nabob_channel<K: Eq + Hash + Clone, T>(
    ) -> (nabob_channel::Sender<K, T>, nabob_channel::Receiver<K, T>) {
        nabob_channel::new(QueueStyle::FIFO, 10, None)
    }
    ...
    fn create_network_sender_and_events(
        ...
        HashMap<NetworkId, nabob_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>>,
        ...
        for network_id in network_ids {
            ...
            let (inbound_request_sender, inbound_request_receiver) = create_nabob_channel();
            let (outbound_request_sender, outbound_request_receiver) = create_nabob_channel();
            let (connection_outbound_sender, _connection_outbound_receiver) =
                create_nabob_channel();
    ...
    async fn wait_for_handler_processing(
        ...
        observer_message_receiver: &mut nabob_channel::Receiver<
            (),
            ConsensusObserverNetworkMessage,
        >,
        publisher_message_receiver: &mut nabob_channel::Receiver<
            (),
            ConsensusPublisherNetworkMessage,
        >,
    ...
    async fn wait_for_outbound_processing(
        ...
        outbound_request_receivers: &mut HashMap<
            NetworkId,
            nabob_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
        >,
