...
impl From<nabob_storage_interface::NabobDbError> for DbError {
    fn from(e: nabob_storage_interface::NabobDbError) -> Self {
...
impl From<nabob_executor_types::ExecutorError> for StateSyncError {
    fn from(e: nabob_executor_types::ExecutorError) -> Self {
        StateSyncError { inner: e.into() }
    }
}
...
pub fn error_kind(e: &anyhow::Error) -> &'static str {
    if e.downcast_ref::<nabob_executor_types::ExecutorError>()
        .is_some()
    {
        return "Execution";
    }
    if let Some(e) = e.downcast_ref::<StateSyncError>() {
        if e.inner
            .downcast_ref::<nabob_executor_types::ExecutorError>()
            .is_some()
        {
            return "Execution";
        }
    ...
    if e.downcast_ref::<nabob_safety_rules::Error>().is_some() {
        return "SafetyRules";
    }
...
mod tests {
    ...
    fn conversion_and_downcast() {
        let error = nabob_executor_types::ExecutorError::InternalError {
