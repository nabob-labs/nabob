...
use nabob_infallible::{Mutex, MutexGuard};
...
impl NabobDBBackend {
    pub fn new(window_size: usize, seek_len: usize, nabob_db: Arc<dyn DbReader>) -> Self {
        Self {
            window_size,
            seek_len,
            nabob_db,
    ...
    fn refresh_db_result(
        ...
        let events = self.nabob_db.get_latest_block_events(limit)?;
...
impl MetadataBackend for NabobDBBackend {
    ...
    fn get_block_metadata(
        ...
        let latest_db_version = self.nabob_db.get_latest_ledger_info_version().unwrap_or(0);
        // lazy init db_result
        if locked.is_none() {
