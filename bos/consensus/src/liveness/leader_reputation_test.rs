...
use super::leader_reputation::{
    extract_epoch_to_proposers_impl, NabobDBBackend, ProposerAndVoterHeuristic,
};
use crate::liveness::{
    leader_reputation::{
...
use nabob_consensus_types::common::{Author, Round};
use nabob_crypto::{bls12381, HashValue};
...
struct Example1 {
    ...
    nabob_db: Arc<MockDbReader>,
    backend: NabobDBBackend,
}
...
impl Example1 {
    fn new(window_size: usize) -> Self {
        ...
        let nabob_db = Arc::new(MockDbReader::new());
        let backend = NabobDBBackend::new(window_size, 0, nabob_db.clone());
...
fn test_api(use_root_hash: bool) {
    ...
    for epoch in 1..1000 {
        ...
        assert_eq!(
            (epoch, 1),
            nabob_db.add_event_with_data(proposers[0], vec![1, 2], vec![])
        );
        assert_eq!(
            (epoch, 2),
            nabob_db.add_event_with_data(proposers[0], vec![3], vec![])
        );
        let backend = Arc::new(NabobDBBackend::new(1, 4, nabob_db.clone()));
        ...
        let state = if use_root_hash {
            [
                nabob_db.get_accumulator_root_hash(0).unwrap().to_vec(),
...
impl DbReader for MockDbReader {
    ...
    fn get_accumulator_root_hash(
        &self,
        _version: Version,
    ) -> nabob_storage_interface::Result<HashValue> {
        Ok(HashValue::zero())
    }
}
...
fn backend_wrapper_test() {
    let nabob_db = Arc::new(MockDbReader::new());
    let backend = NabobDBBackend::new(3, 3, nabob_db.clone());

    nabob_db.add_event(0, 1);
    nabob_db.new_epoch();
    nabob_db.skip_rounds(1);
    for i in 2..6 {
        nabob_db.add_event(1, i);
    }
    ...
    let mut assert_history = |round, expected_history: Vec<Round>, to_fetch| {
        ...
        assert_eq!(fetch_count, nabob_db.fetched(), "At round {}", round);
    };
    ...
    nabob_db.add_another_transaction();
    assert_history(12, vec![12, 11, 10], false);
    ...
    nabob_db.add_another_transaction();
    nabob_db.add_event_after_call(1, 13);
...
fn backend_test_cross_epoch() {
    ...
    nabob_db.add_event(0, 1);
    nabob_db.new_epoch();
    nabob_db.add_event(1, 1);
    nabob_db.add_event(1, 2);
    nabob_db.add_event(1, 3);
    nabob_db.new_epoch();
    nabob_db.add_event(2, 1);
    nabob_db.add_event(2, 2);
