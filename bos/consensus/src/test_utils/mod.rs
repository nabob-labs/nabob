...
use nabob_crypto::{HashValue, PrivateKey, Uniform};
use nabob_logger::Level;
use nabob_types::{ledger_info::LedgerInfo, validator_signer::ValidatorSigner};
...
use nabob_consensus_types::{block::block_test_utils::gen_test_certificate, common::Payload};
use nabob_crypto::ed25519::{Ed25519PrivateKey, Ed25519Signature};
...
pub fn consensus_runtime() -> runtime::Runtime {
    if nocapture() {
        ::nabob_logger::Logger::new().level(Level::Debug).init();
    }

    nabob_runtimes::spawn_named_runtime("consensus".into(), None)
}
