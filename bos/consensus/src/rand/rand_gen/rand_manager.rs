...
use nabob_config::config::ReliableBroadcastConfig;
use nabob_consensus_types::common::{Author, Round};
use nabob_infallible::Mutex;
use nabob_logger::{error, info, spawn_named, trace, warn};
use nabob_network::{protocols::network::RpcError, ProtocolId};
use nabob_reliable_broadcast::{DropGuard, ReliableBroadcast};
...
impl<S: TShare, D: TAugmentedData> RandManager<S, D> {
    ...
    async fn verification_task(
        epoch_state: Arc<EpochState>,
        mut incoming_rpc_request: nabob_channel::Receiver<Author, IncomingRandGenRequest>,
    ...
    pub async fn start(
        ...
        incoming_rpc_request: nabob_channel::Receiver<Author, IncomingRandGenRequest>,
