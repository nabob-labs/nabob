...
use nabob_runtimes_extension::thread_manager::optimal_min_len;
use nabob_logger::prelude::*;
use nabob_mempool::QuorumStoreRequest;
use nabob_types::{transaction::SignedTransaction, PeerId};
...
impl BatchGenerator {
    pub(crate) fn new(
            ...
            .expect("Could not read from db")
        {
            ...
            BatchId::new(nabob_infallible::duration_since_epoch().as_micros() as u64)
        };
    ...
    pub(crate) async fn handle_scheduled_pull(&mut self, max_count: u64) -> Vec<Batch> {
        ...
        let expiry_time = nabob_infallible::duration_since_epoch().as_micros() as u64
    ...
    pub(crate) fn handle_remote_batch(
        ...
        let expiry_time_usecs = nabob_infallible::duration_since_epoch().as_micros() as u64
