...
pub trait PersistentLivenessStorage: Send + Sync {
    ...
    /// Returns a handle of the nabobdb.
    fn nabob_db(&self) -> Arc<dyn DbReader>;
...
impl RootMetadata {
    ...
    pub fn new_empty() -> Self {
        Self {
            accu_hash: *nabob_crypto::hash::ACCUMULATOR_PLACEHOLDER_HASH,
...
impl StorageWriteProxy {
    pub fn new(config: &NodeConfig, nabob_db: Arc<dyn DbReader>) -> Self {
        let db = Arc::new(ConsensusDB::new(config.storage.dir()));
        StorageWriteProxy { db, nabob_db }
    }
}
