...
use nabob_channels::nabob_channel::Receiver;
use nabob_config::config::NodeConfig;
use nabob_consensus_notify::ConsensusNotificationSender;
...
pub fn start_consensus(
    ...
    nabob_db: DbReaderWriter,
    reconfig_events: ReconfigNotificationListener<DbBackedOnChainConfig>,
    vtxn_pool: VTxnPoolState,
    consensus_publisher: Option<Arc<ConsensusPublisher>>,
) -> (Runtime, Arc<StorageWriteProxy>, Arc<QuorumStoreDB>) {
    let runtime = nabob_runtimes::spawn_named_runtime("consensus".into(), None);
    let storage = Arc::new(StorageWriteProxy::new(node_config, nabob_db.reader.clone()));
    ...
    let execution_proxy = ExecutionProxy::new(
        Arc::new(BlockExecutor::<NabobVMBlockExecutor>::new(nabob_db)),
        txn_notifier,
        state_sync_notifier,
        runtime.handle(),
...
pub fn start_consensus_observer(
    ...
    let execution_client = if node_config.consensus_observer.observer_enabled {
        ...
        let execution_proxy = ExecutionProxy::new(
            Arc::new(BlockExecutor::<NabobVMBlockExecutor>::new(nabob_db.clone())),
            txn_notifier,
            state_sync_notifier,
