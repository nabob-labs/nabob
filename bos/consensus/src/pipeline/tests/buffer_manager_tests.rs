...
use nabob_config::{config::ConsensusObserverConfig, network_id::NetworkId};
use nabob_consensus_types::{
    block::block_test_utils::certificate_for_genesis, pipelined_block::PipelinedBlock,
    vote_proposal::VoteProposal,
};
use nabob_crypto::{hash::ACCUMULATOR_PLACEHOLDER_HASH, HashValue};
...
use nabob_safety_rules::{PersistentSafetyStorage, SafetyRulesManager};
use nabob_secure_storage::Storage;
use nabob_types::{
...
pub fn prepare_buffer_manager(
    ...
    Sender<ResetRequest>,
    nabob_channel::Sender<AccountAddress, (AccountAddress, IncomingCommitRequest)>,
    nabob_channels::UnboundedReceiver<Event<ConsensusMsg>>,
    ...
    let safety_storage = PersistentSafetyStorage::initialize(
        Storage::from(nabob_secure_storage::InMemoryStorage::new()),
        signer.author(),
    ...
    let (network_reqs_tx, _network_reqs_rx) = nabob_channel::new(QueueStyle::FIFO, 8, None);
    ...
    let (self_loop_tx, self_loop_rx) = nabob_channels::new_unbounded_test();
    ...
    let (msg_tx, msg_rx) = nabob_channel::new::<
        AccountAddress,
...
async fn loopback_commit_vote(
    ...
    msg_tx: &nabob_channel::Sender<AccountAddress, (AccountAddress, IncomingCommitRequest)>,
