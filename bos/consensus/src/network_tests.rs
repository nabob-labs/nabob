...
use nabob_infallible::{Mutex, RwLock};
use nabob_network::{
...
use nabob_types::{block_info::BlockInfo, PeerId};
...
pub struct NetworkPlayground {
    ...
    ///
    node_consensus_txs:
        Arc<Mutex<HashMap<TwinId, nabob_channel::Sender<(PeerId, ProtocolId), ReceivedMessage>>>>,
...
impl NetworkPlayground {
    ...
    async fn start_node_outbound_handler(
        ...
        drop_config: Arc<RwLock<DropConfig>>,
        src_twin_id: TwinId,
        mut network_reqs_rx: nabob_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
        mut outbound_msgs_tx: mpsc::Sender<(TwinId, PeerManagerRequest)>,
        node_consensus_txs: Arc<
            Mutex<HashMap<TwinId, nabob_channel::Sender<(PeerId, ProtocolId), ReceivedMessage>>>,
        >,
        author_to_twin_ids: Arc<RwLock<AuthorToTwinIds>>,
    ) {
    ...
    pub fn add_node(
        &mut self,
        twin_id: TwinId,
        consensus_tx: nabob_channel::Sender<(PeerId, ProtocolId), ReceivedMessage>,
        network_reqs_rx: nabob_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
        conn_mgr_reqs_rx: nabob_channels::Receiver<nabob_network::ConnectivityRequest>,
    ) {
...
mod tests {
    ...
    use nabob_types::validator_verifier::random_validator_verifier;
    use bytes::Bytes;
    ...
    fn test_network_api() {
        ...
        for (peer_id, peer) in peers.iter().enumerate() {
            let (network_reqs_tx, network_reqs_rx) = nabob_channel::new(QueueStyle::FIFO, 8, None);
            let (connection_reqs_tx, _) = nabob_channel::new(QueueStyle::FIFO, 8, None);
            let (consensus_tx, consensus_rx) = nabob_channel::new(QueueStyle::FIFO, 8, None);
            let (_conn_mgr_reqs_tx, conn_mgr_reqs_rx) = nabob_channels::new_test(1024);
            ...
            let (self_sender, self_receiver) = nabob_channels::new_unbounded_test();
    ...
    fn test_bad_message() {
        ...
        let (peer_mgr_notifs_tx, peer_mgr_notifs_rx) =
            nabob_channel::new(QueueStyle::FIFO, 8, None);
