...
use crate::{
    ...
    liveness::{
        ...
        leader_reputation::{
            extract_epoch_to_proposers, NabobDBBackend, LeaderReputation,
...
use nabob_bounded_executor::BoundedExecutor;
use nabob_channels::{nabob_channel, message_queues::QueueStyle};
use nabob_config::config::{ConsensusConfig, DagConsensusConfig, ExecutionConfig, NodeConfig};
use nabob_consensus_types::{
...
use nabob_crypto::bls12381::PrivateKey;
use nabob_dkg::{
    pvss::{traits::Transcript, Player},
    weighted_vuf::traits::WeightedVUF,
};
use nabob_event_notify::ReconfigNotificationListener;
use nabob_infallible::{duration_since_epoch, Mutex};
use nabob_logger::prelude::*;
use nabob_mempool::QuorumStoreRequest;
use nabob_network::{application::interface::NetworkClient, protocols::network::Event};
use nabob_safety_rules::{
...
pub struct EpochManager<P: OnChainConfigProvider> {
    ...
    self_sender: nabob_channels::UnboundedSender<Event<ConsensusMsg>>,
    network_sender: ConsensusNetworkClient<NetworkClient<ConsensusMsg>>,
    timeout_sender: nabob_channels::Sender<Round>,
    ...
    rand_manager_msg_tx: Option<nabob_channel::Sender<AccountAddress, IncomingRandGenRequest>>,
    // channels to round manager
    round_manager_tx: Option<
        nabob_channel::Sender<(Author, Discriminant<VerifiedEvent>), (Author, VerifiedEvent)>,
    >,
    buffered_proposal_tx: Option<nabob_channel::Sender<Author, VerifiedEvent>>,
    round_manager_close_tx: Option<oneshot::Sender<oneshot::Sender<()>>>,
    epoch_state: Option<Arc<EpochState>>,
    block_retrieval_tx:
        Option<nabob_channel::Sender<AccountAddress, IncomingBlockRetrievalRequest>>,
    quorum_store_msg_tx: Option<nabob_channel::Sender<AccountAddress, (Author, VerifiedEvent)>>,
    quorum_store_coordinator_tx: Option<Sender<CoordinatorCommand>>,
    quorum_store_storage: Arc<dyn QuorumStoreStorage>,
    batch_retrieval_tx:
        Option<nabob_channel::Sender<AccountAddress, IncomingBatchRetrievalRequest>>,
...
impl<P: OnChainConfigProvider> EpochManager<P> {
    ...
    fn create_proposer_election(
        ...
        match &onchain_config.proposer_election_type() {
            ...
            ProposerElectionType::LeaderReputation(leader_reputation_type) => {
                ...
                let backend = Arc::new(NabobDBBackend::new(
                    window_size,
                    seek_len,
                    self.storage.nabob_db(),
                ));
    ...
    fn spawn_block_retrieval_task(
        ...
        let (request_tx, mut request_rx) = nabob_channel::new::<_, IncomingBlockRetrievalRequest>(
            QueueStyle::KLAST,
            10,
    ...
    async fn start_recovery_manager(
        ...
        let (recovery_manager_tx, recovery_manager_rx) = nabob_channel::new(
            QueueStyle::KLAST,
            10,
    ...
    async fn start_round_manager(
        ...
        rand_msg_rx: nabob_channel::Receiver<AccountAddress, IncomingRandGenRequest>,
    ) {
        ...
        let (round_manager_tx, round_manager_rx) = nabob_channel::new(
            QueueStyle::KLAST,
            10,
            Some(&counters::ROUND_MANAGER_CHANNEL_MSGS),
        );

        let (buffered_proposal_tx, buffered_proposal_rx) = nabob_channel::new(
            QueueStyle::KLAST,
            10,
            Some(&counters::ROUND_MANAGER_CHANNEL_MSGS),
        );
    ...
    async fn start_new_epoch(&mut self, payload: OnChainConfigPayload<P>) {
        ...
        let (rand_msg_tx, rand_msg_rx) = nabob_channel::new::<AccountAddress, IncomingRandGenRequest>(
            QueueStyle::KLAST,
            10,
            None,
        );
    ...
    async fn start_new_epoch_with_dag(
        ...
        let (dag_rpc_tx, dag_rpc_rx) = nabob_channel::new(QueueStyle::FIFO, 10, None);
    ...
    fn forward_event_to<K: Eq + Hash + Clone, V>(
        mut maybe_tx: Option<nabob_channel::Sender<K, V>>,
        key: K,
        value: V,
    ) -> anyhow::Result<()> {
    ...
    pub async fn start(
        mut self,
        mut round_timeout_sender_rx: nabob_channels::Receiver<Round>,
