...
use nabob_config::config::{ReliableBroadcastConfig, SafetyRulesConfig};
use nabob_event_notify::{
    EventNotification, EventNotificationListener, ReconfigNotification,
    ReconfigNotificationListener,
};
use nabob_logger::{debug, error, info, warn};
use nabob_network::{application::interface::NetworkClient, protocols::network::Event};
use nabob_reliable_broadcast::ReliableBroadcast;
use nabob_safety_rules::{safety_rules_manager::storage, PersistentSafetyStorage};
use nabob_types::{
...
pub struct EpochManager<P: OnChainConfigProvider> {
    ...
    // Msgs to DKG manager
    dkg_rpc_msg_tx:
        Option<nabob_channel::Sender<AccountAddress, (AccountAddress, IncomingRpcRequest)>>,
    dkg_manager_close_tx: Option<oneshot::Sender<oneshot::Sender<()>>>,
    dkg_start_event_tx: Option<nabob_channel::Sender<(), DKGStartEvent>>,
    vtxn_pool: VTxnPoolState,

    // Network utils
    self_sender: nabob_channels::Sender<Event<DKGMessage>>,
    ...
    async fn start_new_epoch(&mut self, payload: OnChainConfigPayload<P>) -> Result<()> {
        ...
        if let (true, Some(my_index)) = (randomness_enabled, my_index) {
            ...
            let (dkg_start_event_tx, dkg_start_event_rx) =
                nabob_channel::new(QueueStyle::KLAST, 1, None);
            self.dkg_start_event_tx = Some(dkg_start_event_tx);

            let (dkg_rpc_msg_tx, dkg_rpc_msg_rx) = nabob_channel::new::<
                AccountAddress,
