...
use nabob_crypto::Uniform;
use nabob_infallible::duration_since_epoch;
...
use nabob_validator_txpool::{TxnGuard, VTxnPoolState};
use fail::fail_point;
...
pub struct DKGManager<DKG: DKGTrait> {
    ...
    agg_trx_tx: Option<nabob_channel::Sender<(), DKG::Transcript>>,

    // When we put vtxn in the pool, we also put a copy of this so later pool can notify us.
    pull_notification_tx: nabob_channel::Sender<(), Arc<ValidatorTransaction>>,
    pull_notification_rx: nabob_channel::Receiver<(), Arc<ValidatorTransaction>>,
...
impl<DKG: DKGTrait> DKGManager<DKG> {
    ...
    pub async fn run(
        ...
        mut dkg_start_event_rx: nabob_channel::Receiver<(), DKGStartEvent>,
        mut rpc_msg_rx: nabob_channel::Receiver<
            AccountAddress,
        ...
        let (agg_trx_tx, mut agg_trx_rx) = nabob_channel::new(QueueStyle::KLAST, 1, None);
