...
use crate::{
    client::NabobDataClient, error::Error, interface::NabobDataClientInterface,
...
use nabob_time_svc::MockTimeService;
use nabob_types::{
...
pub fn remove_distance_metadata(client: &NabobDataClient, peer: PeerNetworkId) {
...
pub fn remove_latency_metadata(client: &NabobDataClient, peer: PeerNetworkId) {
    // Get the peer monitoring metadata
    let peers_and_metadata = client.get_peers_and_metadata();
    let peer_metadata = peers_and_metadata.get_metadata_for_peer(peer).unwrap();
    let mut peer_monitoring_metadata = peer_metadata.get_peer_monitoring_metadata().clone();
