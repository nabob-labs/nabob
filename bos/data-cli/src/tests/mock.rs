...
use crate::{
    client::NabobDataClient,
    error::Result,
    global_summary::GlobalDataSummary,
    interface::{NabobDataClientInterface, Response, SubscriptionRequestMetadata},
    poller::DataSummaryPoller,
    priority::PeerPriority,
};
use nabob_channels::{nabob_channel, message_queues::QueueStyle};
use nabob_config::{
    config::{NabobDataClientConfig, BaseConfig, RoleType},
...
use nabob_storage_svc::network::{NetworkRequest, ResponseSender};
...
pub struct MockNetwork {
    ...
    peer_mgr_reqs_rxs:
        HashMap<NetworkId, nabob_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>>, // The peer manager request receivers
}

impl MockNetwork {
    pub fn new(
        base_config: Option<BaseConfig>,
        data_client_config: Option<NabobDataClientConfig>,
        networks: Option<Vec<NetworkId>>,
    ) -> (Self, MockTimeService, NabobDataClient, DataSummaryPoller) {
        // Initialize the logger for testing
        ::nabob_logger::Logger::init_for_testing();
        ...
        for network in &networks {
            ...
            let queue_cfg = nabob_channel::Config::new(10).queue_style(QueueStyle::FIFO);
        ...
        let (client, poller) = NabobDataClient::new(
            data_client_config,
            base_config.clone(),
            mock_time.clone(),
...
pub fn create_mock_data_client() -> Arc<dyn NabobDataClientInterface + Send + Sync> {
    Arc::new(MockNabobDataClient::new())
}

// This automatically creates a MockNabobDataClient
mock! {
    pub NabobDataClient {}
...
impl DbReader for MockDatabaseReader {
    fn get_block_timestamp(&self, version: Version) -> nabob_storage_interface::Result<u64> {
        Ok(version * 100_000)
    }
}
