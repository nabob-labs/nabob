...
/// An error returned by the Nabob Data Client for failed API calls.
...
impl From<nabob_storage_cli::Error> for Error {
    fn from(error: nabob_storage_cli::Error) -> Self {
        Self::UnexpectedErrorEncountered(error.to_string())
    }
}

impl From<nabob_storage_types::responses::Error> for Error {
    fn from(error: nabob_storage_types::responses::Error) -> Self {
