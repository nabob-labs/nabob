...
use crate::{
    ...
    interface::{
        NabobDataClientInterface, Response, ResponseCallback, ResponseContext, ResponseError,
...
use nabob_config::{
    config::{NabobDataClientConfig, BaseConfig},
    network_id::PeerNetworkId,
};
use nabob_id_generator::{IdGenerator, U64IdGenerator};
use nabob_infallible::Mutex;
use nabob_logger::{info, sample, sample::SampleRate, trace, warn};
use nabob_network::{
...
/// An [`NabobDataClientInterface`] that fulfills requests from remote peers' Storage Service
/// over NabobNet.
///
/// The `NabobDataClient`:
///
/// 1. Sends requests to connected Nabob peers.
...
/// and/or threads.
#[derive(Clone, Debug)]
pub struct NabobDataClient {
    /// The base config of the node.
    base_config: Arc<BaseConfig>,
    /// The config for the NabobNet data client.
    data_client_config: Arc<NabobDataClientConfig>,
    /// The underlying NabobNet storage service client.
...
impl NabobDataClient {
        ...
        E: Into<Error>,
    {
        ...
        for peer in peers {
            ...
            let nabob_data_client = self.clone();
    ...
    async fn send_request_to_peer(
        ...
        match result {
            Ok(response) => {
                ...
                // with this RPC.
                let response_callback = NabobNetResponseCallback {
            ...
            Err(error) => {
                ...
                let client_error = match error {
                    nabob_storage_cli::Error::RpcError(rpc_error) => match rpc_error {
                    ...
                    nabob_storage_cli::Error::StorageServiceError(err) => {
...
#[async_trait]
impl NabobDataClientInterface for NabobDataClient {
...
/// The NabobNet-specific request context needed to update a peer's scoring.
struct NabobNetResponseCallback {
    data_client: NabobDataClient,
    id: ResponseId,
    peer: PeerNetworkId,
    request: StorageServiceRequest,
}

impl ResponseCallback for NabobNetResponseCallback {
...
impl fmt::Debug for NabobNetResponseCallback {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("NabobNetResponseCallback")
            .field("data_client", &"..")
            .field("id", &self.id)
