...
use nabob_compression::{client::CompressionClient, CompressedData};
use nabob_config::config::{
    NabobDataClientConfig, StorageServiceConfig, MAX_APPLICATION_MESSAGE_SIZE,
};
...
impl From<nabob_compression::Error> for Error {
    fn from(error: nabob_compression::Error) -> Self {
...
impl StorageServiceResponse {
    ...
    pub fn new(data_response: DataResponse, perform_compression: bool) -> Result<Self, Error> {
        if perform_compression {
            ...
            let compressed_data = nabob_compression::compress(
                raw_data,
    ...
    pub fn get_data_response(&self) -> Result<DataResponse, Error> {
        match self {
            StorageServiceResponse::CompressedResponse(_, compressed_data) => {
                let raw_data = nabob_compression::decompress(
                    compressed_data,
                    CompressionClient::StateSync,
                    MAX_APPLICATION_MESSAGE_SIZE,
                )?;
