...
use nabob_reliable_broadcast::RBNetworkSender;
...
pub struct NetworkReceivers {
    pub rpc_rx: nabob_channel::Receiver<AccountAddress, (AccountAddress, IncomingRpcRequest)>,
}

pub struct NetworkTask {
    all_events: Box<dyn Stream<Item = Event<JWKConsensusMsg>> + Send + Unpin>,
    rpc_tx: nabob_channel::Sender<AccountAddress, (AccountAddress, IncomingRpcRequest)>,
}
...
impl NetworkTask {
    ...
    pub fn new(
        ...
        self_receiver: nabob_channels::Receiver<Event<JWKConsensusMsg>>,
    ) -> (NetworkTask, NetworkReceivers) {
        let (rpc_tx, rpc_rx) = nabob_channel::new(QueueStyle::FIFO, 10, None);
