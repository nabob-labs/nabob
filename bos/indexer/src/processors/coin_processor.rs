...
use nabob_api_types::Transaction as APITransaction;
use nabob_types::{NabobCoinType, CoinType};
...
impl TransactionProcessor for CoinTransactionProcessor {
    ...
    async fn process_transactions(
        ...
        // get nabob_coin info for supply tracking
        // TODO: This only needs to be fetched once. Need to persist somehow
        let maybe_nabob_coin_info =
            &CoinInfoQuery::get_by_coin_type(NabobCoinType::type_tag().to_string(), &mut conn)
                .unwrap();
        ...
        for txn in &transactions {
            let (
                ...
                current_coin_balances,
                mut coin_supply,
            ) = CoinActivity::from_transaction(txn, maybe_nabob_coin_info);
