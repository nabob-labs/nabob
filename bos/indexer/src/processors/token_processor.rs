...
use crate::{
    ...
    models::{
        ...
        token_models::{
            ...
            v2_token_utils::{
                NabobCollection, BurnEvent, FixedSupply, ObjectWithMetadata, PropertyMap, TokenV2,
...
use nabob_api_types::{Transaction, TransactionPayload, WriteSetChange};
            ...
            for wsc in user_txn.info.changes.iter() {
                if let WriteSetChange::WriteResource(wr) = wsc {
                    ...
                    if let Some(aggregated_data) = token_v2_metadata_helper.get_mut(&address) {
                        ...
                        if let Some(nabob_collection) =
                            NabobCollection::from_write_resource(wr, txn_version).unwrap()
                        {
