...
use anyhow::Context;
use nabob_api_types::{WriteResource as APIWriteResource, WriteTableItem as APIWriteTableItem};
    ...
    pub fn get_v2_from_write_resource(
        ...
        if let V2TokenResource::Collection(inner) = &V2TokenResource::from_resource(
            ...
            if let Some(metadata) = token_v2_metadata.get(&resource.address) {
                ...
                // Getting collection mutability config from NabobCollection
                let collection = metadata.nabob_collection.as_ref();
