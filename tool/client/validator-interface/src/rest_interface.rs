...
use crate::{NabobValidatorInterface, FilterCondition};
use anyhow::{anyhow, Result};
use nabob_api_types::{NabobError, NabobErrorCode};
use nabob_framework::{
...
async fn check_and_obtain_source_code(
    ...
    if let Some(target_package) = target_package_opt {
        ...
        if NABOB_PACKAGES.contains(&target_package.name.as_str()) {
            package_cache.insert(
                m.clone(),
                (
                    AccountAddress::ONE,
                    target_package.name.clone(), // all nabob packages are stored under 0x1
                    HashMap::new(),
                ),
            );
            txns.push((
                ...
                Some((
                    AccountAddress::ONE,
                    target_package.name, // all nabob packages are stored under 0x1
                    HashMap::new(),
                )), // do not need to store the package registry for nabob packages
            ));
...
impl NabobValidatorInterface for RestDebuggerInterface {
            ...
            Err(err) => match err {
                RestError::Api(NabobErrorResponse {
                    error:
                        NabobError {
                            error_code:
                                NabobErrorCode::StateValueNotFound | NabobErrorCode::TableItemNotFound, /* bug in pre 1.9 nodes */
                            ..
                        },
                    ..
                }) => Ok(None),
    ...
    > {
        ...
        let extract_entry_fun = |payload: &TransactionPayload| -> Option<EntryFunction> {
            match payload {
                ...
                {
                    let nabob_types::transaction::MultisigTransactionPayload::EntryFunction(e) =
