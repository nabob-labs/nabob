...
use nabob_block_executor::txn_provider::{default::DefaultTxnProvider, TxnProvider};
use nabob_gas_profiling::{GasProfiler, TransactionGasLog};
use nabob_rest_cli::Client;
use nabob_types::{
...
use nabob_validator_interface::{
    NabobValidatorInterface, DBDebuggerInterface, DebuggerStateView, RestDebuggerInterface,
};
use nabob_vm::{
    nabob_vm::NabobVMBlockExecutor, data_cache::AsMoveResolver, NabobVM, VMBlockExecutor,
};
use nabob_vm_environment::environment::NabobEnvironment;
use nabob_vm_logging::log_schema::AdapterLogSchema;
use nabob_vm_types::{module_and_script_storage::AsNabobCodeStorage, output::VMOutput};
use itertools::Itertools;
use std::{path::Path, sync::Arc, time::Instant};

pub struct NabobDebugger {
    debugger: Arc<dyn NabobValidatorInterface + Send>,
}

impl NabobDebugger {
    pub fn new(debugger: Arc<dyn NabobValidatorInterface + Send>) -> Self {
        Self { debugger }
    }
...
fn execute_block_no_limit(
    ...
    let executor = NabobVMBlockExecutor::new();
    executor
