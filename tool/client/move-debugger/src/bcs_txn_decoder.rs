...
use crate::nabob_debugger::NabobDebugger;
use anyhow::Result;
...
impl Command {
    pub async fn run(self) -> Result<()> {
        ...
        let endpoint = format!("https://{}.naboblabs.com/v1", network);
        let debugger = NabobDebugger::rest_client(Client::new(Url::parse(&endpoint)?))?;
        ...
        println!(
            "Overview: https://explorer.naboblabs.com/txn/{:?}/userTxnOverview?network={}",
            version, network
        );
        println!(
            "Payload: https://explorer.naboblabs.com/txn/{:?}/payload?network={}",
            version, network
        );
