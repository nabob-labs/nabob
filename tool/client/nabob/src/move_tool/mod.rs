...
use nabob_api_types::NabobErrorCode;
...
use nabob_gas_schedule::{MiscGasParameters, NativeGasParameters};
use nabob_move_debugger::nabob_debugger::NabobDebugger;
use nabob_rest_cli::{
    nabob_api_types::{EntryFunctionId, HexEncodedBytes, IdentifierWrapper, MoveModuleId},
    error::RestError,
    NabobBaseUrl, Client,
};
use nabob_types::{
    ...
    on_chain_config::nabob_test_feature_flags_genesis,
    transaction::{Transaction, TransactionArgument, TransactionPayload, TransactionStatus},
};
use nabob_vm::data_cache::AsMoveResolver;
...
const HELLO_BLOCKCHAIN_EXAMPLE: &str = include_str!(
    "../../../../../mos/move-examples/hello_blockchain/sources/hello_blockchain.move"
);
...
pub struct FrameworkPackageArgs {
    /// Git revision or branch for the Nabob framework
    ///
    ...
    /// Local framework directory for the Nabob framework
    ///
...
impl FrameworkPackageArgs {
    pub fn init_move_dir(
        ...
        const NABOB_FRAMEWORK: &str = "NabobFramework";
        const NABOB_GIT_PATH: &str = "https://github.com/nabob-labs/nabob-framework.git";
        const SUBDIR_PATH: &str = "nabob-framework";
        ...
        if let Some(ref path) = self.framework_local_dir {
            dependencies.insert(NABOB_FRAMEWORK.to_string(), Dependency {
            ...
            dependencies.insert(NABOB_FRAMEWORK.to_string(), Dependency {
                local: None,
                git: Some(NABOB_GIT_PATH.to_string()),
...
/// This can then be run with `nabob move run-script`
#[derive(Parser)]
...
pub struct TestPackage {
    ...
    /// Collect coverage information for later use with the various `nabob move coverage` subcommands
    #[clap(long = "coverage")]
...
impl CliCommand<&'static str> for TestPackage {
    ...
    async fn execute(self) -> CliTypedResult<&'static str> {
        ...
        let result = move_cli::base::test::run_move_unit_tests(
            ...
            nabob_debug_natives::nabob_debug_natives(
                NativeGasParameters::zeros(),
                MiscGasParameters::zeros(),
            ),
            nabob_test_feature_flags_genesis(),
            None,
            None,
        ...
        if self.compute_coverage {
            ...
            println!("Please use `nabob move coverage -h` for more detailed source or bytecode test coverage of this package");
        }
...
/// Publishes the modules in a Move package to the Nabob blockchain
#[derive(Parser)]
...
fn create_package_publication_data(
    ...
    let payload = match publish_type {
        PublishType::AccountDeploy => {
            nabob_cached_packages::nabob_stdlib::code_publish_package_txn(
                metadata_serialized.clone(),
                compiled_units.clone(),
            )
        },
        PublishType::ObjectDeploy => {
            nabob_cached_packages::nabob_stdlib::object_code_deployment_publish(
                metadata_serialized.clone(),
                compiled_units.clone(),
            )
        },
        PublishType::ObjectUpgrade => {
            nabob_cached_packages::nabob_stdlib::object_code_deployment_upgrade(
                metadata_serialized.clone(),
                compiled_units.clone(),
...
/// Publishes the modules in a Move package to the Nabob blockchain, under an object (legacy version of `deploy-object`)
#[derive(Parser)]
...
/// Publishes the modules in a Move package to the Nabob blockchain, under an object.
#[derive(Parser)]
...
async fn submit_chunked_publish_transactions(
    ...
    if !is_staging_area_empty(txn_options, large_packages_module_address).await? {
        ...
        \nUse the `nabob move clear-staging-area` command to clean up the `StagingArea` resource under the account.",
    ...
    for (idx, payload) in payloads.into_iter().enumerate() {
        ...
        match result {
            ...
            Err(e) => {
                ...
                \nTo avoid this, use the `nabob move clear-staging-area` command to clean up the `StagingArea` resource under your account before retrying.".bold());
                return Err(e);
            },
        }
    }
...
async fn is_staging_area_empty(
    ...
    match staging_area_response {
        ...
        Err(RestError::Api(nabob_error_response))
            if nabob_error_response.error.error_code == NabobErrorCode::ResourceNotFound =>
        {
...
/// Publishes the modules in a Move package to the Nabob blockchain under a resource account
#[derive(Parser)]
...
impl CliCommand<TransactionSummary> for CreateResourceAccountAndPublishPackage {
    ...
    async fn execute(self) -> CliTypedResult<TransactionSummary> {
        ...
        .map(|p| p.account)
        {
            ...
            return Err(CliError::CommandArgumentError(
                "Please provide an account using --profile or run nabob init".to_string(),
            ));
        };
        ...
        let payload = nabob_cached_packages::nabob_stdlib::resource_account_create_resource_account_and_publish_package(
            seed,
...
impl CliCommand<TransactionSummary> for Replay {
    ...
    async fn execute(self) -> CliTypedResult<TransactionSummary> {
        ...
        let rest_endpoint = match &self.network {
            Mainnet => "https://fullnode.mainnet.naboblabs.com",
            Testnet => "https://fullnode.testnet.naboblabs.com",
            Devnet => "https://fullnode.devnet.naboblabs.com",
            RestEndpoint(url) => url,
        };

        // Build the client
        let client = Client::builder(NabobBaseUrl::Custom(
        ...
        let debugger = NabobDebugger::rest_client(client)?;
...
/// Duplicated from nabob_types, as we also need to load_account_arg from the CLI.
#[derive(Debug, Clone)]
pub struct MemberId {
