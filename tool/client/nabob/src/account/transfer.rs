...
use nabob_rest_cli::{
    nabob_api_types::{HashValue, WriteResource, WriteSetChange},
    Transaction,
};
...
impl CliCommand<TransferSummary> for TransferCoins {
    ...
    async fn execute(self) -> CliTypedResult<TransferSummary> {
        self.txn_options
            .submit_transaction(nabob_stdlib::nabob_account_transfer(
                self.account,
                self.amount,
            ))
            .await
...
const SUPPORTED_COINS: [&str; 1] = ["0x1::coin::CoinStore<0x1::nabob_coin::NabobCoin>"];
