...
use nabob_vm_genesis::{default_gas_schedule, AccountBalance, EmployeePool};
...
/// Tool for setting up an Nabob chain Genesis transaction
///
...
pub fn fetch_mainnet_genesis_info(git_options: GitOptions) -> CliTypedResult<MainnetGenesisInfo> {
    ...
    // Check that the user has a reasonable amount of BOB, since below the minimum gas amount is
    // not useful 1 BOB minimally
