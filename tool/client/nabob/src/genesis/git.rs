...
use nabob_config::config::Token;
use nabob_framework::ReleaseBundle;
use nabob_genesis::config::Layout;
use nabob_github_cli::Client as GithubClient;
...
impl FromStr for GithubRepo {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if parts.len() != 2 {
            Err(CliError::CommandArgumentError("Invalid repository must be of the form 'owner/repository` e.g. 'nabob-labs/nabob'".to_string()))
        } else {
            Ok(GithubRepo {
...
pub struct GitOptions {
    /// Github repository e.g. 'nabob-labs/nabob'
    ///
