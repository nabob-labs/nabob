...
use crate::{
    common::{
        types::{
            ...
            ProfileSummary, NABOB_FOLDER_GIT_IGNORE, CONFIG_FOLDER, GIT_IGNORE,
        },
...
use nabob_cli_common::generate_cli_completions;
...
/// Tool for interacting with configuration of the Nabob CLI tool
///
...
impl CliCommand<()> for GenerateShellCompletions {
    ...
    async fn execute(self) -> CliTypedResult<()> {
        generate_cli_completions::<Tool>("nabob", self.shell, self.output_file.as_path())
...
pub struct SetGlobalConfig {
    ...
    ///
    /// `Workspace` will put the `.nabob/` folder in the current directory, where
    /// `Global` will put the `.nabob/` folder in your home directory
    #[clap(long)]
...
pub enum ConfigType {
    /// Per system user configuration put in `<HOME>/.nabob`
    Global,
    /// Per directory configuration put in `<CURRENT_DIR>/.nabob`
    Workspace,
}
