...
use nabob_crypto::ed25519::{Ed25519PrivateKey, Ed25519PublicKey};
use nabob_keygen::KeyGen;
use nabob_logger::{debug, Level};
use nabob_rest_cli::{nabob_api_types::HashValue, Account, Client, FaucetClient, State};
use nabob_telemetry_cli::service::telemetry_is_disabled;
use nabob_types::{
...
async fn send_telemetry_event(command: &str, latency: Duration, error: Option<&str>) {
    ...
    // Send the event
    nabob_telemetry_cli::cli_metrics::send_cli_telemetry_event(
        build_information,
        command.into(),
        latency,
        error.is_none(),
...
pub fn start_logger(level: Level) {
    let mut logger = nabob_logger::Logger::new();
...
pub fn explorer_account_link(hash: AccountAddress, network: Option<Network>) -> String {
    ...
    if let Some(network) = network {
        format!(
            "https://explorer.naboblabs.com/account/{}?network={}",
            hash, network
        )
    } else {
        format!("https://explorer.naboblabs.com/account/{}", hash)
    }
}
...
pub fn explorer_transaction_link(
    ...
    if let Some(network) = network {
        format!(
            "https://explorer.naboblabs.com/txn/{}?network={}",
            hash.to_hex_literal(),
            network
        )
    } else {
        format!(
            "https://explorer.naboblabs.com/txn/{}",
            hash.to_hex_literal()
        )
    }
}
