...
use nabob_crypto::{ed25519::Ed25519PrivateKey, PrivateKey, ValidCryptoMaterialStringExt};
use nabob_ledger;
use nabob_rest_cli::{
    nabob_api_types::{NabobError, NabobErrorCode},
    error::{NabobErrorResponse, RestError},
};
...
///
/// Configuration will be pushed into .nabob/config.yaml
#[derive(Debug, Parser)]
pub struct InitTool {
    ...
    ///
    /// Make sure that you have your Ledger device connected and unlocked, with the Nabob app installed and opened.
...
impl CliCommand<()> for InitTool {
    ...
    async fn execute(self) -> CliTypedResult<()> {
        ...
        let mut profile_config = if let Some(profile_config) = config.remove_profile(profile_name) {
            prompt_yes_with_override(&format!("Nabob already initialized for profile {}, do you want to overwrite the existing config?", profile_name), self.prompt_options)?;
            profile_config
        } else {
        ...
        match network {
            Network::Mainnet => {
                profile_config.rest_url =
                    Some("https://fullnode.mainnet.naboblabs.com".to_string());
                profile_config.faucet_url = None;
            },
            Network::Testnet => {
                profile_config.rest_url =
                    Some("https://fullnode.testnet.naboblabs.com".to_string());
            ...
            Network::Devnet => {
                profile_config.rest_url = Some("https://fullnode.devnet.naboblabs.com".to_string());
                profile_config.faucet_url = Some("https://faucet.devnet.naboblabs.com".to_string());
            },
            Network::Local => {
            ...
            self.hardware_wallet_options.extract_derivation_path()?
        {
            ...
            let account_map = nabob_ledger::fetch_batch_accounts(Some(0..5))?;
            eprintln!(
            ...
            let path = nabob_ledger::DERIVATION_PATH.replace("{index}", input_index);

            // Validate the path
            if !nabob_ledger::validate_derivation_path(&path) {
        ...
        let public_key = if self.is_hardware_wallet() {
            let pub_key = match nabob_ledger::get_public_key(
                derivation_path
                    .ok_or_else(|| {
        ...
        let client = nabob_rest_cli::Client::new(rest_url);
        ...
        let account_exists = match client.get_account(address).await {
            Ok(_) => true,
            Err(err) => {
                if let RestError::Api(NabobErrorResponse {
                    error:
                        NabobError {
                            error_code: NabobErrorCode::ResourceNotFound,
                            ..
                        },
                    ..
                })
                | RestError::Api(NabobErrorResponse {
                    error:
                        NabobError {
                            error_code: NabobErrorCode::AccountNotFound,
                            ..
                        },
                    ..
                }) = err
        ...
        eprintln!(
            "\n---\nNabob CLI is now set up for account {} as profile {}!\n---\n",
