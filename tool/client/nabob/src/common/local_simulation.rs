...
use nabob_gas_profiling::FrameName;
use nabob_move_debugger::nabob_debugger::NabobDebugger;
use nabob_types::transaction::SignedTransaction;
use nabob_vm::{data_cache::AsMoveResolver, NabobVM};
use nabob_vm_environment::environment::NabobEnvironment;
use nabob_vm_logging::log_schema::AdapterLogSchema;
use nabob_vm_types::{
    module_and_script_storage::AsNabobCodeStorage, output::VMOutput, resolver::StateStorageView,
};
use move_core_types::vm_status::VMStatus;
use std::{path::Path, time::Instant};

pub fn run_transaction_using_debugger(
    debugger: &NabobDebugger,
    version: u64,
...
pub fn benchmark_transaction_using_debugger(
    ...
    let resolver = state_view.as_move_resolver();
    let code_storage = state_view.as_nabob_code_storage(env.clone());
    let (vm_status, vm_output) =
        vm.execute_user_transaction(&resolver, &code_storage, &transaction, &log_context);
