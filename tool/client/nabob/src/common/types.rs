...
use nabob_framework::chunked_publish::{CHUNK_SIZE_IN_BYTES, LARGE_PACKAGES_MODULE_ADDRESS};
use nabob_global_constants::adjust_gas_headroom;
use nabob_keygen::KeyGen;
use nabob_logger::Level;
use nabob_move_debugger::nabob_debugger::NabobDebugger;
use nabob_rest_cli::{
    nabob_api_types::{EntryFunctionId, HashValue, MoveType, ViewRequest},
    error::RestError,
    NabobBaseUrl, Client, Transaction,
};
use nabob_sdk::{
...
use nabob_vm_types::output::VMOutput;
...
pub const USER_AGENT: &str = concat!("nabob-cli/", env!("CARGO_PKG_VERSION"));
...
pub const NABOB_FOLDER_GIT_IGNORE: &str = indoc! {"
    *
    testnet/
    config.yaml
"};
...
const X_NABOB_CLIENT_VALUE: &str = concat!("nabob-cli/", env!("CARGO_PKG_VERSION"));
...
pub enum CliError {
    ...
    #[error("Unable to find config {0}, have you run `nabob init`?")]
...
impl From<nabob_config::config::Error> for CliError {
    fn from(e: nabob_config::config::Error) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}

impl From<nabob_github_cli::Error> for CliError {
    fn from(e: nabob_github_cli::Error) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}
...
impl From<nabob_crypto::CryptoMaterialError> for CliError {
    fn from(e: nabob_crypto::CryptoMaterialError) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}
...
impl From<nabob_ledger::NabobLedgerError> for CliError {
    fn from(e: nabob_ledger::NabobLedgerError) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}
...
/// Config saved to `.nabob/config.yaml`
...
pub const CONFIG_FOLDER: &str = ".nabob";

/// An individual profile
#[derive(Debug, Default, Serialize, Deserialize)]
pub struct ProfileConfig {
    /// Name of network being used, if setup from nabob init
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// URL for the Nabob rest endpoint
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl CliConfig {
    ...
    pub fn config_exists(mode: ConfigSearchMode) -> bool {
        if let Ok(folder) = Self::nabob_folder(mode) {
    ...
    pub fn load(mode: ConfigSearchMode) -> CliTypedResult<Self> {
        let folder = Self::nabob_folder(mode)?;
    ...
    /// Saves the config to ./.nabob/config.yaml
    pub fn save(&self) -> CliTypedResult<()> {
        let nabob_folder = Self::nabob_folder(ConfigSearchMode::CurrentDir)?;

        // Create if it doesn't exist
        let no_dir = !nabob_folder.exists();
        create_dir_if_not_exist(nabob_folder.as_path())?;

        // If the `.nabob/` doesn't exist, we'll add a .gitignore in it to ignore the config file
        // so people don't save their credentials...
        if no_dir {
            write_to_user_only_file(
                nabob_folder.join(GIT_IGNORE).as_path(),
                GIT_IGNORE,
                NABOB_FOLDER_GIT_IGNORE.as_bytes(),
            )?;
        }

        // Save over previous config file
        let config_file = nabob_folder.join(CONFIG_FILE);
        ...
        let legacy_config_file = nabob_folder.join(LEGACY_CONFIG_FILE);
    ...
    /// Finds the current directory's .nabob folder
    fn nabob_folder(mode: ConfigSearchMode) -> CliTypedResult<PathBuf> {
...
impl RestOptions {
    ...
    pub fn url(&self, profile: &ProfileOptions) -> CliTypedResult<reqwest::Url> {
        ...
        .map(|p| p.rest_url)
        {
            ...
            Err(CliError::CommandArgumentError("No rest url given.  Please add --url or add a rest_url to the .nabob/config.yaml for the current profile".to_string()))
        }
    }

    pub fn client(&self, profile: &ProfileOptions) -> CliTypedResult<Client> {
        let mut client = Client::builder(NabobBaseUrl::Custom(self.url(profile)?))
            .timeout(Duration::from_secs(self.connection_timeout_secs))
            .header(nabob_api_types::X_NABOB_CLIENT, X_NABOB_CLIENT_VALUE)?;
...
pub struct MovePackageDir {
    ...
    /// Do apply extended checks for Nabob (e.g. `#[view]` attribute) also on test code.
    /// NOTE: this behavior will become the default in the future.
    /// See <https://github.com/nabob-labs/nabob/issues/10335>
    #[clap(long, env = "NABOB_CHECK_TEST_CODE")]
    ...
    /// Note: `nabob move prove` does not support v1
...
pub fn load_account_arg(str: &str) -> Result<AccountAddress, CliError> {
            ...
            .map(|p| p.private_key)
    {
        ...
        Err(CliError::CommandArgumentError(
            "'--account' or '--profile' after using nabob init must be provided".to_string(),
        ))
    }
}
...
pub struct FaucetOptions {
    /// URL for the faucet endpoint e.g. `https://faucet.devnet.naboblabs.com`
    #[clap(long)]
        ...
        match profile.faucet_url {
            ...
            None => match profile.network {
                Some(Network::Mainnet) => {
                    Err(CliError::CommandArgumentError("There is no faucet for mainnet. Please create and fund the account by transferring funds from another account. If you are confident you want to use a faucet, set --faucet-url or add a faucet URL to .nabob/config.yaml for the current profile".to_string()))
                },
                Some(Network::Testnet) => {
                    Err(CliError::CommandArgumentError(format!("To get testnet APT you must visit {}. If you are confident you want to use a faucet programmatically, set --faucet-url or add a faucet URL to .nabob/config.yaml for the current profile", get_mint_site_url(None))))
                },
                _ => {
                    Err(CliError::CommandArgumentError("No faucet given. Please set --faucet-url or add a faucet URL to .nabob/config.yaml for the current profile".to_string()))
                },
            },
        }
    }
...
impl TransactionOptions {
        ...
        ) -> CliTypedResult<(VMStatus, VMOutput)>,
    {
        ...
        let balance = client
            .view_bob_account_balance_at_version(sender_address, version)
            .await
            .map_err(|err| CliError::ApiError(err.to_string()))?
            .into_inner();
        ...
        let debugger = NabobDebugger::rest_client(client).unwrap();
...
pub fn get_mint_site_url(address: Option<AccountAddress>) -> String {
    ...
    format!("https://nabob.dev/network/faucet{}", params)
}
