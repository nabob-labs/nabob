...
use nabob_rest_cli::VersionedNewBlockEvent;
...
impl AnalyzeValidators {
    ...
    pub fn fetch_epoch(epoch: u64, nabob_db: &dyn DbReader) -> Result<Vec<VersionedNewBlockEvent>> {
        let batch = 100;

        let mut cursor = u64::max_value();
        let mut result: Vec<VersionedNewBlockEvent> = vec![];
        let ledger_version = nabob_db.get_latest_ledger_info()?.ledger_info().version();

        loop {
            let raw_events = nabob_db.get_events(
                &new_block_event_key(),
                cursor,
                Order::Descending,
                batch as u64,
