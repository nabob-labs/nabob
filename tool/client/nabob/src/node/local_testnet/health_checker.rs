...
use nabob_protos::indexer::v1::GetTransactionsRequest;
    ...
    pub async fn check(&self) -> Result<()> {
        match self {
            ...
            HealthChecker::NodeApi(url) => {
                nabob_rest_cli::Client::new(Url::clone(url))
                    .get_index()
                    .await?;
                Ok(())
            },
            HealthChecker::DataServiceGrpc(url) => {
                let mut client = nabob_indexer_grpc_utils::create_data_service_grpc_client(
                    url.clone(),
