...
use nabob_db::backup::restore_handler::RestoreHandler;
use nabob_executor::chunk_executor::ChunkExecutor;
use nabob_executor_types::{ChunkExecutorTrait, TransactionReplayer, VerifyExecutionMode};
...
use nabob_vm::{nabob_vm::NabobVMBlockExecutor, NabobVM};
use clap::Parser;
use futures::{
    future,
    future::TryFutureExt,
...
impl TransactionRestoreBatchController {
    ...
    async fn run_impl(self) -> Result<()> {
        ...
        if let RestoreRunMode::Restore { restore_handler } = self.global_opt.run_mode.as_ref() {
            ...
            NabobVM::set_concurrency_level_once(self.global_opt.replay_concurrency_level);
    ...
    async fn save_before_replay_version(
        ...
        >,
    > {
        // get the next expected transaction version of the current nabob db from txn_info CF
    ...
    async fn replay_transactions(
        ...
        let replay_start = Instant::now();
        let db = DbReaderWriter::from_arc(Arc::clone(&restore_handler.nabobdb));
        let chunk_replayer = Arc::new(ChunkExecutor::<NabobVMBlockExecutor>::new(db));
