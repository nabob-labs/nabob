...
pub fn get_postgres_connection_string_within_docker_network(instance_id: Uuid) -> String {
    format!(
        "postgres://{}@nabob-workspace-{}-postgres:{}/{}",
...
fn create_container_options_and_config(
    ...
    let options = CreateContainerOptions {
        name: format!("nabob-workspace-{}-postgres", instance_id),
        ..Default::default()
    };
...
pub fn start_postgres(
    ...
    let volume_name = format!("nabob-workspace-{}-postgres", instance_id);
    ...
    let fut_postgres_finish = async move {
        ...
        let _wait = docker
            .wait_container(
                &format!("nabob-workspace-{}-postgres", instance_id),
