...
use super::{errors::NabobTapErrorResponse, ApiTags, NabobTapError, NabobTapErrorCode};
...
impl CaptchaApi {
    ...
    async fn request_captcha(&self) -> Result<Response<Binary<Vec<u8>>>, NabobTapErrorResponse> {
        if !self.enabled {
            return Err(NabobTapError::new(
                "The CaptchaChecker is not enabled".to_string(),
                NabobTapErrorCode::EndpointNotEnabled,
            )
            .into());
        }
        ...
        let (key, image) = match captcha_manager.create_challenge() {
            ...
            Err(e) => {
                return Err(
                    NabobTapError::new_with_error_code(e, NabobTapErrorCode::CheckerError).into(),
                );
            },
        };
