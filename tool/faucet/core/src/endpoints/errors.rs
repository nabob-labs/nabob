...
pub struct NabobTapError {
    /// A message describing the error
    pub message: String,
    /// A code describing the error for programmatic use.
    pub error_code: NabobTapErrorCode,
...
impl NabobTapError {
    pub fn new(message: String, error_code: NabobTapErrorCode) -> Self {
        if *USE_HELPFUL_ERRORS.get().unwrap_or(&true) {
            ...
            Self {
                ...
                error_code: NabobTapErrorCode::YeahNahYeahYeahYeahNahYeahNah,
                rejection_reasons: vec![],
                txn_hashes: vec![],
            }
        }
...
impl std::fmt::Display for NabobTapError {
...
impl From<NabobTapError> for NabobTapErrorResponse {
    fn from(error: NabobTapError) -> Self {
...
pub enum NabobTapErrorCode {
    ...
    /// Failed when making requests to the Nabob API.
    NabobApiError = 50,
...
impl NabobTapErrorCode {
    pub fn status(&self) -> StatusCode {
        match self {
            NabobTapErrorCode::InvalidRequest
            | NabobTapErrorCode::AccountDoesNotExist
            | NabobTapErrorCode::EndpointNotEnabled => StatusCode::BAD_REQUEST,
            NabobTapErrorCode::Rejected
            | NabobTapErrorCode::SourceIpMissing
            | NabobTapErrorCode::TransactionFailed
            | NabobTapErrorCode::AuthTokenInvalid => StatusCode::FORBIDDEN,
            NabobTapErrorCode::NabobApiError
            | NabobTapErrorCode::TransactionTimedOut
            | NabobTapErrorCode::SerializationError
            | NabobTapErrorCode::BypasserError
            | NabobTapErrorCode::CheckerError
            | NabobTapErrorCode::StorageError => StatusCode::INTERNAL_SERVER_ERROR,
            NabobTapErrorCode::ServerOverloaded | NabobTapErrorCode::FunderAccountProblem => {
                StatusCode::SERVICE_UNAVAILABLE
            },
            NabobTapErrorCode::YeahNahYeahYeahYeahNahYeahNah => StatusCode::IM_A_TEAPOT,
            // We shouldn't get here, this code is only used in error_converter.rs.
            NabobTapErrorCode::WebFrameworkError => StatusCode::INTERNAL_SERVER_ERROR,
        }
    }
}
