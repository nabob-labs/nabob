...
use super::errors::NabobTapErrorResponse;
...
/// be a JSON representation of NabobTapError. For our own errors, this is exactly
...
pub async fn convert_error(error: poem::Error) -> impl poem::IntoResponse {
    ...
    if is_framework_error {
        // Build an NabobTapErrorResponse and then reset its status code
...
fn build_error_response(error_string: String) -> Response {
    NabobTapErrorResponse::from(NabobTapError::new_with_error_code(
        error_string,
        NabobTapErrorCode::WebFrameworkError,
    ))
    .into_response()
}
