...
impl FundApi {
    ...
    ) -> poem::Result<Json<FundResponse>, NabobTapErrorResponse> {
        let txns = self
            .components
    ...
    async fn is_eligible(
        ...
        // Same thing, this uses FromRequest.
        header_map: &HeaderMap,
    ) -> poem::Result<(), NabobTapErrorResponse> {
...
impl FundApiComponents {
    ...
    async fn preprocess_request(
        ...
        header_map: &HeaderMap,
        dry_run: bool,
    ) -> poem::Result<(CheckerData, bool, Option<SemaphorePermit>), NabobTapError> {
        let permit = match &self.concurrent_requests_semaphore {
            Some(semaphore) => match semaphore.try_acquire() {
                Ok(permit) => Some(permit),
                Err(_) => {
                    return Err(NabobTapError::new(
                        "Server overloaded, please try again later".to_string(),
                        NabobTapErrorCode::ServerOverloaded,
                    ))
                },
            },
            None => None,
        ...
        for bypasser in &self.bypassers {
            if bypasser
                ...
                .await
                .map_err(|e| {
                    NabobTapError::new_with_error_code(e, NabobTapErrorCode::BypasserError)
                })?
            {
                info!(
        ...
        for checker in &self.checkers {
            rejection_reasons.extend(checker.check(checker_data.clone(), dry_run).await.map_err(
                |e| NabobTapError::new_with_error_code(e, NabobTapErrorCode::CheckerError),
            )?);
    ...
    async fn fund_inner(
        ...
        header_map: &HeaderMap,
        dry_run: bool,
    ) -> poem::Result<Vec<SignedTransaction>, NabobTapError> {
        let (checker_data, bypass, _semaphore_permit) = self
        ...
        if !bypass {
            ...
            for checker in &self.checkers {
                checker.complete(complete_data.clone()).await.map_err(|e| {
                    NabobTapError::new_with_error_code(e, NabobTapErrorCode::CheckerError)
                })?;
            }
        }
...
pub async fn mint(
    ...
    // We take the NabobTapError and convert it into an anyhow error with just the
    ...
    if return_txns.unwrap_or(false) {
        let txn_bcs =
            nabob_sdk::bcs::to_bytes(&txns).map_err(|e| poem::Error::from(anyhow::anyhow!(e)))?;
