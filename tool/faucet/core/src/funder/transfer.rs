...
use crate::{
    endpoints::{NabobTapError, NabobTapErrorCode, RejectionReason, RejectionReasonCode},
...
impl TransferFunder {
    ...
    async fn is_healthy_as_result(&self) -> Result<(), NabobTapError> {
...
impl FunderTrait for TransferFunder {
    ...
    async fn fund(
        ...
        let transactions = if check_only {
            ...
            let txn = self
                .execute_transaction(
                    &client,
                    nabob_stdlib::nabob_account_transfer(receiver_address, amount),
                    &receiver_address,
                )
                .await?;
            info!(
    ...
    async fn is_healthy(&self) -> FunderHealthMessage {
        ...
        let funder_balance = match self
            .get_api_client()
            .view_bob_account_balance(account_address)
            .await
        {
