...
use crate::endpoints::{NabobTapError, NabobTapErrorCode};
use anyhow::{Context, Result};
use nabob_logger::info;
use nabob_sdk::{
    crypto::ed25519::Ed25519PublicKey,
    rest_client::Client,
    transaction_builder::{nabob_stdlib, TransactionFactory},
    types::{
...
static MINTER_SCRIPT: &[u8] = include_bytes!(
    "../../../../../mos/move-examples/scripts/minter/build/Minter/bytecode_scripts/main.mv"
);
...
impl MintFunder {
    ...
    async fn get_gas_unit_price(&self) -> Result<u64, NabobTapError> {
        match self.txn_config.gas_unit_price_override {
            ...
            None => self
                ...
                .await
                .map_err(|e| {
                    NabobTapError::new_with_error_code(e, NabobTapErrorCode::NabobApiError)
                }),
        }
    }

    async fn get_transaction_factory(&self) -> Result<TransactionFactory, NabobTapError> {
        Ok(self
            .transaction_factory
            .clone()
    ...
    pub async fn use_delegated_account(&mut self) -> Result<()> {
        ...
        // Delegate minting to the account
        {
            ...
            client
                .submit_and_wait(&faucet_account.sign_with_transaction_builder(
                    transaction_factory.payload(nabob_stdlib::nabob_coin_delegate_mint_capability(
        ...
        client
            .submit_and_wait(&delegated_account.sign_with_transaction_builder(
                transaction_factory.payload(nabob_stdlib::nabob_coin_claim_mint_capability()),
            ))
            .await
    ...
    ) -> Result<Vec<SignedTransaction>, NabobTapError> {
