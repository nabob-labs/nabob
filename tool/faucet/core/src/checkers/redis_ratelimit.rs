...
impl RatelimitKeyProvider {
    ...
    /// JWT's `sub` field).
    pub async fn ratelimit_key_value(&self, data: &CheckerData) -> Result<String, NabobTapError> {
        match self {
...
/// worry about it as a followup: https://github.com/nabob-labs/nabob-tap/issues/15.
...
impl RedisRatelimitChecker {
    ...
    pub async fn get_redis_connection(&self) -> Result<Connection, NabobTapError> {
        self.db_pool.get().await.map_err(|e| {
            NabobTapError::new_with_error_code(
                format!("Failed to connect to redis storage: {}", e),
                NabobTapErrorCode::StorageError,
            )
        })
    }
...
impl CheckerTrait for RedisRatelimitChecker {
    async fn check(
        ...
        data: CheckerData,
        dry_run: bool,
    ) -> Result<Vec<RejectionReason>, NabobTapError> {
        let mut conn = self
            .get_redis_connection()
            .await
            .map_err(|e| NabobTapError::new_with_error_code(e, NabobTapErrorCode::StorageError))?;
    ...
    async fn complete(&self, data: CompleteData) -> Result<(), NabobTapError> {
