...
use crate::{
    ...
    endpoints::{NabobTapError, RejectionReason},
};
use anyhow::Result;
use nabob_sdk::types::account_address::AccountAddress;
...
pub trait CheckerTrait: Sync + Send + 'static {
    ...
    /// dry_run mode.
    async fn complete(&self, _data: CompleteData) -> Result<(), NabobTapError> {
        Ok(())
    }
