...
use nabob_config::keys::ConfigKey;
use nabob_faucet_metrics_svc::{run_metrics_server, MetricsServerConfig};
use nabob_logger::info;
use nabob_sdk::{
    crypto::ed25519::Ed25519PrivateKey,
    types::{account_config::nabob_test_root_address, chain_id::ChainId},
};
use clap::Parser;
    ...
    pub fn build_for_cli(
        ...
        Self {
            ...
            funder_config: FunderConfig::MintFunder(MintFunderConfig {
                ...
                mint_account_address: Some(nabob_test_root_address()),
                do_not_delegate,
            }),
            handler_config: HandlerConfig {
...
mod test {
    ...
    use crate::{
        endpoints::{
            NabobTapError, NabobTapErrorCode, FundRequest, FundResponse, RejectionReasonCode,
        },
    ...
    async fn start_server(config_content: &'static str) -> Result<(u16, JoinHandle<Result<()>>)> {
        ...
        let port = nabob_config::utils::get_available_port();
    ...
    async fn test_checkers() -> Result<()> {
        ...
        let response = reqwest::Client::new()
            ...
            .send()
            .await?;
        let nabob_error = NabobTapError::parse_from_json_string(&response.text().await?)
            .expect("Failed to read response as NabobError");
        assert!(!nabob_error.rejection_reasons.is_empty());
        ...
        let nabob_error = NabobTapError::parse_from_json_string(&response.text().await?)
            .expect("Failed to read response as NabobError");
        let rejection_reason_codes: HashSet<RejectionReasonCode> = nabob_error
            .rejection_reasons
            .into_iter()
            .map(|r| r.get_code())
            .collect();
    ...
    async fn test_redis_ratelimiter() -> Result<()> {
        ...
        let nabob_node_api_client = nabob_sdk::rest_client::Client::new(
            reqwest::Url::from_str("http://127.0.0.1:8080").unwrap(),
        );
        nabob_node_api_client
            .get_index_bcs()
            .await
    ...
    async fn test_transfer_health() -> Result<()> {
        ...
        let serialized_keys = nabob_sdk::bcs::to_bytes(&private_key)?;
        ...
        unwrap_reqwest_result(
            reqwest::Client::new()
                .post("https://faucet.devnet.naboblabs.com/fund")
                .body(
                    FundRequest {
        ...
        let nabob_error = NabobTapError::parse_from_json_string(&response.text().await?)
            .expect("Failed to read response as NabobError");
        assert_eq!(
            nabob_error.error_code,
            NabobTapErrorCode::FunderAccountProblem
        );
    ...
    async fn test_mint_funder() -> Result<()> {
        ...
        let response = nabob_node_api_client
