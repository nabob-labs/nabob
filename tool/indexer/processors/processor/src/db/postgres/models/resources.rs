...
use crate::db::{
    common::models::{
        ...
        token_v2_models::v2_token_utils::{
            NabobCollection, Collection, ConcurrentSupply, FixedSupply, PropertyMapModel,
...
use anyhow::Result;
use nabob_protos::transaction::v1::WriteResource;
...
pub const TYPE_NABOB_COLLECTION: &str = formatcp!("{TOKEN_V2_ADDR}::nabob_token::NabobCollection");
...
pub enum V2TokenResource {
    NabobCollection(NabobCollection),
...
impl Resource for NabobCollection {
    fn type_str() -> &'static str {
        TYPE_NABOB_COLLECTION
    }
}
...
impl V2TokenResource {
    pub fn from_write_resource(write_resource: &WriteResource) -> Result<Option<Self>> {
        let type_str = MoveResource::get_outer_type_from_write_resource(write_resource);
        Ok(Some(match type_str.as_str() {
            TYPE_NABOB_COLLECTION => Self::NabobCollection(write_resource.try_into()?),
