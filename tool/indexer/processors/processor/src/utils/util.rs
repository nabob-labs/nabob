...
lazy_static! {
    pub static ref BOB_METADATA_ADDRESS_RAW: [u8; 32] = {
        let mut addr = [0u8; 32];
        addr[31] = 10u8;
        addr
    };
    pub static ref BOB_METADATA_ADDRESS_HEX: String =
        format!("0x{}", hex::encode(*BOB_METADATA_ADDRESS_RAW));
}
...
mod tests {
    ...
    fn test_deserialize_string_from_bcs() {
        ...
        assert_eq!(d.module_name.as_str(), "nabob_coin");
        assert_eq!(d.struct_name.as_str(), "NabobCoin");
    }
