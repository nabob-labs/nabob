...
use nabob_config::config::DEFAULT_MAX_NUM_NODES_PER_LRU_CACHE_SHARD;
use nabob_consensus_types::block::Block;
use nabob_db::NabobDB;
use nabob_executor::block_executor::BlockExecutor;
...
use nabob_types::{
    account_config::{nabob_test_root_address, AccountResource, CoinStoreResource},
...
pub fn test_execution_with_storage_impl() -> Arc<NabobDB> {
    let path = nabob_temppath::TempPath::new();
...
pub fn test_execution_with_storage_impl_inner(
    ...
    let (genesis, validators) = nabob_vm_genesis::test_genesis_change_set_and_validators(Some(1));
    let genesis_txn = Transaction::GenesisTransaction(WriteSetPayload::Direct(genesis));

    let core_resources_account: LocalAccount = LocalAccount::new(
        nabob_test_root_address(),
        AccountKey::from_private_key(nabob_vm_genesis::GENESIS_KEYPAIR.0.clone()),
        0,
    );

    let (nabob_db, db, executor, waypoint) =
        create_db_and_executor(db_path, &genesis_txn, force_sharding);

    let parent_block_id = executor.committed_block_id();
    let signer = nabob_types::validator_signer::ValidatorSigner::new(
    ...
    let reconfig1 = core_resources_account.sign_with_transaction_builder(
        txn_factory.payload(nabob_stdlib::nabob_governance_force_end_epoch_test_only()),
    );
...
pub fn create_db_and_executor<P: AsRef<std::path::Path>>(
    ...
    Arc<NabobDB>,
    DbReaderWriter,
    BlockExecutor<NabobVMBlockExecutor>,
    Waypoint,
) {
    let (db, dbrw) = force_sharding
        .then(|| {
            DbReaderWriter::wrap(NabobDB::new_for_test_with_sharding(
                &path,
                DEFAULT_MAX_NUM_NODES_PER_LRU_CACHE_SHARD,
            ))
        })
        .unwrap_or_else(|| DbReaderWriter::wrap(NabobDB::new_for_test(&path)));
    let waypoint = bootstrap_genesis::<NabobVMBlockExecutor>(&dbrw, genesis).unwrap();
...
pub fn get_account_balance(state_view: &dyn StateView, address: &AccountAddress) -> u64 {
    CoinStoreResource::<NabobCoinType>::fetch_move_resource(state_view, address)
        .unwrap()
