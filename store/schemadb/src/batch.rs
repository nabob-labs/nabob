...
use crate::{
    metrics::{NABOB_SCHEMADB_DELETES_SAMPLED, NABOB_SCHEMADB_PUT_BYTES_SAMPLED, TIMER},
    schema::{KeyCodec, Schema, ValueCodec},
    ColumnFamilyName, DB,
};
use nabob_drop_helper::DropHelper;
use nabob_metrics_core::TimerHelper;
use nabob_storage_interface::Result as DbResult;
use std::{
    collections::HashMap,
    fmt::{Debug, Formatter},
};
...
impl BatchStats {
    ...
    fn commit(&self) {
        ...
        for (cf_name, num_deletes) in &self.num_deletes {
            NABOB_SCHEMADB_DELETES_SAMPLED
                .with_label_values(&[cf_name])
