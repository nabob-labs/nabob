...
use crate::{
    metrics::{
        NABOB_SCHEMADB_BATCH_COMMIT_BYTES, NABOB_SCHEMADB_BATCH_COMMIT_LATENCY_SECONDS,
        NABOB_SCHEMADB_GET_BYTES, NABOB_SCHEMADB_GET_LATENCY_SECONDS, NABOB_SCHEMADB_ITER_BYTES,
        NABOB_SCHEMADB_ITER_LATENCY_SECONDS, NABOB_SCHEMADB_SEEK_LATENCY_SECONDS,
    },
...
use nabob_storage_interface::{NabobDbError, Result as DbResult};
...
impl DB {
    ...
    pub fn get<S: Schema>(&self, schema_key: &S::Key) -> DbResult<Option<S::Value>> {
        let _timer = NABOB_SCHEMADB_GET_LATENCY_SECONDS
    ...
    pub fn write_schemas(&self, batch: impl IntoRawBatch) -> DbResult<()> {
        let _timer = NABOB_SCHEMADB_BATCH_COMMIT_LATENCY_SECONDS.timer_with(&[&self.name]);
    ...
    pub fn get_property(&self, cf_name: &str, property_name: &str) -> DbResult<u64> {
        self.inner
            ...
            .into_db_res()?
            .ok_or_else(|| {
                nabob_storage_interface::NabobDbError::Other(
                    format!(
...
fn to_db_err(rocksdb_err: rocksdb::Error) -> NabobDbError {
    match rocksdb_err.kind() {
        ErrorKind::Incomplete => NabobDbError::RocksDbIncompleteResult(rocksdb_err.to_string()),
        ErrorKind::NotFound
        ...
        | ErrorKind::Unknown => NabobDbError::OtherRocksDbError(rocksdb_err.to_string()),
    }
}
