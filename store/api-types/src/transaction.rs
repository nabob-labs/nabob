...
use crate::{
    Address, NabobError, EntryFunctionId, EventGuid, HashValue, HexEncodedBytes,
    ...
    pub fn from_transaction_onchain_data(
        ...
        if txn.version > latest_ledger_version {
            match txn.transaction {
                nabob_types::transaction::Transaction::UserTransaction(txn) => {
...
pub struct TransactionOnChainData {
    ...
    pub transaction: nabob_types::transaction::Transaction,
    /// Information about the transaction
    pub info: nabob_types::transaction::TransactionInfo,
    /// Events emitted by the transaction
    pub events: Vec<ContractEvent>,
    /// The accumulator root hash at this version
    pub accumulator_root_hash: nabob_crypto::HashValue,
    /// Final state of resources changed by the transaction
    pub changes: nabob_types::write_set::WriteSet,
}

impl From<(TransactionWithProof, nabob_crypto::HashValue)> for TransactionOnChainData {
    fn from((txn, accumulator_root_hash): (TransactionWithProof, nabob_crypto::HashValue)) -> Self {
        Self {
            version: txn.version,
...
impl
    From<(
        u64,
        nabob_types::transaction::Transaction,
        nabob_types::transaction::TransactionInfo,
        Vec<ContractEvent>,
        nabob_crypto::HashValue,
        nabob_types::write_set::WriteSet,
    )> for TransactionOnChainData
{
    fn from(
...
/// Enum of the different types of transactions in Nabob
...
pub struct BlockMetadataTransaction {
    ...
    /// If some, it means the internal txn type is `nabob_types::transaction::Transaction::BlockMetadataExt`.
    /// Otherwise, it is `nabob_types::transaction::Transaction::BlockMetadata`.
    ///
    /// NOTE: we could have introduced a new BOB txn type to represent the corresponding internal type,
...
impl
    From<(
        nabob_types::validator_txn::ValidatorTransaction,
        TransactionInfo,
        Vec<Event>,
        u64,
    )> for ValidatorTransaction
{
    fn from(
        ...
        match txn {
            nabob_types::validator_txn::ValidatorTransaction::DKGResult(dkg_transcript) => {
            ...
            nabob_types::validator_txn::ValidatorTransaction::ObservedJWKUpdate(
...
/// A more API-friendly representation of the on-chain `nabob_types::jwks::QuorumCertifiedUpdate`.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Object)]
...
/// A more API-friendly representation of the on-chain `nabob_types::aggregate_signature::AggregateSignature`.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Object)]
...
/// A more API-friendly representation of the on-chain `nabob_types::jwks::ProviderJWKs`.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Object)]
...
impl TryFrom<MultiEd25519Signature> for TransactionAuthenticator {
    ...
    fn try_from(value: MultiEd25519Signature) -> Result<Self, Self::Error> {
        ...
        Ok(TransactionAuthenticator::multi_ed25519(
            ...
            nabob_crypto::multi_ed25519::MultiEd25519Signature::new_with_signatures_and_bitmap(
                ed25519_signatures,
