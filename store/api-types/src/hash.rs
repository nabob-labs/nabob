...
pub struct HashValue(pub nabob_crypto::hash::HashValue);

impl From<nabob_crypto::hash::HashValue> for HashValue {
    fn from(val: nabob_crypto::hash::HashValue) -> Self {
        Self(val)
    }
}

impl From<HashValue> for nabob_crypto::hash::HashValue {
...
impl FromStr for HashValue {
    ...
    fn from_str(s: &str) -> anyhow::Result<Self, anyhow::Error> {
        if let Some(hex) = s.strip_prefix("0x") {
            Ok(hex.parse::<nabob_crypto::hash::HashValue>()?.into())
        } else {
            Ok(s.parse::<nabob_crypto::hash::HashValue>()?.into())
        }
    }
}
...
impl HashValue {
    pub fn zero() -> Self {
        Self(nabob_crypto::hash::HashValue::zero())
    }
}
