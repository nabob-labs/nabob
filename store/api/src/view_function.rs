...
use nabob_api_types::{
    NabobErrorCode, AsConverter, MoveValue, ViewFunction, ViewRequest, MAX_RECURSIVE_TYPES_ALLOWED,
    U64,
};
use nabob_bcs_utils::serialize_uleb128;
...
pub enum ViewFunctionRequest {
    ...
    #[oai(content_type = "application/x.nabob.view_function+bcs")]
    Bcs(Bcs),
}
...
impl ViewFunctionApi {
    ...
    ///
    /// The Nabob nodes prune account state history, via a configurable time window.
...
fn view_request(
    ...
    let values = output.values.map_err(|err| {
        BasicErrorWith404::bad_request_with_code_no_info(err, NabobErrorCode::InvalidInput)
    })?;
