...
async fn test_account_abstraction_single_signer() {
    ...
    let txn = futures::executor::block_on(async move {
        let path = PathBuf::from(std::env!("CARGO_MANIFEST_DIR"))
            .join("../mos/move-examples/account_abstraction/bls12381_single_key");
...
async fn test_account_abstraction_multi_agent_with_abstracted_sender() {
    ...
    let factory = context.transaction_factory();
    let balance_start = context.get_bob_balance(d.address()).await;
    ...
    assert_eq!(
        balance_start + 3,
        context.get_bob_balance(d.address()).await
    );
}
