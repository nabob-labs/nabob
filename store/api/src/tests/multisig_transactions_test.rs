...
use nabob_api_test_context::{current_function_name, TestContext};
use nabob_types::{
...
async fn test_multisig_transaction_with_payload_succeeds() {
    ...
    let multisig_account = context
        ...
        .await;
    assert_eq!(1000, context.get_bob_balance(multisig_account).await);
    ...
    // The multisig tx that transfers away 1000 BOB should have succeeded.
    assert_eq!(0, context.get_bob_balance(multisig_account).await);
}
...
async fn test_multisig_transaction_with_existing_account() {
    ...
    let org_multisig_balance = context.get_bob_balance(multisig_account.address()).await;
    let org_owner_1_balance = context.get_bob_balance(owner_account_1.address()).await;
    ...
    assert_eq!(
        ...
        context.get_bob_balance(multisig_account.address()).await
    );
    assert_eq!(
        org_owner_1_balance + 1000,
        context.get_bob_balance(owner_account_1.address()).await
    );
}
...
async fn test_multisig_transaction_with_payload_and_failing_execution() {
    ...
    context
        ...
        .await;
    // Target transaction execution should fail because the multisig account only has 1000 BOB but
...
async fn test_simulate_multisig_transaction_should_charge_gas_against_sender() {
    ...
    let multisig_account = context
        ...
        .await;
    assert_eq!(10, context.get_bob_balance(multisig_account).await);
...
fn construct_multisig_txn_transfer_payload(recipient: AccountAddress, amount: u64) -> Vec<u8> {
    bcs::to_bytes(&MultisigTransactionPayload::EntryFunction(
        EntryFunction::new(
            ModuleId::new(CORE_CODE_ADDRESS, ident_str!("nabob_account").to_owned()),
