...
use nabob_api_test_context::current_function_name;
use nabob_crypto::{ed25519::Ed25519PrivateKey, secp256k1_ecdsa, SigningKey};
use nabob_sdk::types::{
    transaction::{
        authenticator::{
...
async fn test_multi_secp256k1_ecdsa() {
    ...
    let private_key: secp256k1_ecdsa::PrivateKey = nabob_crypto::Uniform::generate(&mut rng);
    let public_key = nabob_crypto::PrivateKey::public_key(&private_key);
    ...
    let balance_start = context.get_bob_balance(other.address()).await;
    ...
    assert_eq!(
        balance_start + 5,
        context.get_bob_balance(other.address()).await
    );
