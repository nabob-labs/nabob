...
use nabob_api_types::{
    verify_function_identifier, verify_module_identifier, Address, NabobError, NabobErrorCode,
...
use nabob_crypto::{hash::CryptoHash, signing_message};
use nabob_types::{
    ...
    NabobCoinType, CoinType,
};
use nabob_vm::{NabobSimulationVM, NabobVM};
...
pub enum SubmitTransactionPost {
    ...
    // TODO: https://github.com/nabob-labs/nabob/issues/2275
    #[oai(content_type = "application/x.nabob.signed_transaction+bcs")]
    Bcs(Bcs),
}
...
impl TransactionsApi {
    ...
    ///   1. Hash message bytes: "RawTransaction" bytes + BCS bytes of [Transaction](https://nabob-labs.github.io/nabob/nabob_types/transaction/enum.Transaction.html).
    ...
    /// Make sure to use the `application/x.nabob.signed_transaction+bcs` Content-Type.
    ...
    async fn encode_submission(
        ...
        data.0
            ...
            .map_err(|err| {
                BasicError::bad_request_with_code_no_info(err, NabobErrorCode::InvalidInput)
            })?;
...
impl TransactionsApi {
    ...
    async fn create_internal(&self, txn: SignedTransaction) -> Result<(), NabobError> {
        ...
        match mempool_status.code {
            ...
            MempoolStatusCode::InvalidSeqNumber => Err(NabobError::new_with_error_code(
                mempool_status.message,
                NabobErrorCode::SequenceNumberTooOld,
            )),
            MempoolStatusCode::InvalidUpdate => Err(NabobError::new_with_error_code(
                mempool_status.message,
                NabobErrorCode::InvalidTransactionUpdate,
            )),
            MempoolStatusCode::UnknownStatus => Err(NabobError::new_with_error_code(
    ...
    async fn create(
        ...
        match self.create_internal(txn.clone()).await {
            ...
            Err(error) => match error.error_code {
                NabobErrorCode::InternalError => Err(
                    SubmitTransactionError::internal_from_nabob_error(error, ledger_info),
                ),
                NabobErrorCode::VmError
                | NabobErrorCode::SequenceNumberTooOld
                | NabobErrorCode::InvalidTransactionUpdate => Err(
                    SubmitTransactionError::bad_request_from_nabob_error(error, ledger_info),
                ),
                NabobErrorCode::MempoolIsFull => Err(
                    SubmitTransactionError::insufficient_storage_from_nabob_error(
                        error,
                        ledger_info,
                    ),
                ),
                _ => Err(SubmitTransactionError::internal_from_nabob_error(
                    error,
                    ledger_info,
                )),
            },
    ...
    // TODO: This function leverages a lot of types from nabob_types, use the
    ...
    pub fn simulate(
        ...
        let (vm_status, output) =
            NabobSimulationVM::create_vm_and_simulate_signed_transaction(&txn, &state_view);
        ...
        let txn = nabob_types::transaction::Transaction::UserTransaction(txn);
        let zero_hash = nabob_crypto::HashValue::zero();
        let info = nabob_types::transaction::TransactionInfo::new(
            txn.hash(),
            zero_hash,
            zero_hash,
            None,
    ...
    pub fn get_signing_message(
        ...
        if accept_type == &AcceptType::Bcs {
            return Err(BasicError::bad_request_with_code_no_info(
                ...
                NabobErrorCode::BcsNotSupported,
            ));
        }
        ...
        let raw_txn: RawTransaction = state_view
            ...
            .map_err(|err| {
                BasicError::bad_request_with_code(err, NabobErrorCode::InvalidInput, &ledger_info)
            })?;
