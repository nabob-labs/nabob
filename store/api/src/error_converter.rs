...
use nabob_api_types::{mime_types::JSON, NabobError, NabobErrorCode};
use poem::{
...
/// be a JSON representation of NabobError. For our own code, this is exactly
...
fn build_error_response(error_string: String) -> Response {
    Json(NabobError::new_with_error_code(
        error_string,
        NabobErrorCode::WebFrameworkError,
    ))
    .into_response()
}
