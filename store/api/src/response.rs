...
//! The Nabob API response / error handling philosophy.
//!
...
// TODO: https://github.com/nabob-labs/nabob/issues/2279

use super::{accept_type::AcceptType, bcs_payload::Bcs};
use nabob_api_types::{Address, NabobError, NabobErrorCode, HashValue, LedgerInfo};
use move_core_types::{
...
pub enum NabobResponseContent<T: ToJSON + Send + Sync> {
...
/// an error response to manipulate the NabobError inside it.
pub trait NabobErrorResponse {
    fn inner_mut(&mut self) -> &mut NabobError;
}
...
/// implementor also implements NabobErrorResponse, which saves functions from
...
macro_rules! generate_error_traits {
    ($($trait_name:ident),*) => {
        paste::paste! {
        $(
        pub trait [<$trait_name Error>]: NabobErrorResponse {
            // With ledger info and an error code
            #[allow(unused)]
            fn [<$trait_name:snake _with_code>]<Err: std::fmt::Display>(
                err: Err,
                error_code: nabob_api_types::NabobErrorCode,
                ledger_info: &nabob_api_types::LedgerInfo,
            ...
            fn [<$trait_name:snake _with_vm_status>]<Err: std::fmt::Display>(
                err: Err,
                error_code: nabob_api_types::NabobErrorCode,
                vm_status: nabob_types::vm_status::StatusCode,
                ledger_info: &nabob_api_types::LedgerInfo
            ) -> Self where Self: Sized;

            #[allow(unused)]
            fn [<$trait_name:snake _from_nabob_error>](
                nabob_error: nabob_api_types::NabobError,
                ledger_info: &nabob_api_types::LedgerInfo
            ) -> Self where Self: Sized;
        }
        )*
...
macro_rules! generate_error_response {
    ($enum_name:ident, $(($status:literal, $name:ident)),*) => {
        ...
        pub enum $enum_name {
            $(
            #[oai(status = $status)]
            $name(poem_openapi::payload::Json<Box<nabob_api_types::NabobError>>,
                // We use just regular u64 here instead of U64 since all header
                // values are implicitly strings anyway.
                /// Chain ID of the current chain
                #[oai(header = "X-Nabob-Chain-Id")] Option<u8>,
                /// Current ledger version of the chain
                #[oai(header = "X-Nabob-Ledger-Version")] Option<u64>,
                /// Oldest non-pruned ledger version of the chain
                #[oai(header = "X-Nabob-Ledger-Oldest-Version")] Option<u64>,
                /// Current timestamp of the chain
                #[oai(header = "X-Nabob-Ledger-TimestampUsec")] Option<u64>,
                /// Current epoch of the chain
                #[oai(header = "X-Nabob-Epoch")] Option<u64>,
                /// Current block height of the chain
                #[oai(header = "X-Nabob-Block-Height")] Option<u64>,
                /// Oldest non-pruned block height of the chain
                #[oai(header = "X-Nabob-Oldest-Block-Height")] Option<u64>,
                /// The cost of the call in terms of gas
                #[oai(header = "X-Nabob-Gas-Used")] Option<u64>,
            ),
            )*
        }
        ...
        impl $crate::response::[<$name Error>] for $enum_name {
            fn [<$name:snake _with_code>]<Err: std::fmt::Display>(
                ...
                let error = nabob_api_types::NabobError::new_with_error_code(err, error_code);
            ...
            fn [<$name:snake _with_vm_status>]<Err: std::fmt::Display>(
                ...
                let error = nabob_api_types::NabobError::new_with_vm_status(err, error_code, vm_status);
                let payload = poem_openapi::payload::Json(Box::new(error));
                Self::from($enum_name::$name(
                    payload,
                    Some(ledger_info.chain_id),
            ...
            fn [<$name:snake _from_nabob_error>](
                nabob_error: nabob_api_types::NabobError,
                ledger_info: &nabob_api_types::LedgerInfo
            ) -> Self where Self: Sized {
                let payload = poem_openapi::payload::Json(Box::new(nabob_error));
                Self::from($enum_name::$name(
                    payload,
                    Some(ledger_info.chain_id),
                    Some(ledger_info.ledger_version.into()),
        ...
        // Generate a function that helps get the NabobError within.
        impl $crate::response::NabobErrorResponse for $enum_name {
            fn inner_mut(&mut self) -> &mut nabob_api_types::NabobError {
                match self {
                    $(
    ...
    ($enum_name:ident, $(($status:literal, $name:ident)),*) => {
        ...
        pub enum $enum_name<T: poem_openapi::types::ToJSON + Send + Sync> {
            ...
            $name(
                // We use just regular u64 here instead of U64 since all header
                // values are implicitly strings anyway.
                $crate::response::NabobResponseContent<T>,
                /// Chain ID of the current chain
                #[oai(header = "X-Nabob-Chain-Id")] u8,
                /// Current ledger version of the chain
                #[oai(header = "X-Nabob-Ledger-Version")] u64,
                /// Oldest non-pruned ledger version of the chain
                #[oai(header = "X-Nabob-Ledger-Oldest-Version")] u64,
                /// Current timestamp of the chain
                #[oai(header = "X-Nabob-Ledger-TimestampUsec")] u64,
                /// Current epoch of the chain
                #[oai(header = "X-Nabob-Epoch")] u64,
                /// Current block height of the chain
                #[oai(header = "X-Nabob-Block-Height")] u64,
                /// Oldest non-pruned block height of the chain
                #[oai(header = "X-Nabob-Oldest-Block-Height")] u64,
                /// The cost of the call in terms of gas
                #[oai(header = "X-Nabob-Gas-Used")] Option<u64>,
                ...
                #[oai(header = "X-Nabob-Cursor")] Option<String>,
            ),
            )*
        }
        ...
        // Generate a From impl that builds a response from NabobResponseContent.
        ...
        impl <T: poem_openapi::types::ToJSON + Send + Sync> From<($crate::response::NabobResponseContent<T>, &nabob_api_types::LedgerInfo, [<$enum_name Status>])>
            for $enum_name<T>
        {
            fn from(
                (value, ledger_info, status): (
                    $crate::response::NabobResponseContent<T>,
                    &nabob_api_types::LedgerInfo,
                    [<$enum_name Status>]
                ),
            ) -> Self {
                match status {
        ...
        impl<T: poem_openapi::types::ToJSON + Send + Sync> From<(poem_openapi::payload::Json<T>, &nabob_api_types::LedgerInfo, [<$enum_name Status>])>
            for $enum_name<T>
        {
            fn from(
                (value, ledger_info, status): (poem_openapi::payload::Json<T>, &nabob_api_types::LedgerInfo, [<$enum_name Status>]),
            ) -> Self {
                let content = $crate::response::NabobResponseContent::Json(value);
                Self::from((content, ledger_info, status))
            }
        }

        // Generate a From impl that builds a response from a Bcs<Vec<u8>> and friends.
        impl<T: poem_openapi::types::ToJSON + Send + Sync> From<($crate::bcs_payload::Bcs, &nabob_api_types::LedgerInfo, [<$enum_name Status>])>
            for $enum_name<T>
        {
            fn from(
                (value, ledger_info, status): (
                ...
                let content = $crate::response::NabobResponseContent::Bcs(value);
                Self::from((content, ledger_info, status))
            }
        }
        ...
        impl<T: poem_openapi::types::ToJSON + Send + Sync + serde::Serialize> $enum_name<T> {
                        ...
                        $crate::bcs_payload::Bcs(
                            bcs::to_bytes(&value)
                                .map_err(|e| E::internal_with_code(
                                    e,
                                    nabob_api_types::NabobErrorCode::InternalError,
                                    ledger_info
                                ))?
                        ),
                        ledger_info,
           ...
           pub fn try_from_json<E: $crate::response::InternalError>(
            ...
            pub fn with_cursor(mut self, new_cursor: Option<nabob_types::state_store::state_key::StateKey>) -> Self {
                match self {
                    $(
                    [<$enum_name>]::$name(_, _, _, _, _, _, _, _, _, ref mut cursor) => {
                        *cursor = new_cursor.map(|c| nabob_api_types::StateKeyWrapper::from(c).to_string());
                    }
                    )*
                }
                self
...
pub fn version_not_found<E: NotFoundError>(ledger_version: u64, ledger_info: &LedgerInfo) -> E {
    build_not_found(
        ...
        NabobErrorCode::VersionNotFound,
        ledger_info,
    )
}
...
pub fn transaction_not_found_by_version<E: NotFoundError>(
    ...
    build_not_found(
        "Transaction",
        format!("Ledger version({})", ledger_version),
        NabobErrorCode::TransactionNotFound,
        ledger_info,
    )
}
...
pub fn account_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        NabobErrorCode::AccountNotFound,
        ledger_info,
    )
}
...
pub fn resource_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        NabobErrorCode::ResourceNotFound,
        ledger_info,
    )
}
...
pub fn struct_field_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        NabobErrorCode::StructFieldNotFound,
        ledger_info,
    )
}
...
pub fn table_item_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        NabobErrorCode::TableItemNotFound,
        ledger_info,
    )
}
