...
//! nabob_channel provides an mpsc channel which has two ends `nabob_channel::Receiver`
//! and `nabob_channel::Sender` similar to existing mpsc data structures.
...
use nabob_infallible::{Mutex, NonZeroUsize};
use nabob_metrics_core::IntCounterVec;
use futures::{
    channel::oneshot,
...
/// The sending end of the nabob_channel.
#[derive(Debug)]
pub struct Sender<K: Eq + Hash + Clone, M> {
    shared_state: Arc<Mutex<SharedState<K, M>>>,
}

/// The status of an element inserted into a nabob_channel. If the element is successfully
...
/// The receiving end of the nabob_channel.
#[derive(Debug)]
...
/// Configuration for a new nabob_channel queue.
#[derive(Clone, Copy)]
pub struct Config {
...
impl Config {
    /// The nabob_channel has a "sub-queue" per key. The `max_capacity` controls
...
pub fn new<K: Eq + Hash + Clone, M>(
    ...
    let max_queue_size_per_key =
        NonZeroUsize!(max_queue_size_per_key, "nabob_channel cannot be of size 0");
