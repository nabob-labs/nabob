...
use nabob_crypto::{x25519::PrivateKey, Uniform};
use nabob_event_notify::{ReconfigNotification, ReconfigNotificationListener};
use nabob_infallible::{Mutex, MutexGuard, RwLock};
...
pub struct NodeNetworkInterface {
    ...
    pub(crate) network_reqs_rx: nabob_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
    /// Peer notification sender for sending outgoing messages to other peers
    pub(crate) network_notifs_tx: nabob_channel::Sender<(PeerId, ProtocolId), ReceivedMessage>,
}
...
fn setup_node_network_interface(
    ...
    let (connection_reqs_tx, _) = nabob_channel::new(QueueStyle::FIFO, MAX_QUEUE_SIZE, None);
...
fn start_node_mempool(
    ...
    let runtime = nabob_runtimes::spawn_named_runtime("shared-mem".into(), None);
