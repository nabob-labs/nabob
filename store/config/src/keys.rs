...
impl<T: DeserializeOwned + PrivateKey + ValidCryptoMaterial + Serialize> ConfigKey<T> {
    ...
    pub fn public_key(&self) -> T::PublicKeyMaterial {
        nabob_crypto::PrivateKey::public_key(&self.key)
    }
...
#[cfg(test)]
impl<T: PrivateKey + Serialize + nabob_crypto::Uniform> Default for ConfigKey<T> {
    fn default() -> Self {
        Self {
            key: nabob_crypto::Uniform::generate_for_testing(),
        }
    }
}
