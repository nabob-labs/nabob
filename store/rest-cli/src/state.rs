...
use nabob_api_types::{
    X_NABOB_BLOCK_HEIGHT, X_NABOB_CHAIN_ID, X_NABOB_CURSOR, X_NABOB_EPOCH,
    X_NABOB_LEDGER_OLDEST_VERSION, X_NABOB_LEDGER_TIMESTAMP, X_NABOB_LEDGER_VERSION,
    X_NABOB_OLDEST_BLOCK_HEIGHT,
};
...
impl State {
    pub fn from_headers(headers: &reqwest::header::HeaderMap) -> anyhow::Result<Self> {
        let maybe_chain_id = headers
            .get(X_NABOB_CHAIN_ID)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let maybe_version = headers
            .get(X_NABOB_LEDGER_VERSION)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let maybe_timestamp = headers
            .get(X_NABOB_LEDGER_TIMESTAMP)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        ...
        let maybe_oldest_ledger_version = headers
            .get(X_NABOB_LEDGER_OLDEST_VERSION)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let maybe_block_height = headers
            .get(X_NABOB_BLOCK_HEIGHT)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let maybe_oldest_block_height = headers
            .get(X_NABOB_OLDEST_BLOCK_HEIGHT)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let cursor = headers
            .get(X_NABOB_CURSOR)
