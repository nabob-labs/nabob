...
pub use crate::client_builder::{NabobBaseUrl, ClientBuilder};
use crate::{nabob::NabobVersion, error::RestError};
use anyhow::{anyhow, Result};
pub use nabob_api_types::{
...
use nabob_api_types::{
    ...
    NabobError, NabobErrorCode, BcsBlock, Block, GasEstimation, HexEncodedBytes, IndexResponse,
...
use nabob_logger::{debug, info, sample, sample::SampleRate};
use nabob_types::{
...
const X_NABOB_SDK_HEADER_VALUE: &str = concat!("nabob-rust-sdk/", env!("CARGO_PKG_VERSION"));

type NabobResult<T> = Result<T, RestError>;
...
impl Client {
    pub fn builder(nabob_base_url: NabobBaseUrl) -> ClientBuilder {
        ClientBuilder::new(nabob_base_url)
    }

    pub fn new(base_url: Url) -> Self {
        Self::builder(NabobBaseUrl::Custom(base_url)).build()
    }
    ...
    pub fn version_path_base(mut self, version_path_base: String) -> NabobResult<Self> {
    ...
    pub fn build_path(&self, path: &str) -> NabobResult<Url> {
    ...
    pub async fn get_nabob_version(&self) -> NabobResult<Response<NabobVersion>> {
        self.get_resource::<NabobVersion>(CORE_CODE_ADDRESS, "0x1::version::Version")
            .await
    }
    ...
    pub async fn lookup_address(
        ...
        must_exist: bool,
    ) -> NabobResult<Response<AccountAddress>> {
        ...
        {
            Ok(inner) => Ok(inner),
            Err(RestError::Api(NabobErrorResponse {
                error:
                    NabobError {
                        error_code: NabobErrorCode::TableItemNotFound,
                        ..
                    },
                ..
            })) => {
    ...
    pub async fn view_bob_account_balance_at_version(
        &self,
        address: AccountAddress,
        version: u64,
    ) -> NabobResult<Response<u64>> {
        self.view_account_balance_bcs_impl(address, "0x1::nabob_coin::NabobCoin", Some(version))
            .await
    }

    pub async fn view_bob_account_balance(
        &self,
        address: AccountAddress,
    ) -> NabobResult<Response<u64>> {
        self.view_account_balance_bcs_impl(address, "0x1::nabob_coin::NabobCoin", None)
            .await
    }

    pub async fn get_index(&self) -> NabobResult<Response<IndexResponse>> {
        self.get(self.build_path("")?).await
    }

    pub async fn get_index_bcs(&self) -> NabobResult<Response<IndexResponseBcs>> {
        let url = self.build_path("")?;
        let response = self.get_bcs(url).await?;
        Ok(response.and_then(|inner| bcs::from_bytes(&inner))?)
    }

    // TODO: Remove this, just use `get_index`: https://github.com/nabob-labs/nabob/issues/5597.
    pub async fn get_ledger_information(&self) -> NabobResult<Response<State>> {
    ...
    pub async fn view(
        ...
        request: &ViewRequest,
        version: Option<u64>,
    ) -> NabobResult<Response<Vec<serde_json::Value>>> {
    ...
    ) -> NabobResult<Response<Vec<UserTransaction>>> {
    ...
    pub async fn simulate_bcs(
        &self,
        txn: &SignedTransaction,
    ) -> NabobResult<Response<TransactionOnChainData>> {
        let txn_payload = bcs::to_bytes(txn)?;
        let url = self.build_path("transactions/simulate")?;
    ...
    pub async fn submit(
        &self,
        txn: &SignedTransaction,
    ) -> NabobResult<Response<PendingTransaction>> {
        let txn_payload = bcs::to_bytes(txn)?;
    ...
    pub async fn submit_bcs(&self, txn: &SignedTransaction) -> NabobResult<Response<()>> {
        let txn_payload = bcs::to_bytes(txn)?;
        let url = self.build_path("transactions")?;
    ...
    ) -> NabobResult<Response<TransactionsBatchSubmissionResult>> {
    ...
    pub async fn submit_and_wait(
        &self,
        txn: &SignedTransaction,
    ) -> NabobResult<Response<Transaction>> {
    ...
    where
        F: Fn(HashValue) -> Fut,
        Fut: Future<Output = NabobResult<WaitForTransactionResult<T>>>,
    {
        ...
        loop {
            ...
            match fetch(hash).await {
                ...
                Ok(WaitForTransactionResult::NotFound(error)) => {
                    if let RestError::Api(nabob_error_response) = error {
                        if let Some(state) = nabob_error_response.state {
                            if expiration_timestamp_secs <= state.timestamp_usecs / 1_000_000 {
                                if reached_mempool {
            ...
            if let Some(max_server_lag_wait_duration) = max_server_lag_wait {
                if nabob_infallible::duration_since_epoch().as_secs()
                    ...
                    debug!(
                        ...
                        if let Some(timestamp_usecs) = chain_timestamp_usecs {
                            format!(
                                ...
                                nabob_infallible::duration_since_epoch()
    ...
    pub async fn get_transactions(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> NabobResult<Response<Vec<Transaction>>> {
        let url = self.build_path("transactions")?;
    ...
    pub async fn get_transactions_bcs(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> NabobResult<Response<Vec<TransactionOnChainData>>> {
        let url = self.build_path("transactions")?;
    ...
    pub async fn get_transaction_by_hash_bcs(
        &self,
        hash: HashValue,
    ) -> NabobResult<Response<TransactionData>> {
    ...
    pub async fn get_account_resources(
        &self,
        address: AccountAddress,
    ) -> NabobResult<Response<Vec<Resource>>> {
    ...
    pub async fn get_account_resources_bcs(
        &self,
        address: AccountAddress,
    ) -> NabobResult<Response<BTreeMap<StructTag, Vec<u8>>>> {
    ...
    pub async fn get_account_resource(
        ...
        address: AccountAddress,
        resource_type: &str,
    ) -> NabobResult<Response<Option<Resource>>> {
        let url = self.build_path(&format!(
            "accounts/{}/resource/{}",
            address.to_hex(),
            resource_type
    ...
    pub async fn get_account_modules(
        &self,
        address: AccountAddress,
    ) -> NabobResult<Response<Vec<MoveModuleBytecode>>> {
        self.paginate_with_cursor(
    ...
    pub async fn get_account_modules_bcs(
        &self,
        address: AccountAddress,
    ) -> NabobResult<Response<BTreeMap<MoveModuleId, Vec<u8>>>> {
        self.paginate_with_cursor_bcs(
            &format!("accounts/{}/modules", address.to_hex()),
            MODULES_PER_CALL_PAGINATION,
            None,
    ...
    pub async fn get_account_module(
        ...
        address: AccountAddress,
        module_name: &str,
    ) -> NabobResult<Response<MoveModuleBytecode>> {
        let url = self.build_path(&format!(
            "accounts/{}/module/{}",
            address.to_hex(),
            module_name
    ...
    pub async fn get_account_module_bcs(
        ...
        address: AccountAddress,
        module_name: &str,
    ) -> NabobResult<Response<bytes::Bytes>> {
        let url = self.build_path(&format!(
            "accounts/{}/module/{}",
            address.to_hex(),
            module_name
    ...
    pub async fn get_account_events(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> NabobResult<Response<Vec<VersionedEvent>>> {
        let url = self.build_path(&format!(
            "accounts/{}/events/{}/{}",
    ...
    pub async fn get_account_sequence_number(
        ...
        match res {
            ...
            Err(error) => match error {
                RestError::Api(error) => {
                    if matches!(error.error.error_code, NabobErrorCode::AccountNotFound) {
    ...
    pub async fn get_account_events_bcs(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> NabobResult<Response<Vec<EventWithVersion>>> {
        let url = self.build_path(&format!(
            "accounts/{}/events/{}/{}",
            address.to_hex_literal(),
            struct_tag,
    ...
    pub async fn get_account(&self, address: AccountAddress) -> NabobResult<Response<Account>> {
    ...
    pub async fn get_account_bcs(
        &self,
        address: AccountAddress,
    ) -> NabobResult<Response<AccountResource>> {
        let url = self.build_path(&format!("accounts/{}", address.to_hex()))?;
        let response = self.get_bcs(url).await?;
        Ok(response.and_then(|inner| bcs::from_bytes(&inner))?)
    }

    pub async fn estimate_gas_price(&self) -> NabobResult<Response<GasEstimation>> {
        let url = self.build_path("estimate_gas_price")?;
        let response = self.inner.get(url).send().await?;
        self.json(response).await
    }

    pub async fn set_failpoint(&self, name: String, actions: String) -> NabobResult<String> {
    ...
    ) -> NabobResult<(reqwest::Response, State)> {
        if !response.status().is_success() {
            Err(parse_error(response).await)
        } else {
    ...
    pub async fn health_check(&self, seconds: u64) -> NabobResult<()> {
    ...
    async fn get<T: DeserializeOwned>(&self, url: Url) -> NabobResult<Response<T>> {
        self.json(self.inner.get(url).send().await?).await
    }

    async fn get_bcs(&self, url: Url) -> NabobResult<Response<bytes::Bytes>> {
        let response = self.inner.get(url).header(ACCEPT, BCS).send().await?;
        self.check_and_parse_bcs_response(response).await
    }

    pub async fn make_prover_request(&self, req: ProverRequest) -> NabobResult<ZeroKnowledgeSig> {
    ...
    pub async fn make_pepper_request(&self, req: PepperRequest) -> NabobResult<Pepper> {
    ...
    where
        F: Fn() -> Fut,
        RetryFun: Fn(StatusCode, Option<NabobError>) -> bool,
        Fut: Future<Output = NabobResult<T>>,
    {
    ...
    pub async fn paginate_with_cursor_bcs<T: for<'a> Deserialize<'a> + Ord>(
        ...
        limit_per_request: u64,
        ledger_version: Option<u64>,
    ) -> NabobResult<Response<BTreeMap<T, Vec<u8>>>> {
...
pub fn retriable_with_404(status_code: StatusCode, nabob_error: Option<NabobError>) -> bool {
    retriable(status_code, nabob_error) | matches!(status_code, StatusCode::NOT_FOUND)
}

pub fn retriable(status_code: StatusCode, _nabob_error: Option<NabobError>) -> bool {
    matches!(
        status_code,
...
fn parse_state(response: &reqwest::Response) -> NabobResult<State> {
...
async fn parse_error(response: reqwest::Response) -> RestError {
    ...
    match response.json::<NabobError>().await {
