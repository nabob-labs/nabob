...
use nabob_crypto::{ed25519::Ed25519Signature, secp256r1_ecdsa, HashValue, PrivateKey, SigningKey};
use nabob_ledger::NabobLedgerError;
use nabob_rest_cli::{nabob_api_types::MoveStructTag, Client, PepperRequest, ProverRequest};
pub use nabob_types::*;
use nabob_types::{
    event::EventKey,
...
lazy_static! {
    pub static ref BOB_METADATA_ADDRESS: AccountAddress = {
        let mut addr = [0u8; 32];
        addr[31] = 10u8;
...
/// LocalAccount represents an account on the Nabob blockchain. Internally it
...
pub fn get_bob_primary_store_address(address: AccountAddress) -> AccountAddress {
    let mut bytes = address.to_vec();
    bytes.append(&mut AccountAddress::ONE.to_vec());
    bytes.push(0xFC);
    AccountAddress::from_bytes(nabob_crypto::hash::HashValue::sha3_256_of(&bytes).to_vec()).unwrap()
}
...
pub fn get_paired_fa_metadata_address(coin_type_name: &MoveStructTag) -> AccountAddress {
    ...
    if coin_type_name == NABOB_COIN_TYPE_STR {
        *BOB_METADATA_ADDRESS
    } else {
        let mut preimage = BOB_METADATA_ADDRESS.to_vec();
...
impl LocalAccount {
    ...
    /// does not actually create an account on the Nabob blockchain, just a
    ...
    /// create an account on the Nabob blockchain, it just generates a new
    /// account locally.
...
impl HardwareWalletAccount {
    ...
    /// This requires the Ledger device to be connected, unlocked and the Nabob app to be opened
    pub fn from_ledger(
        derivation_path: String,
        sequence_number: u64,
    ) -> Result<Self, NabobLedgerError> {
        let public_key = nabob_ledger::get_public_key(&derivation_path, false)?;
    ...
    pub fn sign_arbitrary_message(
        &self,
        message: &[u8],
    ) -> Result<Ed25519Signature, NabobLedgerError> {
        nabob_ledger::sign_message(&self.derivation_path, message)
    }
}
...
mod tests {
    ...
    use nabob_crypto::ed25519::Ed25519PrivateKey;
    use nabob_rest_cli::{NabobBaseUrl, FaucetClient};
    use reqwest::Url;

    #[test]
    fn test_recover_account_from_derive_path() {
        // Same constants in test cases of TypeScript
        // https://github.com/nabob-labs/nabob/blob/main/ecosystem/typescript/sdk/src/nabob_account.test.ts
