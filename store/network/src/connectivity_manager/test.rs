...
use nabob_crypto::{test_utils::TEST_SEED, x25519, Uniform};
use nabob_logger::info;
use nabob_time_svc::{MockTimeService, TimeService};
use nabob_types::{account_address::AccountAddress, network_address::NetworkAddress};
...
struct TestHarness {
    ...
    conn_mgr_reqs_tx: nabob_channels::Sender<ConnectivityRequest>,
}
...
impl TestHarness {
    fn new(seeds: PeerSet) -> (Self, ConnectivityManager<FixedInterval>) {
        ...
        let (conn_mgr_reqs_tx, conn_mgr_reqs_rx) = nabob_channels::new_test(0);
