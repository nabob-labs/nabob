...
use nabob_crypto::{noise, x25519};
use nabob_infallible::{duration_since_epoch, RwLock};
use nabob_logger::{error, trace};
use nabob_short_hex_str::{AsShortHexStr, ShortHexStr};
...
pub enum HandshakeAuthMode {
    ...
    ///
    /// For example, in the Nabob validator network, validator peers will only
                ...
                match trusted_peer {
                    ...
                    None => {
                        ...
                        let derived_remote_peer_id =
                            nabob_types::account_address::from_identity_public_key(
                                remote_public_key,
                            );
...
mod test {
    ...
    use nabob_config::config::{Peer, PeerRole, RoleType};
    use nabob_crypto::{
