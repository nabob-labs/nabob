...
use nabob_id_generator::{IdGenerator, U32IdGenerator};
use nabob_logger::prelude::*;
// Re-exposed for nabob-network-checker
pub use nabob_netcore::transport::tcp::{resolve_and_connect, TCPBufferCfg, TcpSocket};
use nabob_netcore::transport::{proxy_protocol, tcp, ConnectionOrigin, Transport};
use nabob_short_hex_str::AsShortHexStr;
use nabob_time_svc::{timeout, TimeService, TimeServiceTrait};
use nabob_types::{
    chain_id::ChainId,
...
/// tcp::Transport with Nabob-specific configuration applied.
pub const NABOB_TCP_TRANSPORT: tcp::TcpTransport = tcp::TcpTransport {
    // Use default options.
    ttl: None,
    // Use TCP_NODELAY for Nabob tcp connections.
    nodelay: Some(true),
...
async fn upgrade_inbound<T: TSocket>(
    ...
    // try to negotiate common nabobnet version and supported application protocols
...
// ideas in this PR thread: https://github.com/nabob-labs/nabob/pull/3478#issuecomment-617385633
pub struct NabobNetTransport<TTransport> {
...
impl<TTransport> NabobNetTransport<TTransport>
where
    ...
    TTransport::Listener: Send + 'static,
{
    ...
    fn parse_dial_addr(
        ...
        // parse out the nabobnet protocols (noise ik and handshake)
...
// If using `NabobNetTransport` as a `Transport` trait, then all upgrade futures
...
impl<TTransport: Transport> Transport for NabobNetTransport<TTransport>
where
