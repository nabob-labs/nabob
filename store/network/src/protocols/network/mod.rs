...
//! Convenience Network API for Nabob
...
/// Configuration needed for the client side of NabobNet applications
#[derive(Clone)]
...
/// Configuration needed for the service side of NabobNet applications
#[derive(Clone)]
pub struct NetworkServiceConfig {
    ...
    pub inbound_queue_config: nabob_channel::Config,
}
...
impl NetworkServiceConfig {
    pub fn new(
        direct_send_protocols_and_preferences: Vec<ProtocolId>,
        rpc_protocols_and_preferences: Vec<ProtocolId>,
        inbound_queue_config: nabob_channel::Config,
    ) -> Self {
        Self {
            direct_send_protocols_and_preferences,
            rpc_protocols_and_preferences,
...
/// Configuration needed for NabobNet applications to register with the network
...
pub trait NewNetworkEvents {
    fn new(
        peer_mgr_notifs_rx: nabob_channel::Receiver<(PeerId, ProtocolId), ReceivedMessage>,
...
/// a thin wrapper on `nabob_channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>`,
