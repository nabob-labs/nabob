...
use nabob_config::{config::PeerRole, network_id::NetworkContext};
use nabob_logger::info;
use nabob_memsocket::MemorySocket;
...
fn build_test_peer(
    ...
    PeerHandle,
    MemorySocket,
    nabob_channels::Receiver<TransportNotification<MemorySocket>>,
) {
    ...
    let (connection_notifs_tx, connection_notifs_rx) = nabob_channels::new_test(1);
    let (peer_reqs_tx, peer_reqs_rx) =
        nabob_channel::new(QueueStyle::FIFO, NETWORK_CHANNEL_SIZE, None);
...
async fn assert_disconnected_event(
    ...
    connection_notifs_rx: &mut nabob_channels::Receiver<TransportNotification<MemorySocket>>,
) {
...
#[derive(Clone)]
struct PeerHandle(nabob_channel::Sender<ProtocolId, PeerRequest>);
...
fn test_upstream_handlers() -> (
    ...
    let (sender, receiver) = nabob_channel::new(QueueStyle::FIFO, 100, None);
