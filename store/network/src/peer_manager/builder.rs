...
use crate::{
    ...
    transport::{self, NabobNetTransport, Connection, NABOB_TCP_TRANSPORT},
    ProtocolId,
};
use nabob_channels::{self, nabob_channel, message_queues::QueueStyle};
use nabob_config::{config::HANDSHAKE_VERSION, network_id::NetworkContext};
use nabob_crypto::x25519;
use nabob_logger::prelude::*;
#[cfg(any(test, feature = "testing", feature = "fuzzing"))]
use nabob_netcore::transport::memory::MemoryTransport;
use nabob_netcore::transport::{
    tcp::{TCPBufferCfg, TcpSocket, TcpTransport},
    Transport,
};
use nabob_time_svc::TimeService;
use nabob_types::{chain_id::ChainId, network_address::NetworkAddress, PeerId};
...
struct PeerManagerContext {
    ...
    pm_reqs_tx: nabob_channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>,
    pm_reqs_rx: nabob_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
    connection_reqs_tx: nabob_channel::Sender<PeerId, ConnectionRequest>,
    connection_reqs_rx: nabob_channel::Receiver<PeerId, ConnectionRequest>,

    peers_and_metadata: Arc<PeersAndMetadata>,
    upstream_handlers:
        HashMap<ProtocolId, nabob_channel::Sender<(PeerId, ProtocolId), ReceivedMessage>>,
...
impl PeerManagerContext {
    ...
    fn new(
        ...
        upstream_handlers: HashMap<
            ProtocolId,
            nabob_channel::Sender<(PeerId, ProtocolId), ReceivedMessage>,
        >,
        connection_event_handlers: Vec<conn_notifs_channel::Sender>,
    ...
    fn add_upstream_handler(
        ...
        channel: nabob_channel::Sender<(PeerId, ProtocolId), ReceivedMessage>,
    ) -> &mut Self {
...
type MemoryPeerManager =
    PeerManager<NabobNetTransport<MemoryTransport>, NoiseStream<nabob_memsocket::MemorySocket>>;
type TcpPeerManager = PeerManager<NabobNetTransport<TcpTransport>, NoiseStream<TcpSocket>>;
...
impl PeerManagerBuilder {
    ...
    pub fn create(
        ...
        let (pm_reqs_tx, pm_reqs_rx) = nabob_channel::new(
            QueueStyle::FIFO,
            channel_size,
        ...
        let (connection_reqs_tx, connection_reqs_rx) =
            nabob_channel::new(QueueStyle::FIFO, channel_size, None);
    ...
    pub fn connection_reqs_tx(&self) -> nabob_channel::Sender<PeerId, ConnectionRequest> {
    ...
    pub fn build(&mut self, executor: &Handle) -> &mut Self {
        use nabob_types::network_address::Protocol::*;
        ...
        let mut nabob_tcp_transport = NABOB_TCP_TRANSPORT.clone();
        let tcp_cfg = self.get_tcp_buffers_cfg();
        nabob_tcp_transport.set_tcp_buffers(&tcp_cfg);
    ...
    ) -> nabob_channel::Receiver<(PeerId, ProtocolId), ReceivedMessage> {
        // Register the direct send and rpc protocols
        self.transport_context()
            .add_protocols(&config.direct_send_protocols_and_preferences);
        self.transport_context()
