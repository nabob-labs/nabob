...
use nabob_channels::{self, nabob_channel};
...
pub struct PeerManagerRequestSender {
    inner: nabob_channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>,
}
...
pub struct ConnectionRequestSender {
    inner: nabob_channel::Sender<PeerId, ConnectionRequest>,
}

impl PeerManagerRequestSender {
    /// Construct a new PeerManagerRequestSender with a raw channel::Sender
    pub fn new(inner: nabob_channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>) -> Self {
        Self { inner }
    }
...
impl ConnectionRequestSender {
    /// Construct a new ConnectionRequestSender with a raw nabob_channel::Sender
    pub fn new(inner: nabob_channel::Sender<PeerId, ConnectionRequest>) -> Self {
        Self { inner }
    }
