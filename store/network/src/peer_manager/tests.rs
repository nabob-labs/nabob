...
fn build_test_peer_manager(
    ...
    PeerManager<
        ...
        MemorySocket,
    >,
    nabob_channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>,
    nabob_channel::Sender<PeerId, ConnectionRequest>,
    conn_notifs_channel::Receiver,
) {
    let (peer_manager_request_tx, peer_manager_request_rx) =
        nabob_channel::new(QueueStyle::FIFO, 1, None);
    let (connection_reqs_tx, connection_reqs_rx) = nabob_channel::new(QueueStyle::FIFO, 1, None);
    let (hello_tx, _hello_rx) = nabob_channel::new(QueueStyle::FIFO, 1, None);
