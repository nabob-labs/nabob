...
use crate::{
    schema::state_value_by_key_hash::StateValueByKeyHashSchema, state_kv_db::StateKvDb, NabobDB,
};
use nabob_config::config::{RocksdbConfig, StorageDirPaths};
use nabob_crypto::{hash::CryptoHash, HashValue};
use nabob_db_indexer::db_ops::open_internal_indexer_db;
...
use nabob_schemadb::{ReadOptions, DB};
use nabob_storage_interface::{DbReader, Result};
use nabob_types::{
...
pub fn validate_db_data(
    ...
    let nabob_db = NabobDB::new_for_test_with_sharding(db_root_path, 1000000);
    let batch_size = 20_000;
    let start_version = nabob_db.get_first_txn_version()?.unwrap();
    target_ledger_version = std::cmp::min(
        nabob_db.get_synced_version()?.unwrap(),
