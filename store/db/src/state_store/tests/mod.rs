...
use nabob_storage_interface::{DbReader, DbWriter, StateSnapshotReceiver};
...
fn test_get_values_by_key_prefix() {
    ...
    let key4 = StateKey::resource_typed::<CoinInfoResource<NabobCoinType>>(&address).unwrap();
    ...
    let key5 = StateKey::resource_typed::<CoinStoreResource<NabobCoinType>>(&address1).unwrap();
...
proptest! {
    ...
    fn test_raw_restore(
        ...
        let db1 = NabobDB::new_for_test(&tmp_dir1);
        ...
        let db2 = NabobDB::new_for_test(&tmp_dir2);
    ...
    fn test_get_rightmost_leaf_with_sharding(
        ...
        let tmp_dir1 = TempPath::new();
        let db1 = NabobDB::new_for_test_with_sharding(&tmp_dir1, 1000);
        let store1 = &db1.state_store;
        init_store(store1, input.clone().into_iter());
        ...
        let tmp_dir2 = TempPath::new();
        let db2 = NabobDB::new_for_test_with_sharding(&tmp_dir2, 1000);
    ...
    fn test_get_usage(
        ...
        for batch in input {
            ...
            let tmp_dir = TempPath::new();
            let db2 = NabobDB::new_for_test(&tmp_dir);
