...
use nabob_db_indexer::db_indexer::InternalIndexerDB;
use nabob_db_indexer_schemas::{
...
use nabob_jellyfish_merkle::iterator::JellyfishMerkleIterator;
use nabob_logger::info;
use nabob_metrics_core::TimerHelper;
use nabob_schemadb::batch::{NativeBatch, SchemaBatch, WriteBatch};
...
use nabob_storage_interface::{
    ...
    NabobDbError, DbReader, Result, StateSnapshotReceiver,
};
use nabob_types::{
...
impl StateStore {
    ...
    pub fn catch_up_state_merkle_db(
        ...
        use nabob_config::config::NO_OP_STORAGE_PRUNER_CONFIG;
    ...
    // TODO: change to async comment once it does like https://github.com/nabob-labs/nabob/blob/159b00f3d53e4327523052c1b99dd9889bf13b03/storage/backup/backup-cli/src/backup_types/state_snapshot/restore.rs#L147 or overlap at least two chunks.
    ...
    pub fn get_all_jmt_nodes_referenced(
        &self,
        version: Version,
    ) -> Result<Vec<nabob_jellyfish_merkle::node_type::NodeKey>> {
        nabob_jellyfish_merkle::JellyfishMerkleTree::new(self.state_merkle_db.as_ref())
            .get_all_nodes_referenced(version)
            .map_err(Into::into)
    }

    #[cfg(test)]
    pub fn get_all_jmt_nodes(&self) -> Result<Vec<nabob_jellyfish_merkle::node_type::NodeKey>> {
        let mut iter = self
            .state_db
            .state_merkle_db
            .metadata_db()
        ...
        let mut keys: Vec<nabob_jellyfish_merkle::node_type::NodeKey> =
...
mod test_only {
    ...
    use nabob_schemadb::batch::SchemaBatch;
    use nabob_storage_interface::state_store::{
