...
use crate::NabobDB;
use anyhow::anyhow;
use nabob_config::config::{NodeConfig, StorageDirPaths};
...
/// This is a wrapper around [NabobDB] that is used to bootstrap the node for fast sync mode
pub struct FastSyncStorageWrapper {
    // Used for storing genesis data during fast sync
    temporary_db_with_genesis: Arc<NabobDB>,
    // Used for restoring fast sync snapshot and all the read/writes afterwards
    db_for_fast_sync: Arc<NabobDB>,
...
impl FastSyncStorageWrapper {
    ...
    /// Otherwise, we return NabobDB directly and the FastSyncStorageWrapper is None
    pub fn initialize_dbs(
        config: &NodeConfig,
        internal_indexer_db: Option<InternalIndexerDB>,
        update_sender: Option<Sender<(Instant, Version)>>,
    ) -> Result<Either<NabobDB, Self>> {
        let mut db_main = NabobDB::open(
    ...
    pub fn get_fast_sync_db(&self) -> Arc<NabobDB> {
        self.db_for_fast_sync.clone()
    }

    pub fn get_temporary_db_with_genesis(&self) -> Arc<NabobDB> {
    ...
    pub(crate) fn get_nabob_db_read_ref(&self) -> &NabobDB {
    ...
    pub(crate) fn get_nabob_db_write_ref(&self) -> &NabobDB {
...
impl DbWriter for FastSyncStorageWrapper {
    ...
    fn finalize_state_snapshot(
        ...
        self.get_nabob_db_write_ref().finalize_state_snapshot(
            version,
            output_with_proof,
            ledger_infos,
        )?;
