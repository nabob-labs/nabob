...
use nabob_accumulator::{HashReader, MerkleAccumulator};
use nabob_crypto::{
    hash::{CryptoHash, TransactionAccumulatorHasher, SPARSE_MERKLE_PLACEHOLDER_HASH},
    HashValue,
};
use nabob_infallible::Mutex;
use nabob_logger::debug;
use nabob_scratchpad::SparseMerkleTree;
use nabob_storage_interface::{
    ...
    NabobDbError, DbReader, DbWriter, LedgerSummary, MAX_REQUEST_LIMIT,
};
use nabob_types::{
...
/// Alternate implementation of [NabobDB] for use with consensus-only-perf-test feature.
/// It stores and serves data from in-memory data structures as opposed to [NabobDB],
/// which uses RocksDB. Note that FakeNabobDB re-implements only a subset of the
/// features of [NabobDB] while passing through remaining features to the wrapped inner
/// [NabobDB].
pub struct FakeNabobDB {
    inner: NabobDB,
...
impl FakeNabobDB {
    ...
    fn save_and_compute_root_hash(
        ...
        let (root_hash, writes) =
            MerkleAccumulator::<FakeNabobDB, TransactionAccumulatorHasher>::append(
                self,
    ...
    fn get_frozen_subtree_hashes(&self, num_transactions: u64) -> Result<Vec<HashValue>> {
        MerkleAccumulator::<FakeNabobDB, TransactionAccumulatorHasher>::get_frozen_subtree_hashes(
            self,
            num_transactions,
        )
        .map_err(Into::into)
...
impl DbWriter for FakeNabobDB {
    fn get_state_snapshot_receiver(
        &self,
        version: Version,
        expected_root_hash: HashValue,
    ) -> Result<Box<dyn nabob_storage_interface::StateSnapshotReceiver<StateKey, StateValue>>> {
        self.inner
...
impl DbReader for FakeNabobDB {
    fn get_epoch_ending_ledger_infos(
        &self,
        start_epoch: u64,
        end_epoch: u64,
    ) -> Result<nabob_types::epoch_change::EpochChangeProof> {
    ...
    fn get_events(
        &self,
        event_key: &nabob_types::event::EventKey,
        start: u64,
        order: nabob_storage_interface::Order,
        limit: u64,
        ledger_version: Version,
    ...
    fn get_block_timestamp(&self, version: Version) -> Result<u64> {
        gauged_api("get_block_timestamp", || {
            ...
            if timestamp > 0 {
                ...
                Err(NabobDbError::NotFound("NewBlockEvent".to_string()).into())
            }
        })
    }
    ...
    fn get_account_transactions(
        ...
        include_events: bool,
        ledger_version: Version,
    ) -> Result<nabob_types::transaction::AccountTransactionsWithProof> {
        self.inner
    ...
    fn get_state_value_by_version(
        ...
        // Since the genesis write set is persisted with NabobDB, we call
        // it to serve state values targeting the framework account
        // (to access NabobCoin, for example).
    ...
    fn get_pre_committed_ledger_summary(&self) -> Result<LedgerSummary> {
        // If the genesis is not executed yet, we need to get the executed trees from the inner NabobDB
        // This is because when we call save_transactions for the genesis block, we call [NabobDB::save_transactions]
        // where there is an expectation that the root of the SMTs are the same pointers. Here,
        // we get from the inner NabobDB which ensures that the pointers match when save_transactions is called.
    ...
    fn get_latest_version(&self) -> Result<Version> {
        gauged_api("get_latest_version", || {
            self.latest_version
                .lock()
                .ok_or_else(|| NabobDbError::NotFound("No latest version found.".to_string()))
        })
    }
...
/// requests.
impl HashReader for FakeNabobDB {
...
fn error_if_too_many_requested(num_requested: u64, max_allowed: u64) -> Result<()> {
    if num_requested > max_allowed {
        Err(NabobDbError::TooManyRequested(num_requested, max_allowed).into())
    } else {
        Ok(())
    }
}
...
mod tests {
    ...
    proptest! {
        ...
        fn test_save_transactions(input in arb_blocks_to_commit()) {
            ...
            let db = FakeNabobDB::new(NabobDB::new_for_test(&tmp_dir));
