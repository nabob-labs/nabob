...
use nabob_storage_interface::state_store::state::State;
use nabob_storage_interface::state_store::state_summary::StateSummary;
use nabob_storage_interface::state_store::state_view::hot_state_view::HotStateView;
use nabob_types::block_info::BlockHeight;

impl DbReader for NabobDB {
    ...
    /// Returns the transaction by version, delegates to `NabobDB::get_transaction_with_proof`.
    ...
    fn get_table_info(&self, handle: TableHandle) -> Result<TableInfo> {
        gauged_api("get_table_info", || {
            self.get_table_info_option(handle)?
                .ok_or_else(|| NabobDbError::NotFound(format!("TableInfo for {:?}", handle)))
        })
    }
