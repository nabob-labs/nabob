...
use nabob_metrics_core::IntGaugeHelper;
use nabob_storage_interface::block_info::BlockInfo;
...
impl NabobDB {
    ...
    fn open_indexer(
        ...
        info!("NabobDB Indexer caught up.");
    ...
    fn get_raw_block_info_by_height(&self, block_height: u64) -> Result<BlockInfo> {
        if !self.skip_index_and_usage {
            ...
            Ok(self
                ...
                .ok_or_else(|| {
                    NabobDbError::NotFound(format!("BlockInfo not found at height {block_height}"))
                })?)
        }
    }
    ...
    fn get_raw_block_info_by_version(
        ...
        if !self.skip_index_and_usage {
            let (first_version, event_index, block_height) = self
                ...
                .ok_or_else(|| NabobDbError::NotFound("NewBlockEvent".to_string()))?;
    ...
    fn to_api_block_info(
        ...
        let last_version = match self.get_raw_block_info_by_height(block_height + 1) {
            ...
            Err(NabobDbError::NotFound(..)) => committed_version,
...
fn error_if_too_many_requested(num_requested: u64, max_allowed: u64) -> Result<()> {
    if num_requested > max_allowed {
        Err(NabobDbError::TooManyRequested(num_requested, max_allowed))
    } else {
        Ok(())
    }
}
