...
use nabob_schemadb::{iterator::SchemaIterator, ReadOptions};
    ...
    fn expect_continuous_versions(
        ...
        Ok(ContinuousVersionIter {
            ...
            end_version: first_version
                ...
                .ok_or(NabobDbError::TooManyRequested(first_version, limit as u64))?,
...
impl<'a> PrefixedStateValueIterator<'a> {
    pub fn new(
        ...
        // nabob/abc|0
        // nabob/abd|1
        // if we seek('nabob/'), and call next, we may not reach `nabob/abd/1` because the prefix extractor we adopted
        // here will stick with prefix `nabob/abc` and return `None` or any arbitrary result after visited all the
        // keys starting with `nabob/abc`.
    ...
    fn next_impl(&mut self) -> Result<Option<Vec<ContractEvent>>> {
        ...
        while let Some(res) = self.inner.peek() {
            let ((version, _index), _event) = res
                .as_ref()
                .map_err(|e| NabobDbError::Other(format!("Hit error iterating events: {}", e)))?;
        ...
        self.expected_next_version = self
            .expected_next_version
            .checked_add(1)
            .ok_or_else(|| NabobDbError::Other("expected version overflowed.".to_string()))?;
        Ok(Some(ret))
    }
}
