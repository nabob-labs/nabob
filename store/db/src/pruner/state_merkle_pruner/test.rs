...
use nabob_config::config::{LedgerPrunerConfig, StateMerklePrunerConfig};
use nabob_crypto::{hash::CryptoHash, HashValue};
use nabob_storage_interface::{state_store::NUM_STATE_SHARDS, DbReader};
...
fn test_state_store_pruner() {
    ...
    let nabob_db = NabobDB::new_for_test_no_cache(&tmp_dir);
    let state_store = &nabob_db.state_store;
    ...
    {
        let pruner =
            create_state_merkle_pruner_manager(&nabob_db.state_merkle_db(), prune_batch_size);
...
fn test_state_store_pruner_partial_version() {
    ...
    let tmp_dir = TempPath::new();
    let nabob_db = NabobDB::new_for_test_with_sharding(&tmp_dir, 0);
    let state_store = &nabob_db.state_store;
    ...
    if nabob_db.state_merkle_db().sharding_enabled() {
...
fn verify_state_value_pruner(inputs: Vec<Vec<(StateKey, Option<StateValue>)>>) {
    let tmp_dir = TempPath::new();
    let db = NabobDB::new_for_test(&tmp_dir);
