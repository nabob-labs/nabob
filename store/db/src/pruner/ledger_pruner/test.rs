...
use crate::{
    schema::version_data::VersionDataSchema, NabobDB, LedgerPrunerManager, LedgerStore,
    PrunerManager, TransactionStore,
};
use nabob_accumulator::HashReader;
use nabob_config::config::LedgerPrunerConfig;
use nabob_schemadb::SchemaBatch;
...
fn verify_write_set_pruner(write_sets: Vec<WriteSet>) {
    ...
    let nabob_db = NabobDB::new_for_test(&tmp_dir);
    let transaction_store = &nabob_db.transaction_store;
    let num_write_sets = write_sets.len();

    let pruner = LedgerPrunerManager::new(Arc::clone(&nabob_db.ledger_db), LedgerPrunerConfig {
        enable: true,
        prune_window: 0,
        batch_size: 1,
...
fn verify_txn_store_pruner(
    ...
    let nabob_db = NabobDB::new_for_test(&tmp_dir);
    let transaction_store = &nabob_db.transaction_store;
    let ledger_store = LedgerStore::new(Arc::clone(&nabob_db.ledger_db));
    ...
    for i in (0..=num_transaction).step_by(step_size) {
        ...
        // logic.
        let pruner =
            LedgerPrunerManager::new(Arc::clone(&nabob_db.ledger_db), LedgerPrunerConfig {
                enable: true,
                prune_window: 0,
                batch_size: 1,
                user_pruning_window_offset: 0,
        ...
        for j in 0..i {
            ...
            assert!(nabob_db.state_store.get_usage(Some(j as u64)).is_err());
        }
        // ensure all other are valid in DB
        for j in i..num_transaction {
            ...
            nabob_db.get_accumulator_summary(j as Version).unwrap();
            assert!(nabob_db.state_store.get_usage(Some(j as u64)).is_ok());
        }
