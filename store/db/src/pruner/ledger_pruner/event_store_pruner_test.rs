...
use crate::{NabobDB, EventStore, LedgerPrunerManager, PrunerManager};
use nabob_config::config::LedgerPrunerConfig;
use nabob_proptest_helpers::Index;
...
fn verify_event_store_pruner(events: Vec<Vec<ContractEvent>>) {
    ...
    let event_store = &nabob_db.event_store;
    ...
    nabob_db.ledger_db.event_db().write_schemas(batch).unwrap();
