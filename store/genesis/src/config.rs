...
use nabob_crypto::{bls12381, ed25519::Ed25519PublicKey, x25519};
use nabob_types::{
...
use nabob_vm_genesis::{AccountBalance, EmployeePool, Validator, ValidatorWithCommissionRate};
        ...
        for (i, pool) in map.inner.into_iter().enumerate() {
            ...
            if pool.validator.join_during_genesis {
                ...
                if pool.validator.stake_amount < 100000000000000 {
                    errors.push(anyhow::anyhow!(
                        "Employee pool #{} is setup to join during genesis but has a low stake amount {} < 1000000 BOB",
                        i,
