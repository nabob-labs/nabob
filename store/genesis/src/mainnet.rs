...
use nabob_vm_genesis::{AccountBalance, EmployeePool, ValidatorWithCommissionRate};
...
impl MainnetGenesisInfo {
    ...
    fn generate_genesis_txn(&self) -> Transaction {
        nabob_vm_genesis::encode_nabob_mainnet_genesis_transaction(
            &self.accounts,
            &self.employee_vesting_accounts,
            &self.validators,
            &self.framework,
            self.chain_id,
            &nabob_vm_genesis::GenesisConfiguration {
    ...
    pub fn generate_waypoint(&mut self) -> anyhow::Result<Waypoint> {
        ...
        let db_rw = DbReaderWriter::new(nabobdb);
        nabob_executor::db_bootstrapper::generate_waypoint::<NabobVMBlockExecutor>(&db_rw, genesis)
    }
}
