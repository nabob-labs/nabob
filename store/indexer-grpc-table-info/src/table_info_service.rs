...
use nabob_api::context::Context as ApiContext;
use nabob_api_types::TransactionOnChainData;
use nabob_db_indexer::db_v2::IndexerAsyncV2;
use nabob_indexer_grpc_fullnode::stream_coordinator::{
    IndexerStreamCoordinator, TransactionBatchInfo,
};
use nabob_indexer_grpc_utils::counters::{log_grpc_step, IndexerGrpcStep};
use nabob_logger::{debug, error, info, sample, sample::SampleRate};
use nabob_types::write_set::WriteSet;
...
impl TableInfoService {
        ...
        let backup_is_enabled = match self.backup_restore_operator.clone() {
            Some(backup_restore_operator) => {
                ...
                let _task = tokio::spawn(async move {
                    loop {
                        nabob_logger::info!("[Table Info] Checking for snapshots to backup.");
    ...
    async fn backup_snapshot_if_present(
        ...
        if epochs_to_backup.is_empty() {
            ...
            nabob_logger::info!("[Table Info] No snapshot to backup. Skipping the backup.");
            return;
        }
        nabob_logger::info!(
