...
use nabob_metrics_core::{Encoder, TextEncoder};
use std::{
    env,
    ops::Sub,
    sync::mpsc,
...
const DEFAULT_DASHBOARD_BASE_URL: &str = "https://naboblabs.grafana.net/d/execution/execution";
...
impl MetricsPusher {
    fn push(
        ...
        if let Err(e) = TextEncoder::new().encode(&nabob_metrics_core::gather(), &mut buffer) {
