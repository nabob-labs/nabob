...
fn test_nil_block() {
    ...
    let nil_block_child = Block::new_proposal(
        ...
        2,
        nabob_infallible::duration_since_epoch().as_micros() as u64,
        nil_block_qc,
        &signer,
        Vec::new(),
    )
...
fn test_same_qc_different_authors() {
    ...
    let round = 1;
    let payload = Payload::empty(false, true);
    let current_timestamp = nabob_infallible::duration_since_epoch().as_micros() as u64;
...
fn test_block_metadata_bitvec() {
    ...
    let payload = Payload::empty(false, true);
    let start_round = 1;
    let start_timestamp = nabob_infallible::duration_since_epoch().as_micros() as u64;
