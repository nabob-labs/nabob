...
use nabob_config::config::internal_indexer_db_config::InternalIndexerDBConfig;
...
use nabob_storage_interface::{
    db_ensure as ensure, db_other_bail as bail, NabobDbError, DbReader, Result,
};
use nabob_types::{
    ...
    pub fn get_account_transaction_version_iter(
        ...
        Ok(AccountTransactionVersionIter::new(
            ...
            min_seq_num
                ...
                .ok_or(NabobDbError::TooManyRequested(min_seq_num, num_versions))?,
            ledger_version,
        ))
    }
...
impl DBIndexer {
    ...
    pub fn process_a_batch(&self, start_version: Version, end_version: Version) -> Result<Version> {
        let _timer: nabob_metrics_core::HistogramTimer =
