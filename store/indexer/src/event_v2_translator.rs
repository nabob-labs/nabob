...
use nabob_db_indexer_schemas::schema::event_sequence_number::EventSequenceNumberSchema;
use nabob_schemadb::DB;
use nabob_storage_interface::{
    state_store::state_view::db_state_view::LatestDbStateCheckpointView, NabobDbError, DbReader,
    Result,
};
use nabob_types::{
    account_config::{
...
impl EventV2Translator for TokenMutationTranslator {
    fn translate_event_v2_to_v1(
        ...
        let (key, sequence_number) = if let Some(state_value_bytes) = engine
            ...
            return Err(NabobDbError::from(anyhow::format_err!(
