...
use crate::{
    get_hash,
    metrics::{NABOB_JELLYFISH_INTERNAL_ENCODED_BYTES, NABOB_JELLYFISH_LEAF_ENCODED_BYTES},
    Key, TreeReader,
};
    ...
    pub fn encode(&self) -> Result<Vec<u8>> {
        ...
        match self {
            Node::Internal(internal_node) => {
                ...
                NABOB_JELLYFISH_INTERNAL_ENCODED_BYTES.inc_by(out.len() as u64);
            },
            Node::Leaf(leaf_node) => {
                out.push(NodeTag::Leaf as u8);
                out.extend(bcs::to_bytes(&leaf_node)?);
                NABOB_JELLYFISH_LEAF_ENCODED_BYTES.inc_by(out.len() as u64);
            },
            Node::Null => {
