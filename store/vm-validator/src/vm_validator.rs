...
pub trait TransactionValidation: Send + Sync + Clone {
    type ValidationInstance: nabob_vm::VMValidator;
...
fn new_vm_for_validation(state_view: &impl StateView) -> NabobVM {
    info!(
        AdapterLogSchema::new(state_view.id(), 0),
        "NabobVM created for Validation"
    );
    let env = NabobEnvironment::new(state_view);
    NabobVM::new(env, state_view)
}
...
struct ValidationState<S> {
    ...
    module_cache: UnsyncModuleCache<ModuleId, CompiledModule, Module, NabobModuleExtension, usize>,
    vm: NabobVM,
}
...
impl<S: StateView> ModuleCache for ValidationState<S> {
    ...
    type Extension = NabobModuleExtension;
    type Key = ModuleId;
    ...
    > {
        ...
        Ok(if module.extension().hash() == &hash {
            ...
            let extension = Arc::new(NabobModuleExtension::new(state_value));
...
impl TransactionValidation for PooledVMValidator {
    type ValidationInstance = NabobVM;
...
mod tests {
    use super::*;
    use nabob_types::state_store::{state_value::StateValue, MockStateView};
