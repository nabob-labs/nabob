...
use nabob_crypto::{ed25519::Ed25519PrivateKey, PrivateKey, Uniform};
use nabob_db::NabobDB;
use nabob_gas_schedule::{InitialGasSchedule, TransactionGasParameters};
...
struct TestValidator {
    ...
    _db_path: nabob_temppath::TempPath,
}

impl TestValidator {
    fn new() -> Self {
        let _db_path = nabob_temppath::TempPath::new();
        _db_path.create_as_dir().unwrap();
        let (db, db_rw) = DbReaderWriter::wrap(NabobDB::new_for_test(_db_path.path()));
        nabob_executor_test_helpers::bootstrap_genesis::<NabobVMBlockExecutor>(
            &db_rw,
            &nabob_vm_genesis::test_genesis_transaction(),
        )
...
//   language e2e tests in: nabob/language/e2e-testsuite/src/tests/verify_txn.rs ->
...
// * SENDING_ACCOUNT_FROZEN: Tested in functional-tests/tests/nabob_account/freezing.move.
...
fn test_validate_transaction() {
    ...
    let address = account_config::nabob_test_root_address();
    let program = nabob_stdlib::nabob_coin_mint(address, 100);
    let transaction = transaction_test_helpers::get_test_signed_txn(
        address,
        1,
        &nabob_vm_genesis::GENESIS_KEYPAIR.0,
        nabob_vm_genesis::GENESIS_KEYPAIR.1.clone(),
        Some(program),
    );
...
fn test_validate_invalid_signature() {
    ...
    let address = account_config::nabob_test_root_address();
    let program = nabob_stdlib::nabob_coin_transfer(address, 100);
...
fn test_get_account_sequence_number() {
    let vm_validator = TestValidator::new();
    let root_address = account_config::nabob_test_root_address();
